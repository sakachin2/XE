//*CID://+vbAsR~:                             update#= 1219;       //+vbAsR~
//*************************************************************
//*xefcmd43.c*                                                     //~v74VR~//~va8kR~
//**file cmd:JOIN,split from fcmd42.c                        //~v74VR~//~va8kR~
//*************************************************************
//vbAs:240703 (Bug)cutted --> cut                                  //+vbAsI~
//vb77:170103 (BUG) join cmd operand check err;n[-n-s] option cuase error//~vb77I~
...
{                                                                  //~v45xI~
    ...
    int rc;                                                        //~v53NI~
    int bndsoffs1=0,bndsoffs2=0,cutoffs;                               //~v74WR~//~vaf9R~
    int contdelsw;                                                 //~v761I~
...
//  uerrmsg("%d line(s) cutted",                                   //~v45xI~//+vbAsR~
    uerrmsg("%d line(s) cut",                                      //+vbAsI~
            "%d 行を短縮｡",                                        //~v45xI~
                donectr);                                          //~v45xI~
    return 0;                                                      //~v45xI~
}//filesplitcut                                                    //~v74SR~
...
//*undoprep cutoff,set contflag only                               //~va50I~
    ...
    if (donectr)                                                   //~v45xI~
//      uerrmsg("%d line(s) cutted then added %d line",            //~v45xR~//+vbAsR~
        uerrmsg("%d line(s) cut then added %d line",               //+vbAsI~
                "分割行は %d行, %d 行追加｡",                       //~v45xR~
...
//  pcontwk=malloc((UINT)((width+1)<<1));	//data and dbcs,and read next 1 byte//~v76ER~
    ...
    {                                                              //~v60sI~
        ...
        {                                                          //~vagmI~
          if (undowidth)                                           //~vagmI~
	        filerenum(Ppcw,Ppfh,RENUMOPT_RENUM|RENUMOPT_NOUPCTR,0);	//0:numfld for seti,upctrreq in this func//~vagmR~
          else                                                     //~vagmI~
...
//  LONG lbandle=0,lskip=0,repeat;                                 //~v55JR~//~vaf9R~
    LONG lbandle=0,lskip=0,repeat=0;                               //~vaf9I~
	UCHAR *pc;                                                     //~v47iR~
	UCHAR *pc2;                                                    //~v55JI~
//*********************************                                //~v47iI~
...
//set start line                                                   //~v47iI~
  ...
  {                                                                //~v55JI~
    ...
    {                                                              //~v47iI~
        ...
        if (labsw)                                                 //~v47iI~
        {                                                          //~v47iI~
//          seq1=lineseqchk(plhc,plh1);                            //~v47iI~//~vb2ER~
            seq1=(int)lineseqchk(plhc,plh1);                       //~vb2EI~
//          seq2=lineseqchk(plhc,plh2);                            //~v47iI~//~vb2ER~
...
//	rc=lcmdjoin(Ppcw,plh1,plh2,repeat,(int)lbandle,(int)lskip);    //~v55JR~//~vb2ER~
  	rc=lcmdjoin(Ppcw,plh1,plh2,(int)repeat,(int)lbandle,(int)lskip);//~vb2EI~
  }                                                                //~v55JI~
  else                                                             //~v55JI~
  {                                                                //~v55JI~
    if (jointype)                                                  //~v47iI~
...
{                                                                  //~v47iI~
    PULINEH plh,plhc,plhn=NULL;                                         //~v47iR~//~vaf9R~
...
{                                                                  //~v47iI~
//  PULINEH plh,plhs=0,plhe,plhp;                                  //~v47iR~//~vaf9R~
    PULINEH plh,plhs=0,plhe=0,plhp;                                //~vaf9I~
...
#else                                                              //~va8kI~
//**************************************************************** //~va8kI~
// fcmdebcsplitlen                                                 //~va8wR~
//ebcdic strlen                                                    //~va8wR~
//rc: 2:shift to next line,4:err but use org len,0100:appendso     //~va8wI~
//**************************************************************** //~va8kM~
...
//*********************************                                //~va8kM~
    ...
    {                                                              //~va8kI~
	    ...
	    if (Pfuncopt & FCEBSLO_IRB2NDOK)	//cvuttable after dbcs2nd//~va8wI~
    		opt|=XEEBCDSLO_IRB2NDOK;                               //~va8wI~
//      if (Pdatalen==Pwidth+1 && *(Ppdata+Pwidth)==CHAR_SI)       //~va8wR~
//  		opt|=XEEBCDSLO_SIFOLLOWS;                              //~va8wR~
    }                                                              //~va8kI~
...
//                    rc=2;   //notify shift to next line          //~va8kR~
			if (rc2==3)	//SI overwrap to next dbcs1st of right boundary//~va8wI~
            {                                                      //~va8wI~
                newlen=splitlen;	//include last SI              //~va8wR~
        		rc|=FCSPLRC_DELSI;                                 //~va8wI~
            }                                                      //~va8wI~
            else                                                   //~va8wI~
...
//*********************************                                //~va8wI~
    ...
    if (*(Pdbcs1+Plen1-1)!=UDBCSCHK_DBCS2ND||*(Pdbcs2+1)!=UDBCSCHK_DBCS1ST)//~va8wI~
    	return 0;                                                  //~va8wI~
    return 1;                                                      //~va8wI~
}//fcmdebcchkdbcsconcatbndsupdateL                                 //~va8wR~
//**************************************************************** //~va8wI~
// dbcs string concatination chk to delete intermediate so/si      //~va8wI~
//  12SI+12-->1212                                                 //~va8wI~
// return 0:not avail,1:so/si deletable                            //~va8wI~
...
//*********************************                                //~va8wI~
    ...
    ||  (Popt & FCSPL_SBCS))                                       //~va8wI~
  	...
  	if (Sopt2 & SOPT2_SPLCRLF)	//display width change             //~va8wI~
    	return 0;                                                  //~va8wI~
    if (Plen1!=Pwidth+1||Plen1<3||Plen2<2)                         //~va8wR~
    	return 0;                                                  //~va8wI~
    if (*(Pdata1+Plen1-1)!=CHAR_SI||*(Pdbcs1-2)!=UDBCSCHK_DBCS2ND) //~va8wR~
    	return 0;                                                  //~va8wI~
    if (*(Pdbcs2)!=UDBCSCHK_DBCS1ST)                               //~va8wI~
    	return 0;                                                  //~va8wI~
    return 1;                                                      //~va8wI~
}//fcmdebcchkdbcsconcatbndsupdateR                                 //~va8wR~
//**************************************************************** //~va8wI~
// chk tgline dbcs concatinable                                    //~va8wI~
// rc:0x01:left boundary,02:right boundary                         //~va8wI~
//**************************************************************** //~va8wI~
...
{                                                                  //~va8wI~
    ...
    UCHAR *pcd;                                                    //~va8wR~
//*********************************                                //~va8wI~
	Sotheropt&=~(SOTHO_OLB2ND|SOTHO_ORB1ST);                       //~va8wI~
    if (!Pplh)                                                     //~va8wM~
        return 0;                                                  //~va8wI~
	if (!Sswebcfile                                                //~va8wI~
//  ||  (Popt & FCSPL_TFLOW)                                       //~va8wR~
    ||  (Popt & FCSPL_SBCS))                                       //~va8wI~
        return 0;                                                  //~va8wI~
    pcd=Pplh->ULHdbcs;                                             //~va8wI~
    if (Poffs1>0 && *(pcd+Poffs1-1)==UDBCSCHK_DBCS2ND)             //~va8wR~
    {                                                              //~va8wI~
    	rc|=1;                                                     //~va8wI~
        Sotheropt|=SOTHO_OLB2ND;                                   //~va8wI~
    }                                                              //~va8wI~
    if (Poffs2<Pplh->ULHlen                                        //~va8wR~
    && *(pcd+Poffs2)==UDBCSCHK_DBCS1ST)                            //~va8wR~
    {                                                              //~va8wI~
      if (Popt & FCSPL_NOWIDTH 	//if width parm,search last delm matched//~va8wI~
      ||  Pwidth==Poffs2)	//no meaning if width<bndsoffs2        //~va8wI~
      {                                                            //~va8wI~
    	rc|=2;                                                     //~va8wI~
        Sotheropt|=SOTHO_ORB1ST;                                   //~va8wI~
      }                                                            //~va8wI~
    }                                                              //~va8wI~
    return rc;                                                     //~va8wI~
}//fcmdebcchkdbcsconcatbndsupdate                                  //~va8wI~
//**************************************************************** //~va8wI~
// dbcs string concatination chk to delete intermediate so/si      //~va8wI~
// return delete count                                             //~va8wR~
//**************************************************************** //~va8wI~
int fcmdebcchkdbcsconcatbnds(int Popt,UCHAR *Pdata,UCHAR *Pdbcs,int Plen,int *Ppcutflag,PSPLITWK Ppspw,int Psrcoffs,int Pwidth,int Psplitlen)//~va8wR~
{                                                                  //~va8wI~
	UCHAR *pdata1,*pdbcs1,*pdata2,*pdbcs2;                         //~va8wR~
    int len1,len2,width,contlen,swdropso=0;                        //~va8wR~
    int rc=0,cutflag=0;                                            //~va8wR~
//*********************************                                //~va8wI~
    *Ppcutflag=0;                                                  //~va8wR~
	if (!Sswebcfile                                                //~va8wI~
//  ||  (Popt & (FCSPL_TFLOW|FCSPL_SBCS|FCSPL_ALLWIDTH0))          //~va8wR~
    ||  (Popt & (FCSPL_SBCS|FCSPL_ALLWIDTH0))                      //~va8wI~
    )                                                              //~va8wI~
        return 0;                                                  //~va8wI~
  	if (Sopt2 & SOPT2_SPLCRLF)	//display width change             //~va8wI~
    	return 0;                                                  //~va8wI~
    pdata1=Pdata+Plen;                                             //~va8wM~
    pdbcs1=Pdbcs+Plen;                                             //~va8wM~
    len1=Plen;                                                     //~va8wM~
    pdata2=Ppspw->SPWdatadbcs+Psrcoffs;                            //~va8wM~
    pdbcs2=Ppspw->SPWdatadbcs+Ppspw->SPWlen+Psrcoffs;              //~va8wM~
  if (Psplitlen>=0)                                                //~va8wI~
    len2=Psplitlen;                                                //~va8wI~
  else                                                             //~va8wI~
    len2=Ppspw->SPWlen-Psrcoffs;                                   //~va8wM~
    contlen=len1;                                                  //~va8wI~
  	if (Sotheropt & SOTHO_OLB2ND 	//tgt line lb is dbcs2nd       //~va8wR~
    &&  !len1	//concat top                                       //~va8wR~
    &&   len2	//concat top                                       //~va8wI~
    )                                                              //~va8wI~
    {                                                              //~va8wI~
        if (*pdata2==CHAR_SO)                                      //~va8wR~
        {                                                          //~va8wR~
            rc++;       //delete 1 byte                            //~va8wR~
            cutflag|=FCEBCCF_TOPSO|FCEBCCF_TOPCONCAT|FCEBCCF_HEADCONCAT;//~va8wR~
            swdropso=1;                                            //~va8wR~
        }                                                          //~va8wR~
        else                                                       //~va8wI~
        if (*pdbcs2==UDBCSCHK_DBCS1ST)                             //~va8wI~
            cutflag|=FCEBCCF_TOPCONCAT|FCEBCCF_HEADCONCAT;         //~va8wR~
    }                                                              //~va8wI~
    if (len1>=3                                                    //~va8wI~
    &&  !(Popt & FCSPL_TFLOW)	//intermediate join is split cmd only//~va8wI~
    &&  (*(pdata1-1)==CHAR_SI && *(pdbcs1-2)==UDBCSCHK_DBCS2ND)    //~va8wI~
    &&  len2>=3                                                    //~va8wR~
    &&  Pwidth-(len1-1)>=2                                         //~va8wR~
    &&  (*pdata2==CHAR_SO && *(pdbcs2+1)==UDBCSCHK_DBCS1ST)        //~va8wI~
    )                                                              //~va8wI~
    {                                                              //~va8wI~
    	rc+=2;                                                     //~va8wI~
        cutflag|=FCEBCCF_INTSOSI|FCEBCCF_HEADCONCAT;               //~va8wR~
        contlen--;                                                 //~va8wI~
        swdropso=1;                                                //~va8wI~
    }                                                              //~va8wI~
    width=Pwidth-contlen;                                          //~va8wI~
  	if (Sotheropt & SOTHO_ORB1ST)	//rb is dbcs1st                //~va8wR~
    {                                                              //~va8wM~
    	if (len2-swdropso>=width && *(pdbcs2+width+swdropso-1)==UDBCSCHK_DBCS2ND) //conatinatable to right boundary//~va8wR~
        {                                                          //~va8wI~
            cutflag|=FCEBCCF_ENDCONCAT;                            //~va8wM~
	    	if (len2-swdropso==width+1 && *(pdata2+swdropso+width)==CHAR_SI)//~va8wI~
    	    {                                                      //~va8wI~
        	    rc++;       //delete 1 byte                        //~va8wI~
            	cutflag|=FCEBCCF_ENDSI;                            //~va8wI~
        	}                                                      //~va8wI~
        }                                                          //~va8wI~
    }                                                              //~va8wM~
  	if (!(cutflag & FCEBCCF_HEADCONCAT))                           //~va8wI~
    	if (len2 && *pdbcs2==UDBCSCHK_DBCS1ST)                     //~va8wI~
            cutflag|=FCEBCCF_INSERTSO;                             //~va8wI~
    if (width>len2 && len2 && *(pdbcs2+len2-1)==UDBCSCHK_DBCS2ND)  //~va8wI~
    	cutflag|=FCEBCCF_APPENDSI;                                 //~va8wR~
    *Ppcutflag=cutflag;                                            //~va8wR~
    return rc;                                                     //~va8wR~
}//fcmdebcchkdbcsconcat                                            //~va8wI~
#endif    //!DOSDOS                                                //~va8kI~
