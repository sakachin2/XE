...
#define  CID_NOAPPEND  "NOAPPEND"                                  //~vbv6I~
    UCHAR labelt[2][ULHLINENOSZ+1];                                //~vbBwI~
    int lblcnt=0;                                                  //~vbBwI~
    PULINEH plht[2]={0,0};  	//output of lcmdgetabplh           //+vbBwR~
//*********************************
...
//*operand chk
	...
	{
        if (*pc==ULCCMDLABEL)    //start by '.'                    //~vbBwI~
        {                                                          //~vbBwI~
          if (lblcnt>=2)                                           //~vbBwI~
              return fcmdlabelerr(pc);                             //~vbBwI~
          strncpy(labelt[lblcnt],pc,ULHLINENOSZ);                  //~vbBwI~
          labelt[lblcnt][ULHLINENOSZ]=0;                           //~vbBwI~
          lblcnt++;                                                //~vbBwI~
          continue;                                                //~vbBwI~
        }                                                          //~vbBwI~
//  	switch(ii)                                                 //~vbBwR~
    	switch(ii-lblcnt)                                          //~vbBwI~
        {
        case 0:		//first opd
...
//      	filegetcid(pfh,2,0);	//reset by chk first line if sline==0//~v0avR~
    ...
    if (lblcnt)                                                    //~vbBwI~
    {                                                              //~vbBwI~
    	if (opid==3||opid==7)	//clear or shift                   //~vbBwI~
        {                                                          //~vbBwI~
        	if (lblcnt!=2)                                         //~vbBwR~
            	return fcmdlabelerr("Need From and To");           //~vbBwI~
	    	if (fcmdgetlabelrange(pfh,lblcnt,labelt,plht,0))       //~vbBwR~
    			return 4;                                          //~vbBwI~
        }                                                          //~vbBwI~
        else                                                       //~vbBwI~
        {                                                          //~vbBwI~
        	uerrmsg("Label is for CLEAR and SHIFT only.",          //~vbBwI~
        			"ラベルは CLEAR と SHIFT でのみ指定できます"); //~vbBwI~
    		return 4;                                              //~vbBwI~
        }                                                          //~vbBwI~
    }                                                              //~vbBwI~
    rc=0;                                                          //~v09rI~
...
//    }                                                            //~vbv6R~
      ...
      {                                                            //~vbv6I~
        ...
        break;
	case 3:	//clear
//  	lineno=filecidclear(Ppcw,pfh,CIDOPTCLEAR);                    //~v06rR~//+vbBwR~
    	lineno=filecidclear(Ppcw,pfh,CIDOPTCLEAR,plht);            //+vbBwI~
...
//    }                                                            //~vbv6R~
      ...
      {                                                            //~vbv6I~
        ...
        break;
	case 7:	//shift                                                   //~v06rI~
//		lineno=filecidclear(Ppcw,pfh,CIDOPTSHIFT);                    //~v06rR~//+vbBwR~
  		lineno=filecidclear(Ppcw,pfh,CIDOPTSHIFT,plht);            //+vbBwI~
