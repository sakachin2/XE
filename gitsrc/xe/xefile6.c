...
//vbzN:240411 (WXE)dbcspace is not displayed by altch even if tabon.//~vbzNI~
//            GSCRSDBCS is ON on wxe(8140 is dbcsspace as fixed.), //~vbzNI~
//            OFF on xe(chk dbcs space code by utfcvu2any1mb and a1a1 is dbcsspace on gb18030).//~vbzNI~
//vbzH:240403 (Bug of vbx4)WINCON:0x01-0x06 cause column shift on windows terminal//~vbzHI~
...
//#ifdef WINCON                                                      //~vbx4I~//~vbzVR~
#ifdef W32                                                         //~vbzVI~
#define UVIOMDEFONLY	//for uviom.h                              //~vbx4I~
#include <uviom.h>                                                 //~vbx4I~
#endif                                                             //~vbx4I~
#ifdef LNX                                                         //~vbzVI~
#define UVIOMDEFONLY	//for uviom.h                              //~vbzVI~
#include <uviom.h>                                                 //~vbzVI~
#endif                                                             //~vbzVI~
                                                                //~5318I~
#include "xe.h"
#include "xescr.h"
...
//  if (UCBITCHK(Ppfh->UFHflag4,UFHF4BIN))  //bin file             //~va00I~//~vbBrR~
    if (UCBITCHK(Ppfh->UFHflag4,UFHF4BIN)  //bin file              //~vbBrI~
    &&  !PFH_ISEBC(Ppfh)//target is not ebc                         //~va6hI~//~vbBrI~
    )                                                              //~vbBrI~
    {                                                              //~va00I~
...
//*************                                                    //~va50I~
	...
	opt|=XEEBCO_CVCTL;  //tab(0x09)-->ebc:0x05                     //~vajmI~
  	opt|=XEEBCO_SETF2LERR;  //set UDBCSCHK_F2LERR at ucvebc3_dd2b  //~vbBqI~
...
//* rc=0:unprintable                                               //~vbx4I~//~vbzHR~
//**************************************************************** //~vbx4I~
int isPrintOnWindows(UCHAR Pch)                                    //~vbx4I~
{                                                                  //~vbx4I~
	int rc;                                                        //~vbx4R~
	if (GunprintableOnTerminal)	//default unprintable by console cp//~vbx4R~
//  	rc=Pch<0x20 && GunprintableOnTerminal[Pch]=='0';           //~vbx4R~//~vbzHR~
    	rc=(Pch<0x20 && GunprintableOnTerminal[Pch]=='1') ? 0 : 1; //~vbzHI~
    else                                                           //~vbx4I~
...
#if defined(LNX) && !defined(XXE)                                  //~vbzVI~
   	 			unpaltch=(UCHAR)GoptcombaltchNP;                   //~vbzVR~
#else                                                              //~vbzVI~
   	 			unpaltch=(UCHAR)Goptcombaltch;                     //~vbzVR~
#endif                                                             //~vbzVI~
              	UTRACEP("%s:combinechar unprintable option unpaltch=0x%02x\n",UTT,unpaltch);//~vbzVI~
	           	*pc++=unpaltch;     //":" by option                //~vbzVR~
               	*pcd++=UDBCSCHK_NPUCS1;	//for csr step dbcs        //~vbzVR~
#ifdef AAA                                                         //~vbzVI~
//	           	*pc=UVIOM_COMBINE_ALTCH2 ; // '.'      //hex code is ucs//~vbzVR~
	           	*pc=unpaltch;                                      //~vbzVR~
#else                                                              //~vbzVI~
  	           	*pc=UVIOM_COMBINE_ALTCH2 ; // '.'      //hex code is ucs//~vbzVI~
#endif                                                             //~vbzVI~
               	*pcd=UDBCSCHK_NPUCS2;//                            //~vbzVR~
               	reslen--;                                          //~vbzVR~
			   	rc=1;   //do setlineattr for set green             //~vbzVR~
            }                                                      //~vbzVI~
            else                                                   //~vbzVI~
...
#else                                                              //~v21iI~
//  if (!UCBITCHK(Gscrstatus,GSCRSDBCS))                        //~5502I~//~vbzNR~
    if (!XE_ISDBCSJ())   //including Ch,ko                         //~vbzNR~
    {                                                              //~va15I~
#ifdef UTF8SUPPH                                                   //~va15I~
        if (!Sdbcsspace[1])	//first time                           //~va15I~
        {                                                          //~va15I~
...
#endif                                                             //~v21iI~
    UTRACEP("%s:dbcsspace 0x%02x,%02x,Gscrstatus=0x%04x,Gotherstatus=0x%04x\n",UTT,dbcsspace1,dbcsspace2,Gscrstatus,Gotherstatus);//~vbzHR~//~vbzNR~//~vbzVR~
