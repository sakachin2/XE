//*CID://+vatfR~:                             update#=  395;       //~vateR~//+vatfR~
//***********************************************************
//* XFC : compare file(win95 missing comp cmd)
//***********************************************************
#define VER "V1.30"   //version                                    //~vasfR~//~vateR~
//***********************************************************
//vatf:241125 xfc1.30 At windows, As Default, assume both 0d0a and 0a as EOL. by /P detect differrent reverse of Linux//+vatfI~
//vate:241125 xfc1.30 text mode read detect 0x1a as EOF. need open not "r" but "rb"//~vateI~
//            Linux read beyond 0x1a and "-P" option to ignore \r before \n; At Win if read by "rb" ,requires \r process//~vateI~
//vatb:241119 xfc 1.29:Bug;unmatch when trailer tab and tab before comment//~vasfI~
...
//*file1                                                           //~va79I~
  ...
  {                                                                //~va79I~
//*file1
//  fh=openfile(Sfnm1,"r",0);	//exit when open faile             //~vateR~
    fh=openfile(Sfnm1,"rb",0);	//exit when open faile             //~vateR~
	Sinp1ctr=getirec(Sfnm1,fh,Stxtbuff,&qh1,Srange1s,Srange1e);
    fclose(fh);
//*file1
//  fh=openfile(Sfnm2,"r",0);	//exit when open faile             //~vateR~
    fh=openfile(Sfnm2,"rb",0);	//exit when open faile             //~vateR~
...
//      UTRACEP("%s:fnm=%s,Pbuff=%s\n",UTT,Pfnm,Pbuff);            //~vasbI~//~vateR~
        if (!Pranges
	    ||  ((ULONG)recno>=Pranges && (ULONG)recno<=Prangee))
        {
        	enqctr++;
...
//        	datacnt=(int)((ULONG)pc-(ULONG)data0)+1;               //~va1yI~//~va66R~
    ...
    {                                                              //~vasbR~
        ...
        {                                                          //~vasbR~
            for (pc=data0+dlen-1;pc>=data0;pc--)                   //~vasbR~
                if (*pc!=' ' && *pc!='\t')                         //~vasbR~
                    break;                                         //~vasbR~
        	datacnt=(int)((ULPTR)pc-(ULPTR)data0)+1;               //~vasbR~
        	Pprh->URHdlennet=datacnt;                              //~vasbR~
        }                                                          //~vasbR~
    }                                                              //~vasbR~
...
//  dlen2=Pprh2->URHdlennet-Smargin1;                              //~v105I~//~vasfR~
    ...
    if (dlen2<0)                                                   //~va9aI~
        dlen2=0;                                                   //~va9aI~
                                                                   //~vasfI~
//  UTRACED("line1",data1,dlen1);                                  //~vasfI~//~vasbR~
//  UTRACED("line2",data2,dlen2);                                  //~vasfI~//~vasbR~
    if (dlen1==dlen2        //length match                         //~v105I~
//  &&  (!dlen1 || !memcmp(data1,data2,(UINT)dlen1)))              //~v105I~//~vaseR~
    &&  (!dlen1 || !xfc_memcmp(data1,data2,(UINT)dlen1)))          //~vaseI~
    {                                                              //~vasbR~
//  	UTRACEP("%s:same except cmt len=%d\n",UTT,dlen1);          //~vasfI~//~vasbR~
...
FILE *openfile(char *Pfile,char *Popt,int Pmsgsw)
{
	FILE *file;
//****************
    UTRACEP("%s:fnm=%s,opt=%s\n",UTT,Pfile,Popt);                  //~vasbI~
...
HELPMSG                                                            //~vasfI~
"                 Tab/Space at EndOfLine are ignored.\n",          //~vasbR~
"                 行末のタブ/スペースは比較されない。\n");         //~vasbR~
HELPMSG                                                            //~va12I~
...
#else                                                              //~vateI~
HELPMSG                                                            //~vateI~
"   %cP         :Detect unmatch of CR(0x0d) and CRLF(0x0d0a)\n",                //~vateI~//+vatfR~
"   %cP         :改行コードCRLF(0x0d0a)とLF(0x0a)の違いを検知する\n",            //~vateI~//+vatfR~
					CMDFLAG_PREFIX);                               //~vateI~
#endif                                                             //~v144I~
                                                                   //~v144I~
HELPMSG                                                            //~va92M~
