...
//********************
#include "xnpsub.h"                                                //~0103R~
//********************
#define LOOPPROT 81      //81
//********************
...
static  int Srprintcase;     //redundancy list count               //~v021I~
...
void xnprchk(int Popt,int *Pdata,int Prlevel,int Pstartpos,PNPWORKT Ppnpwt);
//void xnprprint(void);                                            //~v021R~
void xnprprint(int Plevel);                                        //~v021I~
...
{
    int      rc;
//    PNPWORKT pnpwt;                                              //~v0.1R~
//************
    Stmsgfreq=Ptmsgfreq;                                           //~v0.1I~
    Srepeatmax=Prepeatmax;                                         //~v0.1I~
	Stotloop=0;                                                    //~v0.1I~
	Srlistcnt=0;                                                   //~v0.1I~
//    if (Popt & XNP_REDUNDANTCK)                                  //~v0.1R~
//    {                                                            //~v0.1R~
//        pnpwt=umalloc(sizeof(NPWORKT));     //restore after xnpgasub//~v0.1R~
//        memcpy(pnpwt,Ppnpwt,sizeof(NPWORKT));                    //~v0.1R~
//    }                                                            //~v0.1R~
...
//    memcpy(Ppnpwt,pnpwt,sizeof(NPWORKT));                        //~v0.1R~
//    ufree(pnpwt);                                                //~v0.1R~
//redundancy chk
...
//  if (Smaxrclvl)                                                 //~v021R~
        if (Srlistcnt)                                             //~v0.1I~
//  		uerrmsg("Redundancy List total %d",                    //~v021R~
//  		        "冗長な場所が %d 個あります。",                //~v021R~
//  					Srlistcnt);                                //~v021R~
    		uerrmsg("Max Redundancy Level=%d, level 1 count is %d",//~v021R~
    		        "冗長レベルは最大 %d レベル１は %d 箇所。",    //~v021R~
    					Smaxrclvl,Srlistcnt);                      //~v021I~
		else                                                       //~v0.1I~
			uerrmsg("No Redundancy",                               //~v0.1I~
//  		        "冗長な場所は有りません。",                    //~v0.1I~//+v031R~
    		        "冗長な場所は有りません。");                   //+v031I~
//  					Srlistcnt);                                //~v0.1I~//+v031R~
...
//************
    ...
    {
        xnprprint(Prlevel);        //print redundancy              //~v021R~
    ...
    for (loop=Pstartpos,pi=pi0+Pstartpos;loop<81;loop++,pi++)//no need to start from 0
    {
    	if (!*pi)	//no init data
        	continue;
        iold=*pi;
...
//printf("level=%d,loop=%d,num=%d,row=%d,col=%d\n",Prlevel,loop,iold,row,col);
        ...
        {
            inew=pnpwt->m99[row][col].fnum;   //fixed num
...
//level down
			if (rlevel<Srchkmaxlevel)
	            xnprchk(Popt,pi0,Prlevel+1,loop+1,pnpwt);
            else
            {
//              xnprprint();        //redundancy list              //~v021R~
                xnprprint(rlevel);        //redundancy list        //~v021I~
                xnpprintnum(0,0,0,pnpwt,0,0);   //print initial data
                xnpprintnum(2,row,col,pnpwt,0,0);   //print last data
            }
		}//redundancy
        *pi=iold;		//recov
	}//all idata
...
//  Srlistcnt++;                                                   //~v021R~
    ...
    Srprintcase++;                                                 //~v021I~
    uerrmsg("---Redundancy List  CaseNo %d\n---",                  //~v021R~
			"---冗長性検査結果  CaseNo %d ---",Srprintcase);       //~v021R~
    pi=Srchkwk[0];
    for (ii=0;ii<Smaxrclvl;ii++,pi+=4)
    {
        if (*(pi+2)!=*(pi+3)) //number changed
            printf("#%2d    (%d,%d)(!=%d)  \n",ii+1,*pi+1,*(pi+1)+1,*(pi+2));
        else
            printf("#%2d    (%d,%d)(==%d)  \n",ii+1,*pi+1,*(pi+1)+1,*(pi+2));
    }
#endif                                                             //~v0.1I~
...
//tbl init
	...
	for (ii=0;ii<9;ii++)
        ...
        {
        	pm99->cmsk=ALL_CANDIDATE;		//all number is candidate at init
        	pm99->ccnt=9;					//candidate count
		}
//*set initial data
	for (ii=0,pi=Pdata;ii<9;ii++)
		for (jj=0;jj<9;jj++,pi++)
        	if (*pi)				//initial data
            {
...
//*assign seqno                                                    //~v0.1I~
    ...
    for (ii=0,jj=0,pi=Ppdata,pimask=Ppdatamask;ii<81;ii++,pi++,pimask++)//~v0.1R~
    {                                                              //~v0.1I~
    	num=*pi;                                                   //~v0.1I~
        if (num<0 || num>9)                                        //~v0.1R~
        	return 4;                                              //~v0.1I~
        if (!num)                                                  //~v0.1I~
            continue;                                              //~v0.1I~
...
//*reverse translation                                             //~v0.1I~
    memset(sortwk,0,sizeof(sortwk));                               //~v0.1I~
    for (ii=1,pi=map;ii<=9;ii++,pi++)                              //~v0.1I~
    	sortwk[map[ii]]=ii;                                        //~v0.1I~
//*translation                                                     //~v0.1I~
    for (ii=0,pi=Ppdata;ii<81;ii++,pi++)                           //~v0.1I~
    {                                                              //~v0.1I~
    	num=*pi;                                                   //~v0.1I~
        if (!num)                                                  //~v0.1I~
        	continue;                                              //~v0.1I~
...
//try always from easy method
            ...
            for (num=1;num<=9;num++)    //all number
            {
...
//row
    ...
    for (ii=0;ii<9;ii++,pilc++)
    {
        if (*pilc!=8) 				//not the case only one remained
        	continue;
	    pm99=&Ppnpwt->m99[ii][0];   //matrix of each plase
        for (jj=0;jj<9;jj++,pm99++)
            if (!pm99->fnum)        //not fixed
            {
	            if (xnpfixbymask(ii,jj,pm99->cmsk,Ppnpwt,LEVEL_EASY,"m1R"))
			    	return -1;
            	fixctr++;			//rc
            }
    }//row
//col
    ...
    for (jj=0;jj<9;jj++,pilc++)
    {
        if (*pilc!=8) 				//not the case only one remained
        	continue;
	    pm99=&Ppnpwt->m99[0][jj];   //matrix of each plase
        for (ii=0;ii<9;ii++,pm99+=9)
            if (!pm99->fnum)        //not fixed
            {
	            if (xnpfixbymask(ii,jj,pm99->cmsk,Ppnpwt,LEVEL_EASY,"m1C"))
			    	return -1;
            	fixctr++;			//rc
            }
    }//col
//box
    ...
    for (ibox=0;ibox<9;ibox++,pilc++)  //all box
    {
        if (*pilc!=8) 				//not the case only one remained
        	continue;
        row=(ibox/3)*3;     //box top row
        col=(ibox%3)*3;     //box left col
        pm99=&Ppnpwt->m99[row][col];       //matrix of each plase
        for (ii=row;ii<row+3;ii++,pm99+=6)
            for (jj=col;jj<col+3;jj++,pm99++)
	            if (!pm99->fnum)        //not fixed
                {
                    if (xnpfixbymask(ii,jj,pm99->cmsk,Ppnpwt,LEVEL_EASY,"m1B"))
				    	return -1;
	            	fixctr++;			//rc
                }
    }//all box
...
int xnpmethod1x(PNPWORKT Ppnpwt)
{
    ...
    int   ii,jj,mask;
//************
	pm99=&Ppnpwt->m99[0][0];		//candidate count
	for (ii=0;ii<9;ii++)
		for (jj=0;jj<9;jj++,pm99++)
        	if (!pm99->fnum)		//not fixed
            {
//printf("method1x mask=%04x row=%04x,col=%04x,box=%04x\n",mask,
//       Ppnpwt->fmsk[0][ii],Ppnpwt->fmsk[1][jj],Ppnpwt->fcnt[2][ii/3*3+jj/3]);
                mask=Ppnpwt->fmsk[0][ii]
                    	|Ppnpwt->fmsk[1][jj]
                    	|Ppnpwt->fmsk[2][ii/3*3+jj/3];	//used number mask
...
//************
    ...
    for (ibox=0;ibox<9;ibox++,pilm++)  //all box
    {
        ...
        col=(ibox%3)*3;     //box left col
        pm99=&Ppnpwt->m99[row][col];       //matrix of each plase
        for (cctr=0,ii=row;ii<row+3;ii++,pm99+=6)
            for (jj=col;jj<col+3;jj++,pm99++)
            {
	            if (!pm99->fnum       //not yet fixed
...
//row
    ...
    for (ii=0;ii<9;ii++,pilm++)
    {
	    ...
	    pm99=&Ppnpwt->m99[ii][0];       //matrix of each plase
        sameboxsw=1;
        for (cctr=0,jj=0;jj<9;jj++,pm99++)
        {
        	if (!(pm99->cmsk & mask))  //not candidate
            	continue;
...
//col
    ...
    for (jj=0;jj<9;jj++,pilm++)
    {
        ...
        pm99=&Ppnpwt->m99[0][jj];       //matrix of each plase
        sameboxsw=1;
        for (cctr=0,ii=0;ii<9;ii++,pm99+=9)
        {
            if (!(pm99->cmsk & mask))  //not candidate
            	continue;
            if (cctr)    //not first time candidate
...
//************
    ...
    case 2:			//in box,same row
        pm99=&Ppnpwt->m99[Prow][0];       //matrix of each plase
        box=Pcol/3;
        for (jj=0;jj<9;jj++,pm99++)
        {
            if (!(pm99->cmsk & mask))  //not candidate
    ...
    case 3:			//in box,same col
        pm99=&Ppnpwt->m99[0][Pcol];       //matrix of each plase
        box=Prow/3;
        for (ii=0;ii<9;ii++,pm99+=9)
        {
            if (!(pm99->cmsk & mask))  //not candidate
...
//search pair in the row
    ...
    for (ii=0;ii<9;ii++)
        for (jj=0;jj<9;jj++,pm99++)
        {
//printf("pair srch row=%d,col=%d, ccnt=%d,mask=%04x\n",ii,jj,pm99->ccnt,pm99->cmsk);
            if (pm99->ccnt==2)  //2 candidate
            {
                mask=pm99->cmsk;
                for (kk=jj+1,pm99p=pm99+1;kk<9;kk++,pm99p++)
                {
//printf("pair srch row=%d, col=%d-%d,mask=%04x-%04x\n",ii,jj,kk,mask,pm99p->cmsk);
                    if (pm99p->cmsk==mask)  //pair with 2 candidate
...
//search pair in the col
    for (jj=0;jj<9;jj++)
    {
        pm99=&Ppnpwt->m99[0][jj];       //candidate count
        for (ii=0;ii<9;ii++,pm99+=9)
        {
//printf("pair srch row=%d,col=%d,ccnt=%d, mask=%04x\n",ii,jj,pm99->ccnt,pm99->cmsk);
            if (pm99->ccnt==2)
            {
                mask=pm99->cmsk;
                for (kk=ii+1,pm99p=pm99+9;kk<9;kk++,pm99p+=9)
                {
//printf("pair srch col=%d, row=%d-%d,mask=%04x,%04x\n",jj,ii,kk,mask,pm99p->cmsk);
                    if (pm99p->cmsk==mask)  //pair with same twe candidate
...
//search pair in the box
    for (ibox=0;ibox<9;ibox++)  //all box
    {
        row=(ibox/3)*3;     //box top row
        col=(ibox%3)*3;     //box left col
        pm99=&Ppnpwt->m99[row][col];       //matrix of each plase
        for (ii=row;ii<row+3;ii++,pm99+=6)
            for (jj=col;jj<col+3;jj++,pm99++)
                ...
                {
                	mask=pm99->cmsk;
                    kk=(ii*9+jj+1)/9;                              //~v031I~
                    ll=(ii*9+jj+1)%9;                              //~v031I~
        			pm99p=&Ppnpwt->m99[kk][ll];       //matrix of each plase//~v031I~
//              	for (kk=ii;kk<row+3;kk++,pm99p+=6)             //~v031R~
                	for (;kk<row+3;kk++,pm99p+=6)                  //~v031I~
                    {                                              //~v031I~
//                  	for (ll=jj+1,pm99p=pm99+1;ll<col+3;ll++,pm99p++)//~v031R~
                    	for (;ll<col+3;ll++,pm99p++)               //~v031I~
                    		if (pm99p->cmsk==mask)  //pair with same twe candidate
...
//************
    if (Ppnpwt->m99[Prow1][Pcol1].pair)	//once processed
    	return 0;
    Ppnpwt->m99[Prow1][Pcol1].pair=1;	//once processed id
    Ppnpwt->m99[Prow2][Pcol2].pair=1;	//once processed id
//get 2 candidate
    mask=Pmask;
...
//box
    row=Prow1/3*3;         //top col of box
    col=Pcol1/3*3;        //left col of box
    if (row==Prow2/3*3 && col==Pcol2/3*3)    //in same box?
        for (ii=row;ii<row+3;ii++)
...
int xnpmethod9(PNPWORKT Ppnpwt)
{
    ...
    int   *pic,clist[9],okcnt,oknum,taelvl,level,lasttaelvl;
...
//  	printf("Try and error depth reached to max(%d)\n",MAX_TAELVL);
    ...
    if (Ppnpwt->stat & STAT_MAKE)   //make mode
        if (Ppnpwt->m99[row][col].ccnt>2)   //for performance
            return 0;
//listup candidate
	ccnt=0;
...
//printf("dropcan (%d,%d),mask=%04x,case=%s,level=%04x\n",Prow,Pcol,Pmask,Pcase,Plevel);
	pcmsk=&Ppnpwt->m99[Prow][Pcol].cmsk;
    if (*pcmsk & Pmask)
    {
        if (Stestsw)
        {
        ...
        *pcmsk&=(ALL_CANDIDATE-Pmask);
		Ppnpwt->m99[Prow][Pcol].ccnt--;
		if (!Ppnpwt->m99[Prow][Pcol].ccnt)                         //~v0.1I~
        	return -1;                                             //~v0.1I~
...
//xnpprintcan(Ppnpwt);
    ...
    boxrow=Prow/3;
    boxcol=Pcol/3;
    ibox=boxrow*3+boxcol;
//set fixed info
    Ppnpwt->level |=Plevel;		//dificulty level
    Ppnpwt->m99[Prow][Pcol].fnum=Pnum;				//number fixed
    if (Plevel==LEVEL_INIT)		//initial data setting
    	Ppnpwt->idatano++;
    Ppnpwt->m99[Prow][Pcol].fseq=++Ppnpwt->seqno;     //seqno up
//dup chk before fixed mask on
    ...
    Ppnpwt->m99[Prow][Pcol].cmsk=0;                   //no candidate
    Ppnpwt->m99[Prow][Pcol].ccnt=0;
    Ppnpwt->m99[Prow][Pcol].dlvl=Plevel;   //0:init,
//drop candidate
//row
    pm99=&Ppnpwt->m99[Prow][0];
    for (jj=0;jj<9;jj++,pm99++)
        if (pm99->cmsk & maskon) //now candidate
        {
//printf("row can off num=%d,(%d,%d) by (%d,%d) mask=%04x\n",Pnum,Prow,jj,Prow,Pcol,pm99->cmsk);
            pm99->cmsk &=maskof; //drop candidate
            if (pm99->ccnt)
            {
                if (!--pm99->ccnt)
                    rc=4;           //no candidate place
                if (Stestsw)
                    xnpcmchk(Ppnpwt,pm99->ccnt,pm99->cmsk);
            }
            else
                errsw1=1;
        }
//col
    pm99=&Ppnpwt->m99[0][Pcol];
    for (ii=0;ii<9;ii++,pm99+=9)
        if (pm99->cmsk & maskon) //now candidate
        {
//printf("col can off num=%d,(%d,%d) by (%d,%d) mask=%04x\n",Pnum,ii,Pcol,Prow,Pcol,pm99->cmsk);
            pm99->cmsk &=maskof; //drop candidate
            if (pm99->ccnt)
            {
                if (!--pm99->ccnt)
...
//box
    row=boxrow*3;
    col=boxcol*3;
    pm99=&Ppnpwt->m99[row][col];
    for (ii=row;ii<row+3;ii++,pm99+=6)
        for (jj=col;jj<col+3;jj++,pm99++)
            if (pm99->cmsk & maskon) //now candidate
            {
//printf("box can off num=%d,(%d,%d) by (%d,%d) mask=%04x\n",Pnum,ii,jj,Prow,Pcol,pm99->cmsk);
                pm99->cmsk &=maskof; //drop candidate
                if (pm99->ccnt)
                {
                    if (!--pm99->ccnt)
...
//************
//row
	pm99=&Ppnpwt->m99[0][0];       //matrix of each plase
    for (ii=0;ii<9;ii++)
    {
        for (jj=0,mask=0;jj<9;jj++,pm99++)
        {
        	maskon=Smask[pm99->fnum];
            if (mask & maskon)
            	conflictsw=1;
            else
            	mask|=maskon;
...
//col
    ...
    {
        pm99=&Ppnpwt->m99[0][jj];       //matrix of each plase
        for (ii=0,mask=0;ii<9;ii++,pm99+=9)
        {
        	maskon=Smask[pm99->fnum];
            if (mask & maskon)
            	conflictsw=1;
...
//box
    for (ibox=0;ibox<9;ibox++)  //all box
    {
        ...
        col=(ibox%3)*3;     //box left col
        pm99=&Ppnpwt->m99[row][col];       //matrix of each plase
        for (ii=row,mask=0;ii<row+3;ii++,pm99+=6)
            for (jj=col;jj<col+3;jj++,pm99++)
            {
	        	maskon=Smask[pm99->fnum];
            	if (mask & maskon)
...
//************
    ...
    for (ii=0;ii<9;ii++)
    {
        ...
        for (jj=0;jj<9;jj++,pm99++)
        {
            ...
            if (num)
                if (!Popt)
					if (pm99->dlvl==LEVEL_INIT)
                    	printf("%3d",num);
                    else
						if (pm99->flag==M99F_PATERN)
		                	printf(" +%d",num);
                        else
		                	printf("  _");
                else
    ...
    for (ii=0;ii<9;ii++)
    {
        ...
        for (jj=0;jj<9;jj++,pm99++)
        {
            ...
            else
            {
                ...
                clist[1]=0;             //strz
                for (pc=clist,cctr=0,kk=1;kk<=9;kk++)
                    if (cmsk & Smask[kk])
                    {
                        if (cctr)
                            *pc++=',';
