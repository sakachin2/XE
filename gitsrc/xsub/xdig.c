//*CID://+vat8R~:                             update#=  132;       //~vat8R~
//***********************************************************
//* XDig : apply program into dir/subdir
//***********************************************************
//vat8:241025 xdig:support multiple dirmask                        //~vat8I~
//vat7:241025 xdig:uerrmsg corrupt dbcs msg when redirected by /1xxx//~vat7I~
//vat6:241025 xdig:0c4 is dir parm missing(redirect only),del /pn option which has no meaning.//~vat6R~
//vat5:241023 xdig:add option to apply dirmask once and all        //~vat5R~
//vat4:241023 xdig:2nd:filemask 3rd:dirmask, to skip filemask accept ""//~vat4I~
...
//#define VER "V1.13"  //version                                     //~vae1R~//~van3R~//~vat5R~
#define VER "V1.14"  //version                                     //~vat5I~
#define PGM "XDIG"
...
//* main
//**********************************
	sprintf(Spgmver,"%s:%s:",PGM,VER);                             //~vat5R~
	...
	int opt=UDCWCIO_INTERNAL;	//bypass UDBCSCHK_EXPLICIT and utfucsmapinit(0);//~vat5I~
    udbcschk_wcinit(opt,0);                                        //~va6//~vat5I~
  	uerrmsg_init(Spgmver,stdout,0);//default color                 //~vat5I~
	uerrexit_init(Spgmver,stdout,0,0,0);//no mapfile,no exit,no exit parm//~vat5I~
//**********************************
    Sdwopt=UDIRWALK_NOMSG;              //no msg from udirwalk     //~vat5I~
        parmchk(parmc,parmp);
//*navigate
//  Sdwopt=UDIRWALK_NOMSG;              //no msg from udirwalk     //~vat5R~
    if (Srevsortsw)
    ...
    UTRACEP("%s:Sdwopt=%04x,Sfilesw=%d,Sdirsw=%d\n",UTT,Sdwopt,Sfilesw,Sdirsw);//~vat7R~
...
//**************************
    UTRACEP("%s:Pdir=%s,pfullpath=%s\n",UTT,Pdir,Pfullpath);       //~vat7I~
    if (S1stsw)                                                    //~v124I~
    {                                                              //~van3I~
...
//printf(":dir=%s,fullpath=%s\n",Pdir,Pfullpath);
      if (Ppudirlist)                                              //~vat6I~
        ...
        cmdargv=cmdedit(Pdir,0,Pfullpath);                         //~vat6I~
        if (!Sredironlysw)                                           //~v121I~
        {                                                          //~va2jI~
        	if (Pfullpath)  //fileentry
            	Sfilectr++;
...
//      rc=uspawnvp(P_WAIT,Pargv[0],Pargv);                        //~va99R~
        ...
        else
    {
	    ...
	    UTRACEP("%s:call usystem Pargv[0]=%s\n",UTT,Pargv[0]);     //~vat7I~
        rc=usystem(Pargv[0]);
    }
    flushout();
        *Prc=rc;
...
//**************************                                       //~v126I~
            case 'P':           //msg redirection                  //~v126I~
                ...
                {                                                  //~vat7I~
                    ...
                    case 'F':       //name                             //~v126I~
                                Sfilesw=1;                                     //~v126I~
                                Sdirsw=0;                                      //~v126I~//~vat7R~
                                break;                                         //~v126I~
                    case 'D':       //extention                        //~v126I~
                                Sfilesw=0;                                     //~v126I~//~vat7R~
                                Sdirsw=1;                                      //~v126I~
                                break;                                         //~v126I~
                    case 'B':       //date                             //~v126I~
                                Sfilesw=1;                                     //~v126I~
                                Sdirsw=1;                                      //~v126I~
                                break;                                         //~v126I~
//                  case 'N':       //size                             //~v126I~//~vat6R~
//                              Sfilesw=0;                                     //~v126I~//~vat6R~
//                              Sdirsw=0;                                      //~v126I~//~vat6R~
//                              break;                                         //~v126I~//~vat6R~
                    case 'T':                                      //~vat6I~//~vat7M~
                        Sdwopt|=UDIRWALK_TOPMASK;  //apply dirmask to topdir only//~vat7I~
                        break;                                     //~vat6I~//~vat7M~
                    case 'P':                                      //~vat6I~//~vat7M~
                        Sdwopt|=UDIRWALK_DIRMASK_PATH;  //apply dirmask to all dir//~vat7I~
                        break;                                     //~vat6I~//~vat7M~
                    case '1':                                      //~vat6I~//~vat7R~
                        Sdwopt|=UDIRWALK_DIRMASK_ONLY;  //apply to the dir only(no desendant)//~vat7R~
                        break;                                     //~vat6I~
                    default:                           //not option    //~v126I~
                        optionerr("Apply type",cptr0);                 //~v126I~
                        }                                                  //~v126I~
                }   //for                                          //~vat7I~
                if ((Sdwopt & UDIRWALK_TOPMASK) && (Sdwopt & UDIRWALK_DIRMASK_PATH))//~vat8I~
                	uerrexit("%cPt and %cPp is not consistent.",   //~vat8R~
                				"%cPt と %cPp は矛盾します",       //~vat8I~
        						CMDFLAG_PREFIX,CMDFLAG_PREFIX);    //~vat8I~
                break;                                             //~v126I~
//**************************
//* Word           /X      *                                       //~v125R~
//**************************
...
//chk more
		UTRACEP("%s:filemask=%s,dirmask=%s\n",UTT,Sfilemask,Sdirmask);//~vat4I~
//      if (!cmdparmno)                                            //~vat6R~
        if (!cmdparmno||!*ppcmdstr[0])                             //~vat6I~
    ...
    if (!posparmno)                                                //~v121M~
    {                                                              //~v121I~
//      if (!Sredirectsw)                                          //~v121I~//~vat6R~
//      uerrexit("Sdir-name parameter missing\n",                  //~v121I~
//                      "ディレクトリー名を指定して下さい");               //~v121I~
//          uerrexit("dir-name or redirect parameter is required\n",//~v121I~//~vat6R~
//                              "ディレクトリー名かリダイレクトオプションを指定して下さい");//~v121I~//~vat6R~
            uerrexit("Missing top-directory parameter.\n",         //~vat6I~
                                "ディレクトリー名を指定してください");//~vat6I~
//      uerrmsg("Missing Dir-name parm,Cmd execute only once",     //~v137R~
//               "ディレクトリー名の指定がありませんコマンドは一回のみ実行されます");//~v137R~
//      Sredironlysw=1;                                            //~v121I~//~vat6R~
    }                                                                  //~v121I~
//  else                                                           //~v121I~//~vat6R~
//      if (!Sfilesw && !Sdirsw)                                   //~v121I~//~vat6R~
//              uerrexit("%cPn is invalid when directory specified.\n",//~v174R~//~vat6R~
//                              "ディレクトリーを指定した時は%cPnは無効です\n",//~v174R~//~vat6R~
//  				CMDFLAG_PREFIX);                               //~v174I~//~vat6R~
        if (Sdwopt & (UDIRWALK_DIRMASK_ONLY|UDIRWALK_DIRMASK_PATH|UDIRWALK_TOPMASK))//~vat6I~//~vat7R~
        {                                                          //~vat6I~
        	if (!Sdirmask || !*Sdirmask || !strcmp(Sdirmask,"*") || !strcmp(Sdirmask,"*.*"))//~vat6R~
				uerrexit("Invalid %cP option if missing dir-mask.",//~vat6I~
						"dir-mask 無しでは無効な %cP オプションです",//~vat6R~
						CMDFLAG_PREFIX);                           //~vat6I~
        }                                                          //~vat6I~
//*cmd string create
...
"  file-mask :ファイル選択のワイルドカード指定。\n");//~vat4R~     //~vat8R~
        HELPMSG                                                    //~vat8I~
"             Use \"\" to skip option. Concatinate by \";\" for multiple.\n",//~vat8I~
"             \"\"は指定無しの意味。複数指定は \";\"で連結\n");    //~vat8I~
        HELPMSG
"  dir-mask  :dir name to select. Concatinate by \";\" for multiple.\n",//~vat8R~
"  dir-mask  :ディレクトリー選択のワイルドカード指定。複数指定は \";\"で連結\n");//~vat8R~
        HELPMSG                                                    //~vat8I~
"              ex) \"dirA;subdirB*\".\n",                          //~vat8I~
"              例) \"dirA;subsirB*\"\n");                          //~vat8R~
        HELPMSG                                                    //~vat8I~
"             Concatinate by \";;\" to use top-dir and selection-mask-dir.\n",//~vat8R~
"             トップDirと選択Dirを両方を指定する場合は\";;\"で連結する\n");//~vat8I~
        HELPMSG                                                    //~vat8I~
"              ex) \"top1;top2;;subdir*\".\n",                     //~vat8I~
"              例) \"top1;top2;;subdir*\".\n");                    //~vat8R~
        HELPMSG
...
"    %cP{f|d|b}[{t|p}[1]] :target type.(default is 'f')\n",        //~vat7R~
"    %cP{f|d|b}[{t|p}[1]] :コマンド適用対象(省略値は'f')\n",       //~vat7R~
				CMDFLAG_PREFIX);                                   //~v141I~
        HELPMSG                                                        //~v126I~
//"             f:File, d:Dir, b:Both, n:None(for use redirection)\n",//~v124R~//~vat6R~
//"             f:ファイル, d:ディレクトリー, b:両方, n:リダイレトのみの使用\n");//~v124R~//~vat6R~
//"             f:File, d:Dir, b:Both\n",                            //~vat6I~//~vat7R~
//"             f:ファイル, d:ディレクトリー, b:両方\n");            //~vat6I~//~vat7R~
"             f:File, d:Dir, b:Both\n",                  //~vat7I~ //~vat8R~
"             f:ファイル, d:ディレクトリー, b:両方\n");    //~vat7I~//~vat8R~
        HELPMSG                                                    //~vat7I~
"             {t|p}[1] : pattern to apply dir-mask. default=p.\n",            //~vat7R~//~vat8R~
"             {t|p}[1] : dir-maskの適用レベル。省略値=p。\n");               //~vat7R~//~vat8R~
        HELPMSG                                                    //~vat7M~
"                 t:search top level only. p:search also all descendants.\n",//~vat7R~//~vat8R~
"                 t:トップレベルのみチェック。 p:下位レベルの全Dirもチェック。\n");//~vat7R~//~vat8R~
        HELPMSG                                                    //~vat7I~
"                 1:apply to the matched dir only, not to its desendants.\n",//~vat7R~
"                 1:一致するそのDirのみ適用し、下位レベルには適用しない。\n");//~vat7R~
        HELPMSG                                                    //~vat7I~
"               ex.)  %cpfp, %cpdt1.\n",                           //~vat7R~
"               例)  %cpfp, %cpdt1.\n",                            //~vat7R~
				CMDFLAG_PREFIX,CMDFLAG_PREFIX);                    //~vat7I~
//      HELPMSG                                                        //~v125R~
...
//        Spgm,CMDFLAG_PREFIX,CMDFLAG_PREFIX,CMDFLAG_PREFIX);        //~v141R~//~vat6R~
        HELPMSG                                                    //~van3I~
...
"       %s d1 *.html %cNh %cC$ %c%cxcv $fp$ d2%c$rp$%c$f$ %cs2f ::パラメータ文字\n",//~van4R~
        ...
        HELPMSG                                                    //~vat8I~
"       %s %cpdp . * \"top1d;top2d;;src\" //grep -cv UNMATCH *.c ::get line count.\n",//+vat8R~
"       %s %cpdp . * \"top1d;top2d;;src\" //grep -cv UNMATCH *.c ::行数計算\n",//+vat8R~
        Spgm,CMDFLAG_PREFIX);                                      //~vat8I~
        titlemsg();
        return;
}//help
//**********************************************************************
