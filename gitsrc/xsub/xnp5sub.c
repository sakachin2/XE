...
//********************
#include "xnp5sub.h"                                               //~v0.5R~
//********************
#define LOOPPROT MM_TOTN      //81                                 //~v0.5R~
//********************
...
//static  int Smask[MM_MAXN+1]={0,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x100,//~v0.6R~
//                              0x200,0x400,0x800,0x1000,0x2000,0x4000,0x8000};//~v0.6R~
static  int Smask[MM_MAXN+1]={0,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x100,//~v0.6R~
                              0x200,0x400,0x800,0x1000,0x2000,0x4000,0x8000,//~v0.6R~
                              0x10000, 0x20000, 0x40000, 0x80000,  //~v0.6R~
		    				  0x100000,0x200000,0x400000,0x800000,0x1000000};//~v0.6R~
#define MAX_DEPTH   10
...
//redundancy chk
    ...
    if (Smaxrclvl)
        if (Srlistcnt)                                             //~v0.1I~
			uerrmsg("Redundancy List total %d",                    //~v0.1R~
			        "冗長な場所が %d 個あります。",                //~v0.1R~
						Srlistcnt);                                //~v0.1R~
		else                                                       //~v0.1I~
			uerrmsg("No Redundancy",                               //~v0.1I~
//  		        "冗長な場所は有りません。",                    //~v0.1I~//+v0.6R~
    		        "冗長な場所は有りません。");                   //+v0.6I~
//  					Srlistcnt);                                //~v0.1I~//+v0.6R~
...
//************
    ...
    for (loop=Pstartpos,pi=pi0+Pstartpos;loop<MM_TOTN;loop++,pi++)//no need to start from 0//~v0.5R~
    {
    	if (!*pi)	//no init data
        	continue;
        iold=*pi;
        *pi=0;  //try drop one data
        row=loop/MM_MAXN;                                          //~v0.5R~
        col=loop%MM_MAXN;                                          //~v0.5R~
//printf("level=%d,loop=%d,num=%d,row=%d,col=%d\n",Prlevel,loop,iold,row,col);
        ...
        {
            inew=pnpwt->mxx[row][col].fnum;   //fixed num          //~v0.5R~
...
//tbl init
	...
	for (ii=0;ii<MM_MAXN;ii++)                                     //~v0.5R~
        ...
        {
        	pmxx->cmsk=ALL_CANDIDATE;		//all number is candidate at init//~v0.5R~
        	pmxx->ccnt=MM_MAXN;					//candidate count  //~v0.5R~
		}
//*set initial data
	for (ii=0,pi=Pdata;ii<MM_MAXN;ii++)                            //~v0.5R~
		for (jj=0;jj<MM_MAXN;jj++,pi++)                            //~v0.5R~
        	if (*pi)				//initial data
            {
...
//*assign seqno                                                    //~v0.1I~
    ...
    for (ii=0,jj=0,pi=Ppdata,pimask=Ppdatamask;ii<MM_TOTN;ii++,pi++,pimask++)//~v0.5R~
    {                                                              //~v0.1I~
    	num=*pi;                                                   //~v0.1I~
        if (num<0 || num>MM_MAXN)                                  //~v0.5R~
        	return 4;                                              //~v0.1I~
        if (!num)                                                  //~v0.1I~
            continue;                                              //~v0.1I~
...
//*reverse translation                                             //~v0.1I~
    memset(sortwk,0,sizeof(sortwk));                               //~v0.1I~
    for (ii=1,pi=map;ii<=MM_MAXN;ii++,pi++)                        //~v0.5R~
    	sortwk[map[ii]]=ii;                                        //~v0.1I~
//*translation                                                     //~v0.1I~
    for (ii=0,pi=Ppdata;ii<MM_TOTN;ii++,pi++)                      //~v0.5R~
    {                                                              //~v0.1I~
    	num=*pi;                                                   //~v0.1I~
        if (!num)                                                  //~v0.1I~
        	continue;                                              //~v0.1I~
...
//try always from easy method
            ...
            for (num=1;num<=MM_MAXN;num++)    //all number         //~v0.5R~
            {
            ...
            if (num<=MM_MAXN)                 //fix occured        //~v0.5R~
        ...
        if (Ppnpwt->seqno==MM_TOTN)	//end                          //~v0.5R~
        	break;
...
//row
    ...
    for (ii=0;ii<MM_MAXN;ii++,pilc++)                              //~v0.5R~
    {
        if (*pilc!=MM_LAST) 				//not the case only one remained//~v0.5R~
        	continue;
	    pmxx=&Ppnpwt->mxx[ii][0];   //matrix of each plase         //~v0.5R~
        for (jj=0;jj<MM_MAXN;jj++,pmxx++)                          //~v0.5R~
...
//col
    ...
    for (jj=0;jj<MM_MAXN;jj++,pilc++)                              //~v0.5R~
    {
        if (*pilc!=MM_LAST) 				//not the case only one remained//~v0.5R~
        	continue;
	    pmxx=&Ppnpwt->mxx[0][jj];   //matrix of each plase         //~v0.5R~
        for (ii=0;ii<MM_MAXN;ii++,pmxx+=MM_MAXN)                   //~v0.5R~
            if (!pmxx->fnum)        //not fixed                    //~v0.5R~
            {
...
//box
    ...
    for (ibox=0;ibox<MM_MAXN;ibox++,pilc++)  //all box             //~v0.5R~
    {
        if (*pilc!=MM_LAST) 				//not the case only one remained//~v0.5R~
        	continue;
        row=(ibox/MM_BASE)*MM_BASE;     //box top row              //~v0.5R~
        col=(ibox%MM_BASE)*MM_BASE;     //box left col             //~v0.5R~
        pmxx=&Ppnpwt->mxx[row][col];       //matrix of each plase  //~v0.5R~
        for (ii=row;ii<row+MM_BASE;ii++,pmxx+=MM_NEXT)             //~v0.5R~
            for (jj=col;jj<col+MM_BASE;jj++,pmxx++)                //~v0.5R~
	            if (!pmxx->fnum)        //not fixed                //~v0.5R~
                {
...
//       Ppnpwt->fmsk[0][ii],Ppnpwt->fmsk[1][jj],Ppnpwt->fcnt[2][ii/MM_BASE*MM_BASE+jj/MM_BASE]);//~v0.5R~
                mask=Ppnpwt->fmsk[0][ii]
                    	|Ppnpwt->fmsk[1][jj]
                    	|Ppnpwt->fmsk[2][ii/MM_BASE*MM_BASE+jj/MM_BASE];	//used number mask//~v0.5R~
...
//************
    ...
    for (ibox=0;ibox<MM_MAXN;ibox++,pilm++)  //all box             //~v0.5R~
    {
        ...
        col=(ibox%MM_BASE)*MM_BASE;     //box left col             //~v0.5R~
        pmxx=&Ppnpwt->mxx[row][col];       //matrix of each plase  //~v0.5R~
        for (cctr=0,ii=row;ii<row+MM_BASE;ii++,pmxx+=MM_NEXT)      //~v0.5R~
            for (jj=col;jj<col+MM_BASE;jj++,pmxx++)                //~v0.5R~
            {
...
//row
    ...
    for (ii=0;ii<MM_MAXN;ii++,pilm++)                              //~v0.5R~
    {
        if (*pilm & mask)  //already fixed in this row
        	continue;
	    pmxx=&Ppnpwt->mxx[ii][0];       //matrix of each plase     //~v0.5R~
	    pilm2=&Ppnpwt->fmsk[1][0];      //line(col) fixed mask
        for (cctr=0,jj=0;jj<MM_MAXN;jj++,pmxx++,pilm2++)           //~v0.5R~
        {
            if (pmxx->fnum)        //already fixed                 //~v0.5R~
                continue;
            if (Ppnpwt->fmsk[2][ii/MM_BASE*MM_BASE+jj/MM_BASE] & mask) //num is on box//~v0.5R~
                continue;
...
//col
    ...
    for (jj=0;jj<MM_MAXN;jj++,pilm++)                              //~v0.5R~
    {
        if (*pilm & mask)  //already fixed in this col
        	continue;
        pmxx=&Ppnpwt->mxx[0][jj];       //matrix of each plase     //~v0.5R~
	    pilm2=&Ppnpwt->fmsk[0][0];      //line(row) fixed mask
        for (cctr=0,ii=0;ii<MM_MAXN;ii++,pmxx+=MM_MAXN,pilm2++)    //~v0.5R~
        {
//printf("(%d,%d) num=%d, mask=%04x\n",ii,jj,pmxx->fnum,mask);     //~v0.5R~
            if (pmxx->fnum)        //already fixed                 //~v0.5R~
                continue;
//printf("fmsk2=%04x\n",Ppnpwt->fmsk[2][ii/MM_BASE*MM_BASE+jj/MM_BASE]);//~v0.5R~
            if (Ppnpwt->fmsk[2][ii/MM_BASE*MM_BASE+jj/MM_BASE] & mask) //num is on box//~v0.5R~
                continue;
...
//row
    ...
    for (ii=0;ii<MM_MAXN;ii++,pilm++)                              //~v0.5R~
    {
	    ...
	    pmxx=&Ppnpwt->mxx[ii][0];       //matrix of each plase     //~v0.5R~
        sameboxsw=1;
        for (cctr=0,jj=0;jj<MM_MAXN;jj++,pmxx++)                   //~v0.5R~
        {
        	if (!(pmxx->cmsk & mask))  //not candidate             //~v0.5R~
            	continue;
            if (cctr)    //not first time candidate
                if (fcol/MM_BASE!=jj/MM_BASE)                      //~v0.5R~
                    sameboxsw=0;
            cctr++;
            fcol=jj;
		}//col
...
//col
    ...
    for (jj=0;jj<MM_MAXN;jj++,pilm++)                              //~v0.5R~
    {
        ...
        pmxx=&Ppnpwt->mxx[0][jj];       //matrix of each plase     //~v0.5R~
        sameboxsw=1;
        for (cctr=0,ii=0;ii<MM_MAXN;ii++,pmxx+=MM_MAXN)            //~v0.5R~
        {
            if (!(pmxx->cmsk & mask))  //not candidate             //~v0.5R~
            	continue;
            if (cctr)    //not first time candidate
                if (frow/MM_BASE!=ii/MM_BASE)                      //~v0.5R~
                    sameboxsw=0;
...
//************
    ...
    case 0:			//in row,same box
        row=Prow/MM_BASE*MM_BASE;                                  //~v0.5R~
        col=Pcol/MM_BASE*MM_BASE;                                  //~v0.5R~
        pmxx=&Ppnpwt->mxx[row][col];       //matrix of each plase  //~v0.5R~
        for (ii=row;ii<row+MM_BASE;ii++,pmxx+=MM_NEXT)             //~v0.5R~
            for (jj=col;jj<col+MM_BASE;jj++,pmxx++)                //~v0.5R~
            {
    ...
    case 2:			//in box,same row
        pmxx=&Ppnpwt->mxx[Prow][0];       //matrix of each plase   //~v0.5R~
        box=Pcol/MM_BASE;                                          //~v0.5R~
        for (jj=0;jj<MM_MAXN;jj++,pmxx++)                          //~v0.5R~
        {
            if (!(pmxx->cmsk & mask))  //not candidate             //~v0.5R~
                continue;
            if (jj/MM_BASE==box)                                   //~v0.5R~
                continue;
    ...
    case 3:			//in box,same col
        ...
        box=Prow/MM_BASE;                                          //~v0.5R~
        for (ii=0;ii<MM_MAXN;ii++,pmxx+=MM_MAXN)                   //~v0.5R~
        {
            if (!(pmxx->cmsk & mask))  //not candidate             //~v0.5R~
                continue;
            if (ii/MM_BASE==box)                                   //~v0.5R~
                continue;
...
//printf("pair srch row=%d,col=%d, ccnt=%d,mask=%04x\n",ii,jj,pmxx->ccnt,pmxx->cmsk);//~v0.5R~
            ...
            {
                mask=pmxx->cmsk;                                   //~v0.5R~
                for (kk=jj+1,pmxxp=pmxx+1;kk<MM_MAXN;kk++,pmxxp++) //~v0.5R~
                {
...
//printf("pair srch row=%d,col=%d,ccnt=%d, mask=%04x\n",ii,jj,pmxx->ccnt,pmxx->cmsk);//~v0.5R~
            ...
            {
                mask=pmxx->cmsk;                                   //~v0.5R~
                for (kk=ii+1,pmxxp=pmxx+MM_MAXN;kk<MM_MAXN;kk++,pmxxp+=MM_MAXN)//~v0.5R~
                {
...
//search pair in the box
    for (ibox=0;ibox<MM_MAXN;ibox++)  //all box                    //~v0.5R~
    {
        row=(ibox/MM_BASE)*MM_BASE;     //box top row              //~v0.5R~
        col=(ibox%MM_BASE)*MM_BASE;     //box left col             //~v0.5R~
        pmxx=&Ppnpwt->mxx[row][col];       //matrix of each plase  //~v0.5R~
        for (ii=row;ii<row+MM_BASE;ii++,pmxx+=MM_NEXT)             //~v0.5R~
            for (jj=col;jj<col+MM_BASE;jj++,pmxx++)                //~v0.5R~
                ...
                {
                	mask=pmxx->cmsk;                               //~v0.5R~
                	for (kk=ii;kk<row+MM_BASE;kk++,pmxxp+=MM_NEXT) //~v0.5R~
                    	for (ll=jj+1,pmxxp=pmxx+1;ll<col+MM_BASE;ll++,pmxxp++)//~v0.5R~
                    		if (pmxxp->cmsk==mask)  //pair with same twe candidate//~v0.5R~
...
//************
    if (Ppnpwt->mxx[Prow1][Pcol1].pair)	//once processed           //~v0.5R~
    	return 0;
    Ppnpwt->mxx[Prow1][Pcol1].pair=1;	//once processed id        //~v0.5R~
    Ppnpwt->mxx[Prow2][Pcol2].pair=1;	//once processed id        //~v0.5R~
//get 2 candidate
    mask=Pmask;
    for (num1=1;num1<=MM_MAXN;num1++)                              //~v0.5R~
    	if (!(mask>>=1))
        	break;
    mask=Pmask-Smask[num1];
    for (num2=1;num2<=MM_MAXN;num2++)                              //~v0.5R~
    	if (!(mask>>=1))
        	break;
...
//box
    row=Prow1/MM_BASE*MM_BASE;         //top col of box            //~v0.5R~
    col=Pcol1/MM_BASE*MM_BASE;        //left col of box            //~v0.5R~
    if (row==Prow2/MM_BASE*MM_BASE && col==Pcol2/MM_BASE*MM_BASE)    //in same box?//~v0.5R~
        for (ii=row;ii<row+MM_BASE;ii++)                           //~v0.5R~
            for (jj=col;jj<col+MM_BASE;jj++)                       //~v0.5R~
...
//  	printf("Try and error depth reached to max(%d)\n",MAX_TAELVL);
    ...
    if (Ppnpwt->stat & STAT_MAKE)   //make mode
        if (Ppnpwt->mxx[row][col].ccnt>2)   //for performance      //~v0.5R~
            return 0;
//listup candidate
	ccnt=0;
...
#endif                                                             //~v0.1I~
    pmxx=&Ppnpwt->mxx[0][0];        //candidate count              //~v0.5R~
    for (ii=0,minccnt=10;ii<MM_MAXN;ii++)                          //~v0.5R~
        for (jj=0;jj<MM_MAXN;jj++,pmxx++)                          //~v0.5R~
        {
...
//printf("dropcan (%d,%d),mask=%04x,case=%s,level=%04x\n",Prow,Pcol,Pmask,Pcase,Plevel);
	pcmsk=&Ppnpwt->mxx[Prow][Pcol].cmsk;                           //~v0.5R~
    if (*pcmsk & Pmask)
    {
        if (Stestsw)
        {
        ...
        *pcmsk&=(ALL_CANDIDATE-Pmask);
		Ppnpwt->mxx[Prow][Pcol].ccnt--;                            //~v0.5R~
		if (!Ppnpwt->mxx[Prow][Pcol].ccnt)                         //~v0.5R~
        	return -1;                                             //~v0.1I~
...
//************
	mask=Pmask;
    for (num=1;num<=MM_MAXN;num++)                                 //~v0.5R~
        if (!(mask>>=1))
            break;
...
//xnpprintcan(Ppnpwt);
    ...
    boxrow=Prow/MM_BASE;                                           //~v0.5R~
    boxcol=Pcol/MM_BASE;                                           //~v0.5R~
    ibox=boxrow*MM_BASE+boxcol;                                    //~v0.5R~
//set fixed info
    Ppnpwt->level |=Plevel;		//dificulty level
    Ppnpwt->mxx[Prow][Pcol].fnum=Pnum;				//number fixed //~v0.5R~
    if (Plevel==LEVEL_INIT)		//initial data setting
    	Ppnpwt->idatano++;
    Ppnpwt->mxx[Prow][Pcol].fseq=++Ppnpwt->seqno;     //seqno up   //~v0.5R~
//dup chk before fixed mask on
    ...
    Ppnpwt->mxx[Prow][Pcol].cmsk=0;                   //no candidate//~v0.5R~
    Ppnpwt->mxx[Prow][Pcol].ccnt=0;                                //~v0.5R~
    Ppnpwt->mxx[Prow][Pcol].dlvl=Plevel;   //0:init,               //~v0.5R~
//drop candidate
//row
    pmxx=&Ppnpwt->mxx[Prow][0];                                    //~v0.5R~
    for (jj=0;jj<MM_MAXN;jj++,pmxx++)                              //~v0.5R~
...
//box
    row=boxrow*MM_BASE;                                            //~v0.5R~
    col=boxcol*MM_BASE;                                            //~v0.5R~
    pmxx=&Ppnpwt->mxx[row][col];                                   //~v0.5R~
    for (ii=row;ii<row+MM_BASE;ii++,pmxx+=MM_NEXT)                 //~v0.5R~
        for (jj=col;jj<col+MM_BASE;jj++,pmxx++)                    //~v0.5R~
            if (pmxx->cmsk & maskon) //now candidate               //~v0.5R~
            {
...
void xnpcmchk(PNPWORKT Ppnpwt,int Pcnt,int Pmsk)
{
    int ii,cnt;
//************
    for (ii=1,cnt=0;ii<=MM_MAXN;ii++)                              //~v0.5R~
    	if (Pmsk & Smask[ii])
        	cnt++;
	if (cnt!=Pcnt)
    {
...
//row
    ...
    for (ii=0;ii<MM_MAXN;ii++)                                     //~v0.5R~
    {
        for (jj=0,mask=0;jj<MM_MAXN;jj++,pmxx++)                   //~v0.5R~
        {
...
//col
    ...
    {
        pmxx=&Ppnpwt->mxx[0][jj];       //matrix of each plase     //~v0.5R~
        for (ii=0,mask=0;ii<MM_MAXN;ii++,pmxx+=MM_MAXN)            //~v0.5R~
        {
        	maskon=Smask[pmxx->fnum];                              //~v0.5R~
            if (mask & maskon)
            	conflictsw=1;
...
//box
    for (ibox=0;ibox<MM_MAXN;ibox++)  //all box                    //~v0.5R~
    {
        ...
        col=(ibox%MM_BASE)*MM_BASE;     //box left col             //~v0.5R~
        pmxx=&Ppnpwt->mxx[row][col];       //matrix of each plase  //~v0.5R~
        for (ii=row,mask=0;ii<row+MM_BASE;ii++,pmxx+=MM_NEXT)      //~v0.5R~
            for (jj=col;jj<col+MM_BASE;jj++,pmxx++)                //~v0.5R~
            {
	        	maskon=Smask[pmxx->fnum];                          //~v0.5R~
            	if (mask & maskon)
    ...
    for (ii=0;ii<MM_MAXN;ii++)                                     //~v0.5R~
    {
        ...
        for (jj=0;jj<MM_MAXN;jj++,pmxx++)                          //~v0.5R~
        {
            ...
            if (num)
                ...
                printf("   _");                                    //~v0.5R~
            if (jj%MM_BASE==MM_BASE-1)                             //~v0.5R~
                printf("  ");
        }//a line
        if (ii%MM_BASE==MM_BASE-1)                                 //~v0.5R~
            printf("\n\n");
        else
            printf("\n");
    }//all line
    ...
    for (ii=0;ii<MM_MAXN;ii++)                                     //~v0.5R~
    {
        ...
        for (jj=0;jj<MM_MAXN;jj++,pmxx++)                          //~v0.5R~
        {
            ...
            else
            {
                ...
                clist[1]=0;             //strz
                for (pc=clist,cctr=0,kk=1;kk<=MM_MAXN;kk++)        //~v0.5R~
                    if (cmsk & Smask[kk])
                    {
                        if (cctr)
                            *pc++=',';
