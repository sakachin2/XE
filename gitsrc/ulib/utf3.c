...
//v7a0:240221 W32:console; adjusting ambiguous required for ex œ..hide next char if width=1)//~v7a0I~
...
#include <uvio.h>                                                  //~vbzzR~
#include <uvio2.h>                                                 //~vbzzR~
...
#endif                                                             //~v79nI~
//int chkAmbiguous(int Prc,ULONG Pucs);                              //~vbzzR~//~v7apR~
int chkAmbiguous(int *Pprc,ULONG Pucs);                            //~v7apI~
//**************************************************               //~7712M~
...
//*for ucs2(<=u-ffff) mapinit specify UTF8UCS2, so apichk done.    //~v7apI~
//*Other call get the result from the mapinit tbl even without UTF8UCS2 option.//~v7apI~
    {                                                              //~v640I~
#ifdef LNX                                                         //~v640I~
...
//  if (rc>0)                                                      //~v62UR~//~v650R~
    ...
    {                                                              //~v62UR~
//        if (utf4_isFormat(0,(UWUCS)Pucs))                          //~v6WqI~//~v7apR~
//        {                                                          //~vbzzI~//~v7apR~
////            rc&=~UTFWWF_RC_AMBIG_UNP; //0x04000000                     //~v6WqI~//~vbzzR~//~v7apR~
//            rc&=~UTFWWO_MK_AMBIGUOUS; //0x2000  //apply /G option by bzzR~//~v7apR~
//            rc|=UTFWWF_RC_FORMAT; //0x04000000                     //~vbzzI~//~v7apR~
//        }                                                          //~vbzzI~//~v7apR~
//		chkAmbiguous(rc,Pucs);                                     //~vbzzR~//~v7apR~
  		chkAmbiguous(&rc,Pucs);	//chk also FORMAT                  //~v7apI~
...
UTRACEP("%s: return len=%d,flag=0x%04x,ucs=0x%04x( %s )\n",UTT,len,flag,Pucs,//~v79gR~
		UCV_U2F1(Pucs));                          //~v79gI~//~v79ER~//~v7a0R~
    return len;                                                    //~v62UR~
}//utfwcwidth                                                      //~v6VbR~
...
//****************                                                 //~v6V4I~
	opt=Popt;                                                      //~v6V4I~
//    if (Popt & UTFWWO_UTF8UCS2) //always apichk for ambiguous                           //~v6V4I~//~vbzzR~
//    {                                                              //~v6V4I~//~vbzzR~
...
UTRACEP("%s:wcwidthsubNoMap rc=0x%04x for ucs=%x\n",UTT,rc,Pucs);             //~v6V4I~//~vbzjR~//~vbzzR~
    if (rc>=0 && Pucs>=0x80)   //chk unprintable also for combining char(rc=0)//~v6V4I~
    {                                                              //~v6V4I~
//  	chkAmbiguous(rc,Pucs);                                     //~vbzzR~//~v7apR~
    	chkAmbiguous(&rc,Pucs);                                    //~v7apI~
...
//*******************************************************          //~v6V4I~
//*LNX                                                             //~v7apR~
//*return char width for Pucs>MAP_ENTRY                            //~v7apI~
//*******************************************************          //~v6V4I~
int utfwcwidthNoMap(int Popt,ULONG Pucs,int *Ppflag)               //~v6V4I~
{                                                                  //~v6V4I~
	int flag,rc,len;                                               //~v6V4I~
...
//****************                                                 //~v6V4I~
	opt=Popt;                                                      //~v6V4I~
//    if (Popt & UTFWWO_UTF8UCS2)  always apichk for /G option                                  //~v6V4I~//~v7apR~
//    {                                                              //~v6V4I~//~v7apR~
    #ifdef ARM                                                     //~v6V4I~
...
UTRACEP("%s:wcwidthsubNoMap rc=0x%04x for ucs=%x\n",UTT,rc,Pucs);      //~v6V4R~//~vbzzR~
//    if (rc & UTFWWWF_RC_AMBIG_UNP)                                 //~vbzzI~//~vbzzR~
//    {                                                              //~vbzzI~//~vbzzR~
//        UTRACEP("%s:wcwidthsubNoMap unprintable by /G0 option for ucs=%x\n",UTT,Pucs);//~vbzzI~//~vbzzR~
//        len=-1;                                                    //~vbzzI~//~vbzzR~
//        flag=0;                                                    //~vbzzI~//~vbzzR~
//    }                                                              //~vbzzI~//~vbzzR~
//    else                                                           //~vbzzI~//~vbzzR~
...
//*          3  mk_width                                 combining,dbcs chk logic//~v6V2R~
    ...
    {                                                              //~7920I~
      ...
      {                                                            //~v640I~
       ...
       {                                                           //~v79AI~
//           			UTF_ADJUSTMODE_ON();                       //~v7a0R~
    	datatype=utftbsrch_j(Pucs);//chk before old sjis tbl(for new codepoint//~7925I~
UTRACEP("utfwcwidth utftbsrch_j wc=0x%04x,datatype=0x%x\n",Pucs,datatype);     //~v650R~//~v79uR~//~v7a0R~
//  			UTF_ADJUSTMODE_OFF();                              //~v7a0R~
	    if (datatype)                                              //~7925I~
...
//  UTRACEP("%s:ucs=0x%04x,rc=%x\n",UTT,Pucs,datatype);              //~v6VnR~//~vbzjR~//~v7b2R~
    return datatype;                                               //~v6V4I~
...
#endif  //W32                                                      //~v6V4I~
//*******************************************************          //~vbzzI~//~vbzzR~
//*set GambigWidth by /G option                                    //~vbzzR~
//*GambigWidth=0 if adjust required                                //~vbzzR~
//*******************************************************          //~vbzzR~
//int chkAmbiguous(int Prc,ULONG Pucs)     //~vbzzR~                 //~vbzzR~//~v7apR~
int chkAmbiguous(int *Pprc,ULONG Pucs)                             //~v7apI~
{                                                                  //~vbzzI~//~vbzzR~
...
//*RC_AMBINGLANG is ON when ambig by lang is 0/1/2(No adjust:GULIBUTFMODE_AMBIGLANG off); the following chk is just to be sure.//~v7apR~
    ...
    {                                                              //~vbzzI~//~vbzzR~
        ...
        {                                                          //~vbzzI~//~vbzzR~
            width=1;                                               //~vbzzI~//~vbzzR~
            UTRACEP("%s:by /GN1 change to sbcs ucs=0x%04x(%s)\n",UTT,Pucs,UCV_U2F1(Pucs));//~vbzzI~//~vbzzR~
        }                                                          //~vbzzI~//~vbzzR~
        else                                                       //~vbzzI~//~vbzzR~
        if (Guvio2Stat & UVIO2S_AMBIG_UNP)   //for all ambiguous, default off(Wide)//~vbzzI~//~vbzzR~
        {                                                          //~vbzzI~//~vbzzR~
            width=-1;                                              //~vbzzI~//~vbzzR~
            UTRACEP("%s:by /GN0 change to unprintable ucs=0x%04x(%s)\n",UTT,Pucs,UCV_U2F1(Pucs));//~vbzzI~//~vbzzR~
        }                                                          //~vbzzI~//~vbzzR~
        else                                                       //~vbzzR~
        {                                                          //~vbzzR~
            width=2;                                               //~vbzzR~
	        rc=1;                                                  //~vbzzR~
            UTRACEP("%s:by /GN2 change to dbcs ucs=0x%04x(%s)\n",UTT,Pucs,UCV_U2F1(Pucs));//~vbzzR~
        }                                                          //~vbzzR~
//#ifdef W32                                                       //~v7b2R~
//        if (Pucs==0x00ad)                                        //~v7b2R~
//        {                                                        //~v7b2R~
//            UTRACEP("%s:ad hoc set GambigWidth=1 forucs=0x%04x(%s)\n",UTT,Pucs,UCV_U2F1(Pucs));//~v7b2R~
//            width=1;                                             //~v7b2R~
//            rc=1;                                                //~v7b2R~
//        }                                                        //~v7b2R~
//#endif                                                           //~v7b2R~
    }                                                              //~vbzzI~//~vbzzR~
    GambigWidth=width;                                             //~vbzzR~
//#ifdef UUU  //no special for FORMAT                              //~v7apR~
//#ifdef W32                                                       //~v7b2R~
//    if (Pucs!=0x00ad)                                            //~v7b2R~
//    {                                                            //~v7b2R~
//#endif                                                           //~v7b2R~
