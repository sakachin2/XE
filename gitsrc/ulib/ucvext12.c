//*CID://+v7dTR~:                                   update#=  757; //+v7dTR~
//***********************************************************************
//v7dT:241120 (Bug)ANK option interfared by SUBCHAR_0a option chk  //+v7dTI~
//v7dM:241106 ebcmb; use not iconv but wcrstombs for ecs2local;==> but it cv to utf8. set some subchar.//~v7dMI~
//v7dE:241030 (BUG)coding miss                                     //~v7dEI~
...
#include <ucvext.h>                                                //~v6BjI~
#include <ucvebc.h>
#include <ucvebc2.h>
//#include <ucvext.h>                                              //~v6BjR~
...
//********************                                             //~v69dI~
    ...
    UTRACEP("%s:Spfunctoucs=%p,Spfuncfromucs=%p\n",UTT,Spfunctoucs,Spfuncfromucs);//~v6BjR~
    UTRACED("ucvext2 cfg",Ppcfg,UCVEXTCFGSZ);                      //~v6BjI~
	if (Ppcfg->UCECsubcharsbcs)                                    //~v69dI~
		Ssubcharsbcs=Ppcfg->UCECsubcharsbcs;                       //~v69dI~
	if (Ppcfg->UCECsubchardbcs)                                    //~v69dI~
//		Ssubcharsbcs=Ppcfg->UCECsubchardbcs;                       //~v69dI~//~v7dER~
  		Ssubchardbcs=Ppcfg->UCECsubchardbcs;                       //~v7dEI~
...
//  Ssubcharebcdbcslen=strlen(Ssubcharebcdbcs);                    //~v69dR~//~v6BkR~
    ...
    UTRACEP("%s:subchar dbcs=%x,sbcs=%x\n",UTT,Ssubchardbcs,Ssubcharsbcs);//~v69dI~//~v6BjR~
    UTRACEP("%s:subcharebc dbcs=%02x%02x,sbcs=%02x\n",UTT,*Ssubcharebcdbcs,*(Ssubcharebcdbcs+1),Ssubcharebcsbcs);//~v69dR~//~v6BjR~
    UTRACEP("%s:subcharlocal dbcs=%02x%02x,sbcs=%02x\n",UTT,*Ssubcharlocaldbcs,*(Ssubcharlocaldbcs+1),Ssubcharlocalsbcs);//~v69dR~//~v6BjR~
    return 0;                                                      //~v69dI~
}//ucvext12_init                                                   //~v69dI~
//*************************************************************************
...
//***************************
UTRACEP("%s:opt=%x,converter=%p,inplen=%d,Spfunctoucs=%p\n",UTT,Popt,Pconverter,Pinplen,Spfunctoucs);//~v5mYR~//~v6BjR~
	if (!Pconverter)                                               //~v681I~
    	return 4;                                                  //~v681I~
//#ifdef W32                                                       //~v5n2R~
  if (IS_ICUMODE())                                                //~v5n1R~
...
//      len=(int)(*api)(converter,wkucs,2,wkebc,4,&err);           //~v6BkI~//~v6BsR~
        ...
        dbcssw=1;                                                  //~v66DI~
    	UTRACEP("%s:dbcs len=%d,U_FAILURE=%d,ebc=%02x%02x,out=%04x\n",UTT,len,U_FAILURE(err),*Ppebc,*(Ppebc+1),wkucs[0]);//~v7dEI~
    }
    else
    {
        wkebc[1]=*Ppebc;
...
//      len=(int)(*api)(converter,wkucs,2,wkebc+1,1,&err);         //~v6BkI~//~v6BsR~
    	...
    	UTRACEP("%s:sbcs len=%d,U_FAILURE=%d,ebc=%02x,out=%04x\n",UTT,len,U_FAILURE(err),*Ppebc,wkucs[0]);//~v7dEI~
...
//  if (len!=1 || U_FAILURE(err))                                  //~v6fnI~//~v6BjR~
	...
	{
		UTRACEP("%s:ERR ebc2ucs1 err=%08x=%s\n",UTT,(UINT)err,ucvext_icuuerrname(err));//~v6BjR~
		rc=4;
	}
    else
    {                                                              //~v66DI~
...
//#else //W32                                                      //~v5n2R~
  ...
  {                                                                //~v7dEI~
	...
	UTRACEP("%s:No ICU mode rc=%d,ucs=%04x\n",UTT,rc,*Ppucs);      //~v7dEI~
  }                                                                //~v7dEI~
//#endif                                                           //~v5n2R~
    UTRACEP("%s:rc=%d,in=%02x%02x,out=%04x\n",UTT,rc,*Ppebc,(Pinplen>1?*(Ppebc+1):0),*Ppucs);//~v5mTR~//~v6BjR~//~v7dER~
    return rc;
}//ucvext_icuebc2ucs1
//*************************************************************************
//*translate by icu converter
...
//#endif                                                           //~v5n2R~
    ...
    int opt;                                                       //~v66nI~
//***************************
UTRACEP("%s:opt=0x%08x,converter=%x len=%d\n",UTT,Popt,Pconverter,Pinplen);//~v6BjR~
...
//  ||  ucsctr!=1		//sbcs subch 1a and next sbcs              //~v6fnI~//~v6BjR~
    ...
    )                                                              //~v6fnI~
	{
		...
		rc=4;
		UTRACEP("%s:ERR local2ucs1 chklen=%d,err=%08x=%s\n",UTT,chklen,(UINT)err,ucvext_icuuerrname(err));//~v6fnI~//~v6BjR~
	}
    else
    {
...
//      	rc=ucvext_icusubcharchk(UCVEXTISCO_L2U,*Ppucs,Ppmbs,inplen);//~v66DI~//~v6hhR~
  ...
  if (Ppchklen)                                                    //~v66DI~
    *Ppchklen=chklen;
    UTRACEP("%s:rc=%x,inplen=%d,in=%02x%02x,chklen=%d,outucs=%04x\n",UTT,rc,inplen,*Ppmbs,*(Ppmbs+1),chklen,wkucs[0]);//~v5mTM~//~v6BjR~
    UTRACED("wkucs",wkucs,sizeof(wkucs));               //~v5mTM~  //~v6BjR~
  }                                                                //~v5n2I~
//#else    //W32                                                   //~v5n2R~
  else                                                             //~v5n2I~
...
//#endif                                                           //~v5n2R~
    ...
    int utf16ctr;                                                  //~v6BjI~
//***************************
UTRACEP("%s:converter=%p,opt=%x\n",UTT,Pconverter,Popt);      //~v681R~//~v6BjR~
...
//  len=(*api)(converter,wkebc,sizeof(wkebc),wkucs,utf16ctr,&err); //~v6BjI~//~v6BkR~
    ...
    UTRACEP("%s:api=%p,rc=%x,in=%04x,outlen=%d,out=%02x%02x%02x%02x\n",UTT,api,rc,Pucs,len,wkebc[0],wkebc[1],wkebc[2],wkebc[3]);//~v66rI~//~v6BjR~//~v7dMR~
	UTRACEP("%s:ucs=%04x,UErrorCode=%d(name=%s)\n",UTT,wkucs[0],(UINT)err,ucvext_icuuerrname(err));//~v7dPI~//~v7dMR~
//  if (U_FAILURE(err))                                            //~v66JR~
    if (!len || U_FAILURE(err))                                    //~v66JI~
	{
		UTRACEP("%s:ERR ucs2ebc1 err=%08x=%s\n",UTT,(UINT)err,ucvext_icuuerrname(err));//~v6BjR~
		rc=4;
	}
    else                                                           //~v66DI~
//    if (!U_FAILURE(err) && err==U_INVALID_CHAR_FOUND)    //10      //~v7dPI~//~v7dMR~
//    {                                                              //~v7dPI~//~v7dMR~
//        UTRACEP("%s:ucs=%04x,UErrorCode=U_INVALID_CHAR_FOUND=%d(name=%s)\n",UTT,wkucs,(UINT)err,ucvext_icuuerrname(err));//~v7dPR~//~v7dMR~
//        if (Popt & EBC2ASC_EXT_SETSUBCHRC)                         //~v7dPI~//~v7dMR~
//            swsubch=UCVEXTRC_SUBCH;                                //~v7dPI~//~v7dMR~
//    }                                                              //~v7dPI~//~v7dMR~
//    else                                                           //~v7dPI~//~v7dMR~
    {                                                              //~v66DI~
...
//                	rc=4;                                          //~v66AR~
    ...
    if (swsubch)                                                   //~v66DI~
    	rc=swsubch;                                                //~v66DI~
    UTRACEP("%s:rc=%x,in=%04x,outlen=%d,out=%02x%02x%02x%02x\n",UTT,rc,Pucs,len,wkebc[0],wkebc[1],wkebc[2],wkebc[3]);//~v6BjR~
    return rc;
}//ucvext_icuucs2ebc1
//*************************************************************************
//*translate by icu converter
...
//#endif                                                           //~v5n2R~
    ...
    int rc=0,len=0;                                                //~v7dMR~
    int opt;                                                       //~v66kR~
    int swsubch=0,rc2;                                             //~v66DR~
    int ucsctr;                                                    //~v6BjR~
//***************************
UTRACEP("%s:Popt=x%x,converter=%p\n",UTT,Popt,Pconverter);         //~v6hhR~//~v6BjR~
//#ifdef LNX                                                       //~v7dMR~
//    if (1)                          //not iconv but wcs2mbs      //~v7dMR~
//#else                                                            //~v7dMR~
    if (!(Popt & EBC2ASC_LOCALCV))	//local converter
//#endif                                                           //~v7dMR~
    {                                                              //~v66FI~
        UTRACEP("%s:call udbcschk_ucs2mb1\n",UTT);                 //~v7dMI~
...
//            opt|=UDCU2M1O_BACKCHK; //trans back chk,ret 3 if rev chk err//~v66FI~//~v66KR~
    ...
    memset(wkmbs,0,sizeof(wkmbs));                                 //~v7dEI~
//#ifdef W32                                                       //~v5n2R~
//if (IS_ICUMODE())                                                //~v5n1R~//~v66KR~
...
//  len=(*api)(converter,wkmbs,sizeof(wkmbs),wkucs,ucsctr,&err);   //~v6BjI~//~v6BkR~
	...
	UTRACEP("%s:icuconverter=%p,len=%d,ucs=%x\n",UTT,converter,len,Pucs);//~v6hhI~//~v6BjR~
//  if (!U_FAILURE(err))                                           //~v66JR~
	if (!len || U_FAILURE(err))                                    //~v66JI~
	{
		UTRACEP("%s:ERR=%08x=%s\n",UTT,(UINT)err,ucvext_icuuerrname(err));//~v6BjR~
...
//      	memcpy(Ppmbs,wkmbs,len);                               //~v6BkR~
    ...
    if (swsubch)                                                   //~v66DI~
    	rc=swsubch;                                                //~v66DI~
    UTRACEP("%s:rc=%x,in=%x,outlen=%d,out=%02x%02x%02x%02x\n",UTT,rc,Pucs,len,wkmbs[0],wkmbs[1],wkmbs[2],wkmbs[3]);//~v6hhR~//~v6BjR~
    return rc;
...
//***************************                                      //~v660I~
    ...
    UTRACEP("%s:rc=%x,cv=%lx,in=%04x,outlen=%d,out=%02x\n",UTT,rc,mapsbcs,Pucs,*Ppoutlen,*Ppmbs);//~v660R~//~v6b9R~//~v6BjR~
    return rc;                                                     //~v660I~
...
//***************************
    ...
    UTRACEP("%s:opt=%x ebc1=%02x,inplen=%d,Perrrep=%x,map=%p,mapc=%p\n",UTT,Popt,*Ppebc,Pinplen,Perrrep,psbcsmap,psbcsmapc);//~v6f7R~//~v6BjR~
    ...
    &&   (ch=psbcsmapc[*Ppebc]))  //source is sbcs
    {
        outlen=1;
        *Pmbs=ch;
UTRACEP("%s:by SBCSMAPC/SBCSMAPA2A %02x->%02x\n",UTT,*Ppebc,*Pmbs);//~v6BjR~
    }
    else
    ...
    )
    {
        outlen=1;
        *Pmbs=*(psbcsmap+*Ppebc);
UTRACEP("%s:@@@@map SBCSMAP/SBCSMAPA2A %02x->%02x,map=%p\n",UTT,*Ppebc,*Pmbs,psbcsmap);//~v6BjR~
    }
    else
    {
...
//      	rc2=ucvext_icuucs2local1(Popt,Psbcsmap,ucs[0],Pmbs,&outlen);//~v66DI~//~v6BjR~
        ...
        if (!rc)
        {
//            if (*Pmbs==0x0a && outlen==1)                        //+v7dTR~
//            {                                                    //+v7dTR~
//                if (Sextflag & SEXTF_REPLF)                      //+v7dTR~
//                    rc=4;       //output subchar                 //+v7dTR~
//            }                                                    //+v7dTR~
            if (*Pmbs==0x0a && outlen==1                           //+v7dTI~
            &&  (Sextflag & SEXTF_REPLF))                          //+v7dTI~
            {                                                      //+v7dTI~
            	rc=4;       //output subchar                       //+v7dTI~
                UTRACEP("%s:rc=4 by SEXTF_REPLF(SUBCHAR_0a option) *Ppebc=%02x,*Pmbs=%02x\n",UTT,*Ppebc,*Pmbs);//+v7dTI~
            }                                                      //+v7dTI~
            else
            ...
            {                                                      //+v7dTI~
                ...
                UTRACEP("%s:rc=4 by ANK option) *Ppebc=%02x\n",UTT,*Ppebc);//+v7dTI~
            }                                                      //+v7dTI~
            if (swsubch)                                           //~v66DI~
            {                                                      //~v69cI~
...
//                  	memcpy(Pmbs,Ssubcharlocaldbcs,outlen);     //~v66TR~//~v6BkR~
                ...
                UTRACEP("%s:ERRREP DBCS ebc2mb ebc=%02x%02x, mb=%02x%02x\n",UTT,*Ppebc,*(Ppebc+1),*Pmbs,*(Pmbs+1));//~v6BjR~
            }
            else    //sbcs
            {
...
//                    }                                            //~v66CR~
                ...
                UTRACEP("%s:ERRREP SBCS ebc2mb ebc=%02x, mb=%02x\n",UTT,*Ppebc,*Pmbs);//~v6BjR~
            }//sbcs
	...
	*Ppoutlen=outlen;
    ...
    UTRACEP("%s:rc=%x,in=%02x%02x,outlen=%d,out=%02x%02x\n",UTT,rc,*Ppebc,(Pinplen>1?*(Ppebc+1):0),outlen,Pmbs[0],Pmbs[1]);//~v5n1I~//~v6BjR~
    UTRACED("Pmbs",Pmbs,outlen);                            //~v66WI~//~v6BjR~
    return rc;
}//ucvext_icuebc2mb1
//*************************************************************************
//*translate by icu converter
...
//***************************
    psbcsmap=(UCHAR*)Psbcsmap;
    psbcsmapc=(UCHAR*)Psbcsmapc;
    UTRACEP("%s:ucvext_icumb2ebc1 opt=0x%08x,converetr=%p,inplen=%d,errrep=%x,sbcsmap=%p,sbcsmapc=%p\n",UTT,Popt,Pconverter,Pinplen,Perrrep,psbcsmap,psbcsmapc);//~v6bjR~//~v6BjR~
    ...
    &&   (ch=psbcsmapc[*Ppmbs]))  //source is sbcs
    {
        outlen=chklen=1;
        *Ppebc=ch;
UTRACEP("%s:by SBCSMAPC/A2A %02x->%02x\n",UTT,*Ppmbs,*Ppebc);      //~v6BjR~
    }
    else
...
UTRACEP("%s:mb2ebc1 by SBCSMAP/ABCSMAPA2A %02x->%02x,sbcsmap=%p\n",UTT,*Ppmbs,*Ppebc,psbcsmap);//~v6BjR~
    }
    else
    {
...
//                      outlen=Ssubcharebcdbcslen;                 //~v66CR~//~v69cR~
                ...
                UTRACEP("%s:ERRREP DBCS mb2ebc ebc=%02x%02x, mb=%02x%02x\n",UTT,*Ppebc,*(Ppebc+1),*Ppmbs,*(Ppmbs+1));//~v6BjR~
            }
            else    //sbcs
            {
        		if (errrep=(Perrrep&0xff),errrep)	//by locale code
...
//                      *Ppebc=Ssubcharebcsbcs;                    //~v66CR~//~v69cR~
                ...
                UTRACEP("%s:ERRREP SBCS mb2ebc ebc=%02x, mb=%02x\n",UTT,*Ppebc,*Ppmbs);//~v6BjR~
            }//sbcs
            rc=EBC2ASC_RC_ERRREP;// 0x01
        }//rc
    }//not by map
  ...
  {                                                                //~v66DI~
    ...
    UTRACEP("%s:rc=%x,chklen=%d,outlen=%d\n",UTT,rc,chklen,outlen);//~v6BjR~
...
UTRACEP("%s:opt=%08x,socontsw=%d,Shandle=%p,Pconverters=%p\n",UTT,Popt,socontsw,Shandle,Pconverters);//~v6BjR~//~v7dER~
  if (Shandle)                                                     //~v69cR~
...
UTRACEP("%s:input ebc=%p, inplen=%d, errrep=%x\n",UTT,Ppebc,Pinplen,Perrrep);//~v66KR~//~v6BjR~
  if (socontsw)                                                    //~v66NI~
...
UTRACEP("%s:ebc2mbs dbcssw=%d,reslen=%d\n",UTT,dbcssw,reslen);     //~v6BjR~
        if (reslen==1)
        {
...
//  outlen=(int)((ULONG)pco-(ULONG)Pmbs);                          //~v6hhR~
    ...
    if (Ppoutlen)
        *Ppoutlen=outlen;
UTRACEP("%s:rc=%d,ebc1=0x%02x,mb1=0x%02x\n",UTT,rc,*Ppebc,*Pmbs);  //~v6BjR~
...
UTRACEP("%s:len=%d, inp=%02x,lrecl=%d,lrecl0=%d,errrep=%x\n",UTT,Pinplen,*Pinp,lrecl,lrecl0,Prepchar);//~v5mWR~//~v6BjR~
    ...
    {
    	mbdbcs=(int)*pi;
    UTRACEP("%s:pi=%pmpie=%p,dbcssw=%d,dbcs=%x%x\n",UTT,pi,pie,dbcssw,mbdbcs,*(pi+1));//~v5mWR~//~v6BjR~
...
//          else                                                   //~v5mWI~
//          	explen++;                                          //~v5mWR~
    UTRACEP("%s:setso outlen=%d\n",UTT,outlen);               //~v5mWI~//~v6BjR~
        }//1st dbcs
...
//      	rlen=lrecl-((ULPTR)po-(ULPTR)pos);                     //~v6hhI~//~v6BkR~
    ...
    UTRACEP("%s:pos=%p,poo=%p:%02x,lrecl=%d,rlen=%d\n",UTT,pos,poo,*poo,lrecl,rlen);//~v5mWR~//~v6BjR~
...
//				len=(int)((ULONG)pie-(ULONG)pi);                   //~v6hhR~
    ...
    UTRACEP("%s:setsi chklen=%d,outlen=%d,rc=%x,*pi=%02x\n",UTT,chklen,outlen,rc,*pi);//~v5mWR~//~v6BjR~
                pi+=chklen;
                po+=outlen;
...
//  *Poutlen=(int)((ULONG)po-(ULONG)Pout);                         //~v6hhR~
    *Poutlen=(int)((ULPTR)po-(ULPTR)Pout);                         //~v6hhI~
    ...
    if (Ppexplen)
	    *Ppexplen=explen;
UTRACEP("%s:RET mb2ebcdbcs rc=%d,explen=%d\n",UTT,rc,explen);             //~v5mWR~//~v6BjR~
    return rc;
...
UTRACEP("%s:opt=%x,Shandle=%d,inplen=%d,lrecl=%d,repch=%x\n",UTT,Popt,Shandle,Plen,lrecl,Prepchar);//~v66BI~//~v6BjR~
...
//          explen+=(dbcsoutlen-dbcschklen);	//expand len by SO/SI insert
			explen+=explen1;
            UTRACEP("%s:explen1=%d,explen=%d\n",UTT,explen1,explen);//~v5mPI~//~v6BjR~
            rc|=rc2;
        ...
        if (chi==' ')
        {
            UTRACEP("%s:explen=%d,pco-1=%02x\n",UTT,explen,*(pco-1));//~v5mPI~//~v6BjR~
            if (opt0 & EBC2ASC_SOSISHIFT)    //shift by SO/SI insert
                if (explen)
                {
...
//	outlen=(int)((ULONG)pco-(ULONG)Pobuff);                        //~v6hhR~
    ...
    if (Ppoutlen)
		*Ppoutlen=outlen;
UTRACEP("%s:icumb2ebcs rc=%d,outlen=%d\n",UTT,rc,outlen);          //~v6BjR~
...
//  ULONG pconverters[4];                                          //~v6hhR~
    ...
    int handle;                                                    //~v69pI~
//***************************
	UTRACEP("%s:opt=%08x,miscopt=%04x,fnm=%s\n",UTT,Popt,Pmiscopt,Pinpfilename);//~v6BjI~
...
{                                                                  //~v6f3I~
    ...
    int rc=0;                                                      //~v6f3I~
//***************************                                      //~v6f3I~
UTRACEP("%s:opt=%x,converter=%x len=%d,mb=%02x\n",UTT,Popt,Pconverter,Pinplen,*Ppmbs);//~v6f3I~//~v6BjR~
...
//      ucvext_icureset(0,(ULONG)converter);                       //~v6f3R~//~v6hhR~
		...
		UTRACEP("%s:ERR=%08x=%s\n",UTT,(UINT)err,ucvext_icuuerrname(err));//~v6f3I~//~v6BjR~
...
//***************************                                      //~v6f3R~
UTRACEP("%s:opt=%x,ucs=%04x\n",UTT,Popt,Pucs);//~v6f3R~       //~v6f4R~//~v6f7R~//~v6BjR~
...
//      ucvext_icureset(0,(ULONG)converter);                       //~v6f3R~//~v6hhR~
        ucvext_icureset(0,(ULPTR)converter);                       //~v6hhI~
		UTRACEP("%s:ERR ucs2ebc1 err=%08x=%s\n",UTT,(UINT)err,ucvext_icuuerrname(err));//~v6f3R~//~v6BjR~
		rc=4;                                                      //~v6f3R~
	}                                                              //~v6f3R~
    else                                                           //~v6f3R~
    {                                                              //~v6f4I~
...
//*FIXME :test on win                                              //~v6uNI~
    ...
    UTRACEP("%s:ucvext_icuucs2mb1 rc=%x\n",UTT,rc);                       //~v6f3R~//~v6BjR~
...
//***************************                                      //~v6f1I~//~v6f3I~
UTRACEP("%s:opt=%x\n",UTT,Popt);//~v6f1I~//~v6f3I~      //~v6f7R~  //~v6BjR~
UTRACED("Pconverters",Pconverters,2*sizeof(ULONG));          //~v6f7I~//~v6BjR~
...
UTRACEP("%s:rc=%d,chklen=%d,outlen=%d\n",UTT,rc,chklen,outlen);//~v6f3I~//~v6BjR~
...
UTRACEP("%s:opt=%x,converter=%x dbcstb=%p\n",UTT,Popt,converter,pdbcstb);//~v6f3I~//~v6BjR~
...
//          *pco=ch;                                               //~v6f4I~//~v6BiR~
    ...
    if (Ppreadlen)                                                 //~v6f3I~
    	*Ppreadlen=chklen;                                         //~v6f3I~
UTRACEP("%s:rc=%d,readlen=%d,repctr=%d\n",UTT,rc,chklen,outlen);//~v6f3I~//~v6BjR~
...
UTRACEP("%s:rc=%d,readlen=%d,repctr=%d\n",UTT,rc,readlen,repctr);//~v6f3R~//~v6BjR~
...
//      if (rc2||outlen!=1||*mbs2!=*mbs1)                          //~v6f3I~//~v6foR~
    ...
    UTRACEP("%s:opt=%x,ucs=%x,rc=%d,converter=%x\n",UTT,Popt,Pucs,rc,Pconverters);//~v6fnR~//~v6BjR~
    return rc;                                                     //~v6f3I~
...
UTRACEP("%s:to=%x,from=%x\n",UTT,cvto,cvfrom);//~v6f4R~            //~v6BjR~
...
//	pconverters[HICONV_DBCSSTARTER]=(ULONG)Pdbcsflag;	//[3]:parm to iconv getconverter//~v6f4I~//~v6hhR~
    ...
    *Ppopt=opt;                                                    //~v6f4I~
    UTRACEP("%s:opt in=%x,out=%x\n",UTT,Popt,opt);      //~v6f4I~  //~v6BjR~
...
//***************************                                      //~v6f4I~
UTRACEP("%s:errrep=%x,opt=%x\n",UTT,Perrrep,Popt);         //~v6f4I~//~v6BjR~
...
UTRACEP("%s:rc=%d,chklen=%d,errctr=%d,outlen=%d\n",UTT,rc,readlen,repctr,outlen);//~v6f4I~//~v6BjR~
