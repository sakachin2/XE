...
@rem    nmake  "BINPATH=%EXEPATH64%" "VC=%VC%" "SDK=%SDK%" "ICUHDIR=%ICUHDIR%" "ULIBHDIR=%ULIBHDIR%" "OBJULIB=%OBJULIB%" "OBJULIBMT=%OBJULIBMT%" "trace=no"  "os=w95" "w95=vc" "w98=vc10" "x64=yes" "source=no" "debug=no" %1 %2 %3 /f user.mak
nmake  "BINPATH=%EXEPATH64%" "VC=%VC%" "SDK=%SDK%" "ICUHDIR=%ICUHDIR%" "ULIBHDIR=%ULIBHDIR%" "OBJULIB=%OBJULIB%" "trace=no"  "os=w95" "w95=vc" "w98=vc10" "x64=yes" "source=no" "debug=no" %1 %2 %3 /f user.mak
@set PATH=%PATHO%
@set LIB=%LIBO%
