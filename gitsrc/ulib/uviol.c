...
//*********************************
    ...
    if (RANGE_CHKERR(Prow,Pcol,len))
        return 4;
UTRACED("uviowrtcellstr.pchCellStr",pchCellStr,len*2);                                  //~v5n8M~//~v6j0R~//~v6EqR~//~v7bxR~
    if (Sdbcstbl)                                                  //~v5inI~
...
UTRACED("uviowrtcellstr Spwkchtype",Spwkchtype,len*4);                                  //~v5n8M~//~v6j0R~//~v6EqR~//~v7bxR~
...
{
//*********************************
UTRACEP("%s:row=%d,col=%d,Svisibleblink=%d,Svisible_oldrow=%d,Svisible_oldcol=%d\n",UTT,Prow,Pcol,Svisibleblink,Svisible_oldrow,Svisible_oldcol);                    //~v589R~//~v7bxR~
...
//*********************************                                //~v324I~
	UTRACEP("%s:row=%d,col=%d,attr=0x%02x,onsw=%d\n",UTT,Prow,Pcol,Pattr,Ponsw);//~v7bxI~
    if (RANGE_CHKERR(Prow,Pcol,1))                                 //~v324I~
        return 4;                                                  //~v324I~
...
#ifdef LNXCON                                                      //~vbzPM~
//*************************************************************************//~vbzPI~
//*combining char replaced by altchar                              //~vbzPI~
//*************************************************************************//~vbzPI~
int uviol_cchar2ucsUnp(int Popt,cchar_t *Ppcchar,int Pctr,UWCHART/*int4*/*Ppucs,int *Ppucsctr)//~vbzPR~
{                                                                  //~vbzPI~
	int ii,ucsctr;                                                 //~vbzPR~
	cchar_t *pcchar;                                               //~vbzPR~
    attr_t attr;                                                   //~vbzPI~
    UWCHART *pucs,ucs;                                             //~vbzPI~
    //**********************                                       //~vbzPI~
    for (ii=0,pucs=Ppucs,pcchar=Ppcchar;ii<Pctr;ii++,pcchar++)     //~vbzPI~
    {                                                              //~vbzPM~
    	attr=pcchar->attr;                                         //~vbzPM~
        if (attr & UVIOMATTR_DBCS2) //    0x01         //1:dbcs1st,2:DBCS2ND//~vbzPM~
        	continue;               //dbcs combine is on dbcs1     //~vbzPM~
    	ucs=pcchar->chars[0];                                      //~vbzPI~
        *pucs++=ucs;                                               //~vbzPR~
    	UTRACEP("%s:ii=%d,add unp ucs=0x%04x\n",UTT,ii,ucs);       //~vbzPI~
    }                                                              //~vbzPM~
    ucsctr=PTRDIFF(pucs,Ppucs)/(int)sizeof(ucs);                   //~vbzPI~
    UTRACEP("%s:return ucsctr=%d\n",UTT,ucsctr);                   //~vbzPI~
    *Ppucsctr=ucsctr;                                              //~vbzPI~
    return 0;                                                      //~vbzPI~
}                                                                  //~vbzPI~
//*************************************************************************//~vbzPI~
//*split mode(itself/padding/shadow)                               //~vbzPI~
//*************************************************************************//~vbzPI~
int uviol_cchar2ucsSplit(int Popt,char *Ppdbcs,cchar_t *Ppcchar,int Pctr,UWCHART/*int4*/*Ppucs,int *Ppucsctr)//~vbzPR~
{                                                                  //~vbzPI~
	int ii,jj,/*swDummyBase,*/ucsctr;                                  //~vbzPR~//~v7bxR~
	cchar_t *pcchar;                                               //~vbzPR~
    attr_t attr;                                                   //~vbzPI~
    UWCHART *pucs,ucs,ucscomb;                                     //~vbzPR~
    char *pdbcs;                                                   //~vbzPI~
    //**********************                                       //~vbzPI~
//  swDummyBase=(Gutfcombaltch==UVIOM_ALTCHITSELF||Gutfcombaltch==UVIOM_ALTCHSHADOW); //when padding mode,padding char is on chars[0]//~vbzPI~//~v7bxR~
    UTRACEP("%s:Gutfcombaltch=0x%02x\n",UTT,Gutfcombaltch);//~vbzPR~//~v7bxR~
    for (ii=0,pdbcs=Ppdbcs,pucs=Ppucs,pcchar=Ppcchar;ii<Sscrwidth;ii++,pcchar++,pdbcs++)//~vbzPR~
    {                                                              //~vbzPI~
    	attr=pcchar->attr;                                         //~vbzPI~
    	ucs=pcchar->chars[0];                                      //~vbzPR~
        ucscomb=pcchar->chars[1];                                  //~vbzPR~
    	UTRACEP("%s:ii=%d,attr=0x%04x,ucs=0x%04x,ucscomb=0x%04x,\n",UTT,ii,attr,ucs,ucscomb);//~vbzPR~
        if (!ucscomb)                                              //~vbzPI~
        {                                                          //~vbzPI~
            if (attr & ATTR_COMBINE_PADDING_NOHCOPY) //padding to be dropped from hcopy//~vbzPI~
                continue;                                          //~vbzPI~
	        *pucs++=ucs;                                           //~vbzPI~
        }                                                          //~vbzPI~
        else                                                       //~vbzPI~
        for (jj=1;jj<CCHARW_MAX;jj++) //[0] is dummy base when shadow/split mode, [1]=0 for  padding mode//~vbzPR~
        {                                                          //~vbzPI~
			ucs=pcchar->chars[jj];                                 //~vbzPI~
            if (!ucs)                                              //~vbzPM~
            	break;                                             //~vbzPM~
    		UTRACEP("%s:ii=%d,jj=%d,add combining ucs=0x%04x\n",UTT,ii,jj,ucs);//~vbzPR~
            *pucs++=ucs;                                           //~vbzPI~
        }                                                          //~vbzPI~
    }                                                              //~vbzPI~
    ucsctr=PTRDIFF(pucs,Ppucs)/(int)sizeof(ucs);                   //~vbzPI~
...
//*************************************************************************//~vbzPI~
int uviol_cchar2ucsComb(int Popt,char *Ppdbcs,cchar_t *Ppcchar,int Pctr,UWCHART/*int4*/*Ppucs,int *Ppucsctr)//~vbzPI~
{                                                                  //~vbzPI~
	int ii,jj,swDummyBase,ucsctr;                                  //~vbzPR~
	cchar_t *pcchar;                                               //~vbzPI~
    attr_t attr;                                                   //~vbzPI~
    UWCHART *pucs,ucs;                                             //~vbzPI~
    char *pdbcs;                                                   //~vbzPI~
    //**********************                                       //~vbzPI~
    UTRACED("dbcs",Ppdbcs,Pctr);                                   //~vbzPI~
    swDummyBase=0;                                                 //~vbzPI~
    for (ii=0,pdbcs=Ppdbcs,pucs=Ppucs,pcchar=Ppcchar;ii<Sscrwidth;ii++,pcchar++,pdbcs++)//~vbzPI~
    {                                                              //~vbzPI~
    	attr=pcchar->attr;                                         //~vbzPI~
    	ucs=pcchar->chars[0];                                      //~vbzPI~
        ...
        {                                                          //~vbzPI~
        	if (ucs==Guviomdbcspad) //' '/','                      //~vbzPR~
	        	continue;               //skip padding             //~vbzPR~
        	if (ucs==UVIOM_COMBINE_ALTCH2) //' '/',' or  '.'       //~vbzPI~
            {                                                      //~vbzPI~
	        	swDummyBase=0;                                     //~vbzPI~
	        	continue;               //skip padding             //~vbzPI~
            }                                                      //~vbzPI~
        }                                                          //~vbzPI~
        for (jj=0;jj<CCHARW_MAX;jj++)                              //~vbzPI~
        {                                                          //~vbzPI~
			ucs=pcchar->chars[jj];                                 //~vbzPI~
	        if (jj==0 && ucs==COMBAINE_BASE_SPLIT_ITSELF)  //u00a0 base for combining char when split itself//~vbzPI~
            {                                                      //~vbzPI~
            	swDummyBase=1;                                     //~vbzPI~
            	continue;                                          //~vbzPI~
            }                                                      //~vbzPI~
            if (!ucs)                                              //~vbzPI~
            	break;                                             //~vbzPI~
            if (swDummyBase)                                       //~vbzPI~
            	swDummyBase=2;	//dummy + combining char           //~vbzPI~
            *pucs++=ucs;                                           //~vbzPI~
        }                                                          //~vbzPI~
    }                                                              //~vbzPI~
    ucsctr=PTRDIFF(pucs,Ppucs)/(int)sizeof(ucs);                   //~vbzPI~
...
//*************************************************************************//~vbzPM~
//*rc=0:OK ;4:readconsole err, -1 :end of line                     //~vbzPM~
//*************************************************************************//~vbzPM~
int uvioGetCellData(int Popt,int Prow,UWCHART/*int4*/*Ppucs,int *Ppucsctr)//~vbzPM~
{                                                                  //~vbzPM~
	int rc,maxrow,ucsctr;                                          //~vbzPR~
	cchar_t workcchar[UVIOM_MAXCOL*2];                             //~vbzPR~
    char *pdbcs;                                                   //~vbzPI~
//************************                                         //~vbzPM~
  ...
  if (1)                                                           //~v7bxI~
  {                                                                //~v7bxI~
    rc=uviom_readConsoleUcs(Popt,Prow,0,Sscrwidth,Ppucs);          //~v7bxI~
    ucsctr=Sscrwidth;                                              //~v7bxI~
  }                                                                //~v7bxI~
  else                                                             //~v7bxI~
  {                                                                //~v7bxI~
    rc=uviom_readConsole(Popt,Prow,0,Sscrwidth,workcchar);         //~vbzPR~
    if (rc!=0)	//read faled                                       //~vbzPM~
    {                                                              //~vbzPM~
		UTRACEP("%s:return 4 by readconsole err\n",UTT);           //~vbzPR~
        return 4;                                                  //~vbzPM~
    }                                                              //~vbzPM~
    if (UTF_COMBINEMODE_NP())                                      //~vbzPR~
		uviol_cchar2ucsUnp(Popt,workcchar,Sscrwidth,Ppucs,&ucsctr);//~vbzPI~
    else                                                           //~vbzPI~
    if (UTF_COMBINEMODE())                                         //~vbzPR~
		uviol_cchar2ucsComb(Popt,pdbcs,workcchar,Sscrwidth,Ppucs,&ucsctr);//~vbzPR~
    else                                                           //~vbzPI~
		uviol_cchar2ucsSplit(Popt,pdbcs,workcchar,Sscrwidth,Ppucs,&ucsctr);//~vbzPR~
  }                                                                //~v7bxI~
    *Ppucsctr=ucsctr;                                              //~vbzPI~
    UTRACEP("%s:ucsctr=%d\n",UTT,ucsctr);                          //~vbzPR~
