...
#include <ucvext.h>                                                //~v7a3I~
#include <ucvebc.h>
//#include <ucvext.h>                                              //~v7a3R~
...
//**********************                                           //~v5n1I~
UTRACEP("%s:codepage=%s\n",UTT,Pcharset);                                 //~v5n1I~//~v7a3R~
...
//  	uerrmsg("Invalid Codepage:%s",0,                           //~v5n1R~//~v69cR~
      if (strcmp(Pcharset,"?"))                                    //~v7dqI~
    	uerrmsg("Invalid Windows Codepage:%s (CONVERTER 2)",0,                   //~v69cI~//~v7dMR~
        		Pcharset);                                         //~v5n1R~
...
//          else                                                   //~v6xaI~//~v7dMR~
//		    	uerrmsgcat(" (\"ICU\" option is required for ICU converter).",//~v6xaI~//~v7a3R~
//		    	           " (ICUコンバータ名 の場合は \"ICU\" オプションが必要です)");//~v6x8R~//~v7a3R~
//		    	uerrmsgcat(" (\"ICU\" option of cfg file is required for ICU converter).",//~v7a3I~//~v7dMR~
//  	    	           " (ICUコンバータ名 の場合は cfg ファイルで \"ICU\" オプションが必要です)");//~v7a3I~//~v7dMR~
...
{                                                                  //~v5n1M~
#ifdef LNX                                                         //~v5n1I~
	UTRACEP("%s:Popt=%04x,Pconverter=%s\n",UTT,Popt,Pconverter);   //~v7a3I~
    if (Pconverter)                                                //~v5n7I~
...
UTRACEP("%s:iconvopen from %s to %s ,rc=%x\n",UTT,ENCODING_UCS2,Pcharset,hiconv);//~v624R~//~v65cR~//~v6WdR~//~v7a3R~
    	if (hiconv==(iconv_t)(-1))                                 //~v6a0I~
    	{                                                          //~v6a0I~
...
//  ULONG ucs;                                                     //~v5n6I~//~v6hdR~
    ...
    int lasterr;                                                   //~v6BHI~
//*********************                                            //~v5n1M~
    UTRACEP("%s:cv=%d,opt=%x,inplen=%d data=%02x%02x%02x\n",UTT,Pconverter,Popt,Pinplen,*Ppebc,(Pinplen>1?*(Ppebc+1):0),(Pinplen>2?*(Ppebc+2):0));//~v5n6I~//~v62ER~//~v77BR~
...
//  outlen=olen;                                                   //~v66VI~//~v6BkR~
    ...
    UTRACEP("%s:hiconv=%p,inp=%02x%02x%02x%02x,ilen=%d,olen=%d\n",UTT,hiconv,wkebc[0],wkebc[1],wkebc[2],wkebc[3],ilen,olen);//~v5n7R~//~v7dqR~
...
#endif                                                             //~v65cI~
//***************************
    UTRACEP("%s:len=%d,mbs=%02x%02x\n",UTT,Pinplen,*Ppmbs,(Pinplen>1?*(Ppmbs+1):0));//~v5mTI~//~v6m2R~//~v6BjR~//~v7dMR~
...
//***************************
    ...
    UTRACEP("%s:hiconv=%p,ucs=%04x\n",UTT,hiconv,Pucs);//~v5mTI~   //~v7dqR~
//  rciconv=iconv(hiconv,ICONV_PCTYPE(&pci),&ilen,&pco,&olen);     //~v5n8R~//~v5ntR~
    rciconv=iconv(hiconv,ICONV_PCTYPE(&pci),ICONV_SZTYPE(&ilen),&pco,ICONV_SZTYPE(&olen));//~v5ntI~
...
//*********************                                            //~v6k4I~
    ...
    if (Popt & UCVUCS_ERRSTOP)                                     //~v7a3I~
    	wctmbopt|=UWCTMBO_REVERSECHK;	//avoid reversechk for recursive to utfwcwidth//~v7a3I~
...
//  int rc=0,len;                                                  //~v5n1R~//~v7dqR~
    int rc=0,len=0;                                                //~v7dqI~
#ifdef LNX                                                         //~v5mtM~
...
//    UTRACEP("ucvext_iconvucs2local  ucs2utf ucs=%x,len=%d\n",Pucs,len);//~v6k4I~//~v6m2R~
  ...
  {                                                                //~v5n8I~
    ...
    UTRACEP("%s:hiconv=%p,ucs=%04x\n",UTT,hiconv,Pucs);//~v5mTI~//~v6m2R~//~v6WdR~//~v7dqR~
    UTRACED("ucvext_iconvucs2local pci",pci,(int)ilen);            //~v6WdR~
//  rciconv=iconv(hiconv,ICONV_PCTYPE(&pci),&ilen,&pco,&olen);     //~v5n8R~//~v5ntR~
    rciconv=iconv(hiconv,ICONV_PCTYPE(&pci),ICONV_SZTYPE(&ilen),&pco,ICONV_SZTYPE(&olen));//~v5ntI~
    UTRACEP("%s:rciconv=%d ilen=%d,olen=%d\n",UTT,rciconv,ilen,olen);//~v62NR~//~v6m2R~//~v7dqR~
...
//    else                                                           //~v6uMI~//~v6WdR~
	if (rciconv==(size_t)-1)                                       //~v5mTI~
	{                                                              //~v5mTI~
		UTRACEP("%s:err=%d\n",UTT,errno);             //~v5mTI~    //~v7dqR~
      if (Popt & EBC2ASC_EXT_SETSUBCHRC)                           //~v7dMI~
	   	rc=UCVEXTRC_SUBCH;                                         //~v7dMI~
      else                                                         //~v7dMI~
		rc=4;                                                      //~v5mTI~
	}                                                              //~v5mTI~
    else                                                           //~v5n1I~
...
//  	len=(int)((ULONG)pco-(ULONG)pco0);                         //~v5n1M~//~v6uMR~
    ...
    {                                                              //~v7dMI~
        if (Spcfg)                                                 //~v7dMI~
        //Spcfg=0 when called from uxcext_setupsubch               //~v7dMI~
        {                                                          //~v7dMI~
            if (Popt & EBC2ASC_STAT_SO)                            //~v7dMR~
            {                                                      //~v7dMR~
                len=(int)strlen(Spcfg->UCECsubcharlocaldbcs);      //~v7dMR~
                memcpy(wkmbs,Spcfg->UCECsubcharlocaldbcs,(UINT)len);//~v7dMR~
            }                                                      //~v7dMR~
            else                                                   //~v7dMR~
            {                                                      //~v7dMR~
                len=1;                                             //~v7dMR~
                *wkmbs=Spcfg->UCECsubcharlocalsbcs;                //~v7dMR~
            }                                                      //~v7dMR~
        }                                                          //~v7dMI~
    }                                                              //~v7dMI~
#else                                                              //~v5n1M~
...
//  rc2=ucvext_winucs2cp1(opt,Pconverter,Pucs,wkmbs,&len);         //~v6x5R~
    ...
    {                                                              //~v7dMI~
    	rc=rc2;	//rcsubch                                          //~v66DI~
        if (len==1 && Popt & EBC2ASC_STAT_SO)                      //~v7dMI~
            if (Spcfg && *Spcfg->UCECsubcharlocaldbcs)             //~v7dMR~
            {                                                      //~v7dMI~
            	len=(int)strlen(Spcfg->UCECsubcharlocaldbcs);      //~v7dMI~
                strcpy(wkmbs,Spcfg->UCECsubcharlocaldbcs);         //~v7dMI~
                UTRACEP("%s:STAT_SO & len=1 set sbcharlocaldbcs=%s\n",UTT,wkmbs);//~v7dMI~
            }                                                      //~v7dMI~
    }                                                              //~v7dMI~
#endif                                                             //~v5n1M~
  }//to utf                                                        //~v5n8I~
...
//*************************************************************************//~v5n1I~
//*from xcv                                                        //~v7a3R~
//*translate any to any by iconv/win api                           //~v7a3I~
...
//  UTRACEP("%s:Popt=%x,apiopt=%x inplen=%d,cp=%d,Scodepage_invalidflags=%d\n",UTT,Popt,Papiopt,Pinplen,Pconverter,Scodepage_invalidflags);//~v6WdR~
	opt=Papiopt;                                                   //~v6BHI~
#ifndef TEST                                                       //+v7dMI~
    opt|=MB_ERR_INVALID_CHARS;                                     //+v7dMI~
#endif                                                             //+v7dMI~
