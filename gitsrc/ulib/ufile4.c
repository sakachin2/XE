...
static  int  SoptMask;                                             //~vat5I~
#define OPTMASK_DIRALL   0x01                                      //~vat5I~
#define OPTMASK_FILEALL  0x02                                      //~vat5I~
static  char *Sdirmask;                                            //~v063I~
static  char *Sfilemask;                                           //~v063I~
static  char *SfilemaskM,*SdirmaskM;                               //~vat8R~
...
//  return ftell(Pfh);                                             //~v6xeR~
    return uftell(Pfh);                                            //~v6xeI~
                                                                   ...
                                                                   //~v050I~
//**********************************************************************//~vat8M~
char *multipleMask(char *Pstrs,int *Ppctr)                         //~vat8M~
{                                                                  //~vat8M~
    #define MASK_BUFFSZ  1024                                      //~vat8M~
    char buff[MASK_BUFFSZ],*pci,*pco,*pc,*pce;                      //~v5mTI~//~vat8M~
    int len;                                                       //~vat8M~
    int ctr=0;                                                     //~vat8M~
    #define MASK_DELM ';'                                          //~vat8M~
    //*******************                                          //~vat8M~
    UTRACEP("%s:input=%s\n",UTT,Pstrs);                            //~vat8M~
    if (strlen(Pstrs)+1>MASK_BUFFSZ)                               //~vat8M~
    	return Pstrs;                                              //~vat8M~
    for (pci=Pstrs,pco=buff;;)                                     //~vat8M~
    {                                                              //~vat8M~
        pc=strchr(pci,MASK_DELM);                                  //~vat8M~
        if (!pc)                                                   //~vat8M~
            pce=pci+strlen(pci);                                   //~vat8M~
        else                                                       //~vat8M~
            pce=pc;                                                //~vat8M~
...
//  pc=malloc(len);                                                //~vat8M~//~v7dzR~
    ...
    *Ppctr=ctr;                                                    //~vat8M~
    UTRACEP("%s:ctr=%d\n",UTT,ctr);                                //~vat8M~
    UTRACED("out",pc,len);                                         //~vat8M~
    return pc;                                                     //~vat8M~
}                                                                  //~vat8M~
int  udirwalkRoot(int Popt,char *Pfullpath,char *Pdirmask,char *Pfilemask,//~v7dzI~
                int Pattrmask,int Pstopdepth,                      //~v7dzI~
               int (*Pfunc)(char *Pdir,char *Pfullpath,UDIRLIST *Pudirlist,//~v7dzI~
                            int Pdepth,void *Pdata,int *Papplrc),  //~v7dzI~
               void *Pfuncdata,int *Prc)                           //~v7dzI~
{                                                                  //~v7dzI~
    char fullpath[_MAX_PATH];                                      //~v7dzI~
    PUDIRLIST pudirlist,pudirlist0;                                //~v7dzI~
    int ii,rc=0,pathlen,ctr;                                       //~v7dzR~
//**************                                                   //~v7dzI~
	UTRACEP("%s:Pfullpath=%s,Pdirmask=%s\n",UTT,Pfullpath,Pdirmask);//~v7dzI~
    strcpy(fullpath,Pfullpath);                                    //~v7dzI~
    pathlen=(int)strlen(Pfullpath);                                //~v7dzI~
    if (pathlen>0 && *(fullpath+pathlen-1)!=DIR_SEPC)              //~v7dzM~
    	*(fullpath+pathlen++)=DIR_SEPC;                            //~v7dzM~
	char *proot=Pdirmask;                                          //~v7dzI~
    char *pdirmask=strstr(Pdirmask,";;");                          //~v7dzI~
    *pdirmask=0;                                                   //~v7dzI~
    pdirmask+=2;                                                   //~v7dzI~
    char *prootmaskM=multipleMask(proot,&ctr);                     //~v7dzI~
    int entryno;                                                   //~v7dzI~
    int sorttype=0;                                                //~v7dzI~
    int attrmask=FILE_DIRECTORY;  //also dir at once               //~v7dzI~
    strcpy(fullpath+pathlen,DIR_ALLMEMB);                          //~v7dzI~
    if (Popt & UDIRWALK_NOMSG)    //no msg out req                 //~v7dzI~
        entryno=udirlistnomsg(fullpath,(UINT)attrmask,&pudirlist0,sorttype);//~v7dzI~
    else                                                           //~v7dzI~
        entryno=udirlist(fullpath,(UINT)attrmask,&pudirlist0,sorttype);//~v7dzI~
	UTRACEP("%s:entryno=%d,dir=%s\n",UTT,entryno,fullpath);        //~v7dzI~
    if (entryno==-1)    //list not created                         //~v7dzI~
    {                                                              //~v7dzI~
    	uerrmsg("%s has no such dir(root dir-mask=%s)\n",          //~v7dzI~
                "%s には該当がありません(root dir-mask=%s)\n",     //~v7dzI~
                Pfullpath,proot);                                  //~v7dzI~
        return 0;                                                  //~v7dzI~
    }                                                              //~v7dzI~
    for (ii=0,pudirlist=pudirlist0;ii<entryno;ii++,pudirlist++)    //~v7dzI~
    {                                                              //~v7dzI~
		UTRACEP("%s:ii=%d,name=%s\n",UTT,ii,pudirlist->name);      //~v7dzI~
        if (!strcmp(pudirlist->name,".")                           //~v7dzI~
        ||  !strcmp(pudirlist->name,"..")                          //~v7dzR~
        ||  !(pudirlist->attr & FILE_DIRECTORY))                   //~v7dzI~
            continue;                                              //~v7dzI~
		if (udirwalkChkMask(ctr!=0,pudirlist->name,prootmaskM))    //~v7dzR~
        {                                                          //~v7dzI~
        	UTRACEP("%s:not ROOT MASK name=%s\n",UTT,pudirlist->name);//~v7dzI~
        	continue;                                              //~v7dzI~
        }                                                          //~v7dzI~
        strcpy(fullpath+pathlen,pudirlist->name);                  //~v7dzI~
    	int rc2=udirwalk(Popt,fullpath,pdirmask,Pfilemask,Pattrmask,Pstopdepth,Pfunc,Pfuncdata,Prc);//~v7dzR~
        if (rc2>rc)                                                //~v7dzI~
        	rc=rc2;                                                //~v7dzI~
    }//for all file                                                //~v7dzI~
    return rc;                                                     //~v7dzI~
...
//*save parm to avoid pass through walk                            //~v063I~
    UTRACEP("%s:Popt=%08x,Pstartdir=%s,Pdirmask=%s,Pfilemask=%s\n",UTT,Popt,Pstartdir,Pdirmask,Pfilemask);//~vat5I~
    Sopt=Popt;                                                     //~v063I~
//  Sapplytopmask=(Popt & UDIRWALK_TOPMASK);                       //~v528I~//~vat5R~
    Sapplytopmask=(Popt & (UDIRWALK_TOPMASK|UDIRWALK_DIRMASK_PATH|UDIRWALK_DIRMASK_ONLY));//~vat5R~
    Sfunc=Pfunc;                                                   //~v063I~
...
//      Sfilemask="*.*";                                           //~v369R~
    ...
    if (!strcmp(Sdirmask,DIR_ALLMEMB))                             //~vat5I~
        SoptMask|=OPTMASK_DIRALL;//0x01                            //~vat5I~
    if (!strcmp(Sfilemask,DIR_ALLMEMB))                            //~vat5I~
        SoptMask|=OPTMASK_FILEALL;//0x01                           //~vat5I~
    int ctr;                                                       //~vat8M~
    SdirmaskM=multipleMask(Sdirmask,&ctr);                         //~vat8I~
    if (ctr>1)                                                     //~vat8M~
		Sopt|=UDIRWALK_DIRMASK_MULT;  //apply dirmask to topdir only//~vat8R~
    SfilemaskM=multipleMask(Sfilemask,&ctr);                       //~vat8I~
    if (ctr>1)                                                     //~vat8M~
		Sopt|=UDIRWALK_FILEMASK_MULT;  //apply dirmask to topdir only//~vat8R~
...
//              udirlists.attr=(char)fstat3.attrFile;              //~v59eR~
    ...
    if (!rc)                                                       //~v063I~
    {                                                              //~v063I~
	    if (Pdirmask && strstr(Pdirmask,";;"))                     //~v7dzR~
    	    return udirwalkRoot(Popt,fullpath,Pdirmask,Pfilemask,Pattrmask,Pstopdepth,Pfunc,Pfuncdata,Prc);//~v7dzI~
...
#endif                                                             //~v56pI~
//*******************************                                  //~v063I~
	int swPathOK=(Ppudirlist->workflag & UDIRLWF_WALK_DIRMASK_OK)	//ufile.h    0x02 //for dirwalk//~vat5I~
				&&  (Sapplytopmask & UDIRWALK_DIRMASK_PATH)	    	//all path chk//~vat5I~
				&&  !(Sapplytopmask & UDIRWALK_DIRMASK_ONLY);	    //aply all subdir if found//~vat5I~
	int swPathNG=(Ppudirlist->workflag & UDIRLWF_WALK_DIRMASK_NG);  //ufile.h    0x02 //for dirwalk//~vat5I~
	UTRACEP("%s:level=%d,Pdir=%s,dirmask=%s,filemask=%s,wkflag=%02x,swPathNG=%d,swPathOK=%d\n",UTT,Plevel,Pdir,Sdirmask,Sfilemask,Ppudirlist->workflag,swPathNG,swPathOK);//~v6k4R~//~vat5R~//~vat8I~
    if (swPathNG && !(Sapplytopmask & UDIRWALK_DIRMASK_PATH))      //~vat8I~
    {                                                              //~vat8I~
    	UTRACEP("%s:pathNG and not DIRMASK_PATH,dir=%s\n",UTT,Pdir);//~vat8I~
    	return 0;                                                  //~vat8I~
    }                                                              //~vat8I~
...
//  if (Sopt & UDIRWALK_DIRCALL)                                   //~v063I~//~vat5R~
    if (Sopt & UDIRWALK_DIRCALL && !swPathNG)                      //~vat5I~
    {                                                              //~v063I~
        Scallctr++;                                                //~v063I~
...
//  if (Sopt & UDIRWALK_FILECALL)                                  //~v063I~//~vat8R~
    if (Sopt & UDIRWALK_FILECALL && !swPathNG)                     //~vat8I~
    {                                                              //~v063I~
...
//    {                                                            //~v57aR~
      ...
      {                                                            //~v779I~
        ...
        UTRACEP("%s:TOPMASK !=4,fullpath=%s\n",UTT,fullpath);      //~v779I~
      }                                                            //~v779I~
      else                                                         //~v57aI~
	  if (Sapplytopmask & UDIRWALK_DIRMASK_PATH)	//search all subdir//~vat5R~
      {                                                            //~vat5I~
        strcpy(fullpath+pathlen,DIR_ALLMEMB);	//chk all descendant//~vat5I~
        UTRACEP("%s:DIRMASK_PATH,fullpath=%s\n",UTT,fullpath);     //~vat5I~
      }                                                            //~vat5I~
#else                                                              //~vat8M~
	  if (Sopt & UDIRWALK_FILEMASK_MULT)                           //~vat8I~
      {                                                            //~vat8I~
		strcpy(fullpath+pathlen,DIR_ALLMEMB);                      //~vat8I~
		UTRACEP("%s:multiple FILEMASK fullpath=%s\n",UTT,fullpath);//~vat8I~
      }                                                            //~vat8I~
#endif                                                             //~vat8M~
      else                                                         //~vat5I~
      {                                                            //~v779I~
        strcpy(fullpath+pathlen,Sfilemask);                        //~v063I~
        UTRACEP("%s:single FILEMASK,fullpath=%s\n",UTT,fullpath);      //~v779I~//~vat8R~
      }                                                            //~v779I~
      ...
      if (Sopt & UDIRWALK_NOMSG)    //no msg out req               //~v56bI~
        ...
        UTRACEP("%s:FILECALL udirlist dir=%s,entryno=%d\n",UTT,fullpath,entryno);//~vat5R~//~vat8R~
#ifdef UNX                                                         //~v578I~
...
//                      pudirlist->srcattr|=FILE_SLINKERR;         //~v6k4I~//~v6keR~
	  			...
	  			if (Sopt & UDIRWALK_FILEMASK_MULT)                 //~vat8I~
	                if (udirwalkChkMask(Sopt & UDIRWALK_FILEMASK_MULT,pudirlist->name,SfilemaskM))    ////~vat8R~//~v7dzR~
    	            {                                              //~vat8R~
        	        	UTRACEP("%s:FILEMASK_MULTI unmatch name=%s\n",UTT,pudirlist->name);//~vat8R~
            	        continue;                                  //~vat8R~
                	}                                              //~vat8R~
...
//    }//not the case wild card dir name and top level of xds(TOPMASK)//~v57aR~
      Sapplytopmask |= 4; //dir mask applyed                       //~v56uR~
      UTRACEP("%s:TOPMASK set 4 Sapplytopmask=%04x\n",UTT,Sapplytopmask);//~v779I~
    }//file call                                                   //~v063I~
...
//  if (Sapplytopmask)                                             //~v57aR~
    ...
    {                                                              //~v528I~
        ...
        {                                                          //~v779I~
            if (Sapplytopmask & UDIRWALK_DIRMASK_ONLY)	//not apply to descendant//~vat5I~
            {                                                      //~vat5I~
	        	UTRACEP("%s:TOPMASK 2 is on,return by DIRMASK_ONLY fullpath=%s\n",UTT,fullpath);//~vat5I~
            	return 0;                                          //~vat5R~
            }                                                      //~vat5I~
	    	strcpy(fullpath+pathlen,DIR_ALLMEMB);                  //~v528I~
        	UTRACEP("%s:TOPMASK 2 is on,fullpath=%s\n",UTT,fullpath);//~v779I~
        }                                                          //~v779I~
        else		//1st dir mask apply                           //~v528I~
        {                                                          //~v779I~
//  	    strcpy(fullpath+pathlen,Sdirmask);                     //~v528I~//~vat8R~
	    	strcpy(fullpath+pathlen,DIR_ALLMEMB);                  //~vat8I~
        	UTRACEP("%s:TOPMASK not 2,fullpath=%s\n",UTT,fullpath);//~v779I~
        }                                                          //~v779I~
		Sapplytopmask |= 2;	//dir mask applyed                     //~v528I~
        UTRACEP("%s:TOPMASK set 2 on\n",UTT);                      //~v779I~
    }                                                              //~v528I~
    else                                                           //~v528I~
	if (Sapplytopmask & UDIRWALK_DIRMASK_PATH)	//apply dir mask to top level option//~vat5I~
    {                                                              //~vat5I~
	    strcpy(fullpath+pathlen,DIR_ALLMEMB);                      //~vat5I~
        UTRACEP("%s:Not TOPMASK,fullpath=%s\n",UTT,fullpath);      //~vat5I~
    }                                                              //~vat5I~
    else                                                           //~vat5I~
    {                                                              //~v779I~
//  strcpy(fullpath+pathlen,Sdirmask);                             //~v063I~//~vat8R~
	    strcpy(fullpath+pathlen,DIR_ALLMEMB);                      //~vat8I~
        UTRACEP("%s:Not TOPMASK,fullpath=%s\n",UTT,fullpath);      //~v779I~
    }                                                              //~v779I~
#ifdef UNX                                                         //~v56pI~
...
//    	ufree(pudirlist0s);                                        //~v579R~//~v6qaR~
  ...
  if (Sopt & UDIRWALK_NOMSG)    //no msg out req                   //~v56bI~
    ...
    entryno=udirlist(fullpath,(UINT)attrmask,&pudirlist0,sorttype);//~v063R~
    UTRACEP("%s:DIRMASK udirlist dir=%s,entryno=%d\n",UTT,fullpath,entryno);//~vat5R~//~vat8R~
#ifdef UNX                                                         //~v578I~
     	Gufile_opt&=~GFILEOPT_STATCD;                              //~v578I~
        if (cdoksw==2)                                             //~v57aI~
...
//          if ((*pudirlist->name=='.')                            //~v194I~
            ...
            ||  !(pudirlist->attr & FILE_DIRECTORY))               //~v063I~
                continue;                                          //~v063I~
            UTRACEP("%s:DIRMASK name=%s,Sapplytopmask=%08x,workflag=%02x\n",UTT,pudirlist->name,Sapplytopmask,pudirlist->workflag);//~vat5R~
            pudirlist->workflag=0;                                 //~vat5I~
			if (swPathOK)	//ascendant dir matched with dirmask   //~vat5R~
            {                                                      //~vat5I~
				pudirlist->workflag=UDIRLWF_WALK_DIRMASK_OK;	//ufile.h    0x04 //for dirwalk//~vat5I~
                UTRACEP("%s:name=%s,DIRMASK acendant PATH ok\n",UTT,pudirlist->name);//~vat5R~
            }                                                      //~vat5I~
            else                                                   //~vat5I~
            if (!(SoptMask & OPTMASK_DIRALL))	//dirmask!="*"     //~vat5I~
            {                                                      //~vat5I~
//  			if (Sapplytopmask & UDIRWALK_DIRMASK_PATH)	    //all path chk//~vat5R~//~vat8R~
//              {                                                  //~vat5I~//~vat8R~
                    if (udirwalkChkMask(Sopt & UDIRWALK_DIRMASK_MULT,pudirlist->name,SdirmaskM))    //         //~vat5I~//~vat8R~//~v7dzR~
                    {                                              //~vat5I~
                        UTRACEP("%s:DIRMASK chk NG, dir=%s\n",UTT,pudirlist->name);//~vat5R~
			            pudirlist->workflag=UDIRLWF_WALK_DIRMASK_NG;	//ufile.h    0x02 //for dirwalk//~vat5I~
                    }                                              //~vat5I~
                    else                                           //~vat5I~
                    {                                              //~vat5I~
                        UTRACEP("%s:DIRMASK chk OK dir=%s\n",UTT,pudirlist->name);//~vat5I~
				        pudirlist->workflag=UDIRLWF_WALK_DIRMASK_OK;	//ufile.h    0x04 //for dirwalk//~vat5R~
                    }                                              //~vat5I~
//              }                                                  //~vat5I~//~vat8R~
            }                                                      //~vat5I~
            dirctr++;                                              //~v063I~
...
#endif                                                             //~v6VxI~
//**********************************************************************//~vat5I~//~vat8M~
//*Popt!=0:multiple mask                                           //~v7dzI~
//**********************************************************************//~v7dzI~
int  udirwalkChkMask(int Popt,char *Pname,char *Pmask)                                       //~vat5I~//~vat8R~//~v7dzR~
{                                                                  //~vat5I~//~vat8M~
//  int opt=UFWC_DOS_DEFAULT | UFWC_CASE_DEFAULT | UFWC_0BYTE;     //~vat5I~//~vat8R~
    int opt=UFWC_CASE_DEFAULT | UFWC_0BYTE;                        //~vat8I~
    char *pc;                                                      //~vat8M~
    int rc=0;                                                      //~vat8M~
//  rc=ufilewildcomp(opt,Pname,Sdirmask,0/*pdbcs*/)!=0;        //~vat5R~//~vat8M~
    for (pc=Pmask;;)                                               //~vat8M~
    {                                                              //~vat8M~
	    UTRACEP("%s:Pname=%s and mask=%s\n",UTT,Pname,pc);         //~vat8M~
		rc=ufilewildcomp(opt,Pname,pc,0/*pdbcs*/)!=0;              //~vat8M~
        if (!rc)	//match	                                       //~vat8M~
        	break;                                                 //~vat8M~
	    if (!Popt)                                                 //~vat8R~
        	break;                                                 //~vat8M~
        pc+=strlen(pc)+1;                                          //~vat8M~
        if (!*pc)                                                  //~vat8M~
        	break;                                                 //~vat8M~
    }                                                              //~vat8M~
