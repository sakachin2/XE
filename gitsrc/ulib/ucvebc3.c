//*CID://+vbBqR~:                                   update#=  471; //~vbBqR~
//***********************************************************************
//vbBq:241108 display green when lcmd copy b2m/m2b when cv err     //~vbBqI~
//v7dN:241107 dd2b; display green when m2b err(copy utf8 line to ebc file)//~v7dNI~
//v7dG:241104 coding miss b2uDbcs1 used converterSbcs(But sjis was converted successfully)//~v7dGI~
//vbBk:241103 (BUG)cfg-SUCHAR was not work                         //~vbBkI~
//v7dC:241030 (BUG)variable was not initialized                    //~v7dCI~
...
//************************
    UTRACEP("%s:opt=0x%08x,flag=%08x\n",UTT,Popt,Ppcfg->UCECflag); //~v7dnI~
	opt=0;
    ...
    {
//        if (!(Ppcfg->UCECflag & UCECF_OPENERR)  //converter=0      //~vbB9I~//~vbBqR~
////      &&  !(Ppcfg->UCECflag & UCECF_USE_CONVERTER)    //converter=0//~vbB9I~//~v7diR~//~vbBqR~
//        &&   Ppcfg->UCECflag2 & UCECF2_CP931)           //SJISOPT:LATIN//~vbB9I~//~vbBqR~
//        {                                                          //~vbB9I~//~vbBqR~
////          Gucvebc_stat|=UCVEBCS_SJISLATIN;    //     0x2000 //converter=0 and "sjisopt LATIN"//~vbB9I~//~v7dmR~//~vbBqR~
////          mapopt|=EBC2ASC_SBCSMAP037;             //sbca=cp037   //~v7dmI~//~v7dyR~//~vbBqR~
//            UIUIBITON(mapopt,EBC2ASC_SBCSMAP037);           //sbca=cp037//~v7dyR~//~vbBqR~
//            mapopt|=EBC2ASC_CP931;              //dbcs CP300       //~vataI~//~vbBqR~
//            UTRACEP("%s:set ebcstat:SJISLatin by converter=0 and flag2 Gucvebc_stat=0x%08x\n",UTT,Gucvebc_stat);//~vbB9I~//~vbBqR~
//        }                                                          //~vbB9I~//~vbBqR~
//        else                                                       //~vbB9I~//~vbBqR~
    	if (Ppcfg->UCECflag & UCECF_CP930) //use converter
        {                                                          //~vbB9I~
    		mapopt|=EBC2ASC_CP290;  //Japanese katakana ext
            UTRACEP("%s:set EBC2ASC_CP290 by flag=CP930\n",UTT);   //~vbB9I~
        }                                                          //~vbB9I~
//        else                                                     //~vbBqR~
//        if (Ppcfg->UCECflag2 & UCECF2_CP931) //cp300+cp037  skip set EBC2ASC_EXT//~vbB9I~//~vbBqR~
//        {                                                          //~vbB9I~//~vbBqR~
////          mapopt|=EBC2ASC_SBCSMAP037;  //sbcs is cp037           //~v7dmR~//~v7dyR~//~vbBqR~
//            UIUIBITON(mapopt,EBC2ASC_SBCSMAP037);  //sbcs is cp037 //~v7dyI~//~vbBqR~
//            mapopt|=EBC2ASC_CP931;              //dbcs CP300       //~vataI~//~vbBqR~
//            UTRACEP("%s:cp931\n",UTT);                             //~vbB9I~//~vbBqR~
//        }                                                          //~vbB9I~//~vbBqR~
//        else                                                       //~vbB9I~//~v7dnR~
//        if (Ppcfg->UCECflag2 & UCECF2_CP037) //CONVERTER=0 & SBCACHARSET=cp037 or cmdline /ebcus//~v7diI~//~v7dnR~
//        {                                                          //~v7diI~//~v7dnR~
//            mapopt|=EBC2ASC_SBCSMAP037;  //sbcs is cp037           //~v7dmR~//~v7dnR~
//            UTRACEP("%s:cp037 by UCECF2_CP037\n",UTT);                             //~v7diI~//~v7dmR~//~v7dnR~
//        }                                                          //~v7diI~//~v7dnR~
    	else                                                       //~v7diI~
    	if (Ppcfg->UCECflag & UCECF_SBCSDEF) //CONVERTER=0 & SBCACHARSET=cp037 or cmdline /ebcus//~v7dmI~
        {                                                          //~v7dmI~
//    		mapopt|=EBC2ASC_SBCSMAP037;  //sbcs is cp037           //~v7dmR~//~v7dyR~
      		UIUIBITON(mapopt,EBC2ASC_SBCSMAP037);  //sbcs is cp037 //~v7dyI~
        	UTRACEP("%s:cp037 by UCSCF_SBCSDEF\n",UTT);            //~v7dmI~
        }                                                          //~v7dmI~
    	else                                                       //~v7dmI~
...
#else                                                              //~v7diI~
        	if (!(Ppcfg->UCECflag & (UCECF_SBCSMAP))) //Not use internal map//~v7diI~
#endif                                                             //~v7diI~
    			mapopt|=EBC2ASC_EXT;  //external converter
            UTRACEP("%s:Not set EBC2ASC_CP290/SJISLATIN by flag=CP939 OFF\n",UTT);//~vbB9R~
        }
    }
    Sebcopta2e|=mapopt;
    Sebcopte2a|=mapopt;
...
UTRACEP("%s:UCECflag=0x%08x,Sebcopte2a=%x,a2e=%x\n",UTT,Ppcfg->UCECflag,Sebcopte2a,Sebcopta2e);//~v77BR~
...
UTRACEP("%s:Sebcopte2a=%x,Sebcopta2e=%x\n",UTT,Sebcopte2a,Sebcopta2e);//~v6bjI~//~vbB9R~//~v7dyR~
    return 0;                                                      //~v6bjI~
...
//    Ssubcharebcdbcslen=strlen(Spsubcharebcdbcs);                   //~v66CI~//~v69cR~
    UTRACEP("%s:subcharlocal dbcs=%02x%02x,sbcs=%02x\n",UTT,*Spsubcharlocaldbcs,*(Spsubcharlocaldbcs+1),Ssubcharlocalsbcs);//~v66CI~//~vbB9R~
    return 0;
...
//***************************
    ...
    UTRACEP("%s:converterlocal=%p\n",UTT,plocalconverter);         //~vbB9I~
	for (ii=0;ii<26;ii++)
    {
...
//      Gucvebc_ctype[ebcu]|=EBCTYPE_UPPER;                        //~v69cR~
    ...
    {                                                              //~vbBqI~
        ...
        UTRACED("UCECsbcsmape2a",pce2a,sizeof(Spcfg->UCECsbcsmape2a));//~vbBqR~
    }                                                              //~vbBqI~
    else                                                           //~v66RI~
    	pce2a=0;                                                   //~v66RI~
    ...
    {                                                              //~vbBqI~
        ...
        UTRACED("UCECsbcsmapa2e",pca2e,sizeof(Spcfg->UCECsbcsmapa2e));//~vbBqR~
    }                                                              //~vbBqI~
    else                                                           //~v66RI~
    	pca2e=0;                                                   //~v66RR~
    plocalconverter=Spcfg->UCECconverterlocal;                     //~v66RI~
...
//		if (!ucvebc3_b2mSbcs(optb2m,wkch,wkdbcs,1,wkch+1,sizeof(wkch)-1,&len))//for the case roundtrip(0x15->0x0a may prohibit by cfg option//~v66vR~//~v66RR~
        UTRACEP("%s:pce2a=%p,pce2a[%d]=%02x\n",UTT,pce2a,ii,(pce2a?pce2a[ii]:0));//~vbBqI~
...
//			rc2=ucvebc3_b2mSbcs(optb2m,wkch,wkdbcs,1,wkch+1,wkdbcs,sizeof(wkch)-1,&len);//for the case roundtrip(0x15->0x0a may prohibit by cfg option//~v68bI~//~v69cR~
        ...
        UTRACEP("%s:rc2=%d,wkch=%02x%02x,wkdbcs=%02x%02x,len=%d\n",UTT,rc2,wkch[0],wkch[1],*wkdbcs,*(wkdbcs+1),len);//~vbBqI~
        if (!rc2)                                                  //~v66RI~
        {                                                          //~v66UI~
...
//              pu2bmap[ii]=ii;				//u2b mapping          //~v69cI~//~v6BiR~
        ...
        UTRACEP("%s:b2u ucs=%x,ebc=%x\n",UTT,pb2umap[ii],ii);           //~v66qI~//~v66HR~//~v69cR~//~v6m3R~//~vbB9R~
...
//  	ptolower[ii]=ii;                                           //~v69cI~//~v6BiR~
    ...
    if (pca2e)                                                     //~v66RI~
        ...
        {                                                          //~v66RI~
            ...
            UTRACEP("%s:u2b by cfg a2e ucs=%02x--->ebc=%x\n",UTT,ii,Su2bmap[ii]);//~v66RI~//~vbB9R~
        }                                                          //~v66RI~
#endif                                                             //~v66RI~
	for (ii=0;ii<26;ii++)
    {
...
//		ucvebc3_u2b1(opt3,(WUCS)ii,&wkch,0/*outlen*/);             //~v66qR~//~v6BjR~
        ...
        UTRACEP("%s:fbtest u2b ucs=%x,ebc=%x\n",UTT,ii,(int)wkch);//~v66qI~//~vbB9R~
    }                                                              //~v66qI~
#endif                                                             //~v66qI~
...
//  ucvebc_getsubchar_dbcs(0);		//init dbcs repch              //~v66cI~//~v66CR~
    UTRACED("pucveh",pucveh,sizeof(UCVEBCH));                      //~v7dnR~
    UTRACEP("%s:exit\n",UTT);                                      //~v7dnI~
    return 0;
}//ucvebc3_setletter
//****************************************************************
...
//*****************
    UTRACEP("%s:entry Popt=0x%04x,Gucvebc_stat=0x%04x\n",UTT,Popt,Gucvebc_stat);//~v77BR~
    if (Gucvebc_stat & UCVEBCS_xcv)          //on xcv    process  to ucvext_mapinit//~va54I~//~v6bjI~
    {                                                              //~v6bjI~
        Spcfg=(PUCVEXTCFG)(*Pcfg);                                 //~v6bjR~
        UTRACEP("%s:for xcv cfg=%p\n",UTT,Spcfg);            //~v6bjI~//~vbB9R~
...
//***************************
    UTRACEP("%s:Popt=0x%08x,handle=%d,Sebcopte2a=0x%08x,repch=%02x\n",UTT,Popt,Phandle,Sebcopte2a,Prepch);//~vbB9R~//~v7dyR~
	pucveh=UCVEBC_GETPUCVEH(Phandle);                              //~v69cI~
    UTRACEP("%s:UCVEBCHflag=0x%04x\n",UTT,pucveh->UCVEHflag);      //~vbB9I~
    chebc=*Ppebc;
    opt=Sebcopte2a;                                                //~0428I~
    opt|=EBC2ASC_EXT_SETSUBCHRC;                                   //~v66NR~
    if (Popt & UCVEBC3O_CVCTL)                                     //~0428I~
        opt|=EBC2ASC_CTL;	                                       //~0428I~
    if (Popt & UCVEBC3O_CVCRLF)                                    //~v7dyI~
        opt|=EBC2ASC_CRLF;                                         //~v7dyI~
    UTRACEP("%s:opt=0x%08x\n",UTT,opt);                            //~v7dyI~
...
//      	wucs=(WUCS)Prepch;                                     //~v6BiI~//~v6BjR~
			...
			UTRACEP("%s:set Prepch to wucs=%04x by ucs by map is 0 for ebc=%04x\n",UTT,wucs,chebc);//~vbBkI~
            rc|=1;                                                 //~v66qI~
        }                                                          //~v66qI~
		UTRACEP("%s:@@@@map aftermapinit opt=%04x,ebc=%02x,ucs=%02x,rc=%d,UCVEHb2umap=%p\n",UTT,Popt,chebc,wucs,rc,pucveh->UCVEHb2umap);//~v66qR~//~vbB9R~
      }                                                            //~v66qI~
      else                                                         //~v66qI~
      {                                                            //~v66qI~
       if (Phandle)                                                //~v69cI~
...
//      UTRACEP("ebc3b2uSbcs1 opt=%x ebc1=%02x\n",Popt,*Ppebc);    //~v6m3R~
        UTRACEP("%s:pmap:Spcve2a=%p\n",UTT,pmap);                          //~vbB9I~//~v7dcR~
...
//            }                                                    //~v66DR~
    ...
    *Ppucs=wucs;
    UTRACEP("%s:rc=%x,in=%02x,outucs=%02x\n",UTT,rc,*Ppebc,wucs);//~v6m3R~//~vbB9R~//~v7dyR~
    return rc;
}//ucvebc3_b2uSbcs1
//*************************************************************************
...
//  	repch=Ssubcharsbcs;	//by ucs,may be changed by cfg parm    //~v66CR~//~v69cR~
        ...
        if (repch)                                                 //~vbBkI~
        	subchrc=UCVE3B2DDS1RC_SUBCHAR;// 0x100                 //~vbBkI~
    }                                                              //~vbBkI~
...
//    		len=uccvucs2utf((ULONG)wucs,Pdddata);                  //~0425I~//~v6BkR~
    ...
    if (rc2)                                                       //~0425R~
    {                                                              //~0425R~
UTRACEP("%s:repch=%04x,swb2f=%d,rc2=%d\n",UTT,repch,swb2f,rc2);    //~v7dCR~
    	if (swb2f)	//ebc->utf8                                    //~0425I~
//    		len=uccvucs2utf((ULONG)repch,Pdddata);                 //~0425I~//~v6BkR~
    ...
    if (Ppoutlen)                                                  //~0425I~
    	*Ppoutlen=len;                                             //~0425I~
    UTRACEP("%s:rc=%x,opt=%x,in=%02x,outdd=%02x,outdbcs=%02x,len=%d,Prepch=%08x,repch=%02x\n",UTT,rc,Popt,*Ppebc,*Pdddata,UWHICH(swb2f,0,*Pdddbcs),len,Prepch,repch);//~0425R~//~v660R~//~v69yR~//~vbB9R~//~vbBkR~//~v7dGR~
    UTRACED("ucvebc3_b2ddSbcs1 outdata",Pdddata,len);              //~v660I~
    UTRACED("ucvebc3_b2ddSbcs1 outdbcs",Pdddbcs,len);              //~v7dGI~//~v7dNR~
    return rc;
...
//  		rc2=ucvebc3_b2ddSbcs1(Popt,pci,repch,pdd,pcd,0/*out ucs*/,&outlen);//~0425R~//~0505R~//~v69cR~
      	...
      	if (rc2)                                                   //~vbBkR~
    	{
        	int swsubchar=0;                                       //~vbBkI~
			if (!(Popt & UCVEBC3O_B2F))                            //~0425R~
            {                                                      //~v69yI~
//            if (repch && *pdd==repch && *pcd==0                  //~v6hwI~//~vbBkR~
      	      if ((rc2 & UCVE3B2DDS1RC_SUBCHAR)// 0x100            //~vbBkI~
...
//  USHORT usucs,ussjis;                                           //~v66CR~
    ...
    PUCVEBCH pucveh;                                               //~v69cI~
//*********************
UTRACEP("%s:b2u handle=%d,Sebcopte2a=0x%04x,dbcsbyte=%02x%02x,Prepch=%04x\n",UTT,Phandle,Sebcopte2a,*Ppebc,*(Ppebc+1),Prepch);//~v77BR~//~v7dCR~
UTRACEP("%s:CONVERTER_DBCS=%p\n",UTT,Spcve2a[CONVERTER_DBCS]);     //~v7dcR~
    if (Phandle)                                                   //~v69cI~
    {                                                              //~v69cI~
    	pucveh=UCVEBC_GETPUCVEH(Phandle);                          //~v69cI~
	    optext=EBC2ASC_STAT_SO;                                    //~v69cI~
	    optext|=EBC2ASC_EXT_SETSUBCHRC;                            //~v69cI~
//		pconverter=pucveh->UCVEHpconverter[CONVERTER_SBCS];        //~v69cR~//~v7dGR~
  		pconverter=pucveh->UCVEHpconverter[CONVERTER_DBCS];        //~v7dGI~
        if (!pconverter)                                           //~v7dGI~
			pconverter=pucveh->UCVEHpconverter[CONVERTER_SBCS];    //~v7dGI~
        rc2=ucvext_icuebc2ucs1(optext,pconverter,Ppebc,2,&usucs);  //~v69cI~
    }                                                              //~v69cI~
    else                                                           //~v69cI~
//  if (Sebcopte2a & EBC2ASC_EXT)                                  //~v7dcR~
    if (Sebcopte2a & EBC2ASC_EXT && Spcve2a[CONVERTER_DBCS])  // except EBC2ASC_EXT by set_internal_CP037(width no dbcscharset)//~v7dcI~
    {
...
//*********************
UTRACEP("%s:opt=%08x,handle=%d,repch=%04x\n",UTT,Popt,Phandle,Prepch);//~v7dCR~
	swb2f=(Popt & UCVEBC3O_B2F);                                   //~0425R~
   	repch=(UWUCS)Prepch;                                           //~v7dCI~
...
//    	repch=(UWUCS)Prepch;                                       //~v6BjI~//~v7dCR~
        if (!repch)
        {                                                          //~vbBkI~
...
//      	repch=Ssubchardbcs;	//ucs                              //~v66CI~//~v69cR~
            ...
            if (repch)                                             //~vbBkI~
            	rc|=UCVE3B2DDD1RC_SUBCHAR;// 0x100                 //~vbBkI~
        }                                                          //~vbBkI~
UTRACEP("%s:repch=%04x,swb2f=%d,rc2=%d\n",UTT,repch,swb2f,rc2);    //~v7dCR~
		if (swb2f)                                                 //~0425I~
        {                                                          //~0425I~
...
//            }                                                      //~0425R~//~v66DR~
    ...
    if (Ppoutlen)                                                  //~0425I~
    	*Ppoutlen=len;                                             //~0425R~
UTRACEP("%s:rc=%d dbcsbyte=%02x%02x,ucs=%x,outdd=%02x-%02x,outlen=%d,Prepch=%04x,repch=%04x\n",UTT,rc,*Ppebc,*(Ppebc+1),wucs,//~0425R~//~vbB9R~//~v7dCR~
			*Pdddata,UWHICH(swb2f,*(Pdddata+1),*Pdddbcs),len,Prepch,repch);     //~0425R~//~v7dCR~
    return rc;
}//ucvebc3_b2ddDbcs1
//*************************************************************************
...
UTRACEP("%s:b2dd DBCSstr dbcsbyte=%02x%02x\n",UTT,*pci,*(pci+1));  //~vbB9R~
        if (swb2f)                                                 //~0505I~
        {                                                          //~0505I~
...
//  			rc2=ucvebc3_b2ddSbcs(Popt,pcisbcs,len,pdd,pcd,resleno,&ddlen);//~v69cR~
        		...
        		swso=1;
//        		if (rc2==1)                                        //~vbBkR~
          		if (rc2 & 1)   //include rep by SUBCH_SBCS         //~vbBkI~
            		rc|=1; //errrep
                pdd+=ddlen;
                resleno-=ddlen;
...
int ucvebc3_b2dd(int Popt,int Phandle,UCHAR *Pebc,int Plen,UCHAR *Pdddata,UCHAR *Pdddbcs,int Poutbuffsz,int *Ppddlen)//~v69cI~
{
	int rc;
//********************
    UTRACEP("%s:opt=%x,handle=%d\n",UTT,Popt,Phandle);                      //~v69cI~//~v69mR~//~vbB9R~
...
{                                                                  //~v66HI~
    ...
    PUCVEBCH pucveh;                                               //~v69cI~
//*************                                                    //~v66HI~
    UTRACEP("%s:handle=%d,ucs=%04x\n",UTT,Phandle,Pucs);  //~v7dGI~//~v7dNR~
...
//          	ebc=Su2bnonasciimapebc[ii];                        //~v66HR~//~v69cR~
    ...
    UTRACEP("%s:ucs=%04x,ebc=%02x\n",UTT,Pucs,ebc);       //~v66HI~//~v6biR~//~vbB9R~
    return ebc;                                                    //~v66HI~
...
//  int wcwidth;                                                   //~v66mI~//~v66qR~
	PUCVEBCH pucveh;                                               //~v69cI~
//***************************
UTRACEP("%s:opt=%x,handle=%d,inp ucs=%x\n",UTT,Popt,Phandle,Pucs);//~v69cR~//~vbB9R~
	pucveh=UCVEBC_GETPUCVEH(Phandle);                              //~v69cI~
    if ( Pucs<' '
    &&  !(Popt & UCVEBC3O_CVCTL)
    )
...
//  UCHAR chmb,chebc;                                              //~v6m1R~
    ...
    PUCVEBCH pucveh;                                               //~v69cI~
//***************************
    UTRACEP("%s:opt=0x%08x,handle=%d,mbch=0x%2x\n",UTT,Popt,Phandle,Pmbch);//~v7dnI~
    ebclen=0;                                                      //~v7dCI~
	pucveh=UCVEBC_GETPUCVEH(Phandle);                              //~v69cI~
...
//        }                                                          //~v66qI~//~v66DR~
    ...
    if (rc2)
        ebc=-1;
UTRACEP("%s:opt=%x,mb=%x,ebc=%x\n",UTT,Popt,chmb,ebc);  //~v66qR~  //~vbB9R~
    return ebc;
...
UTRACEP("%s:opt=%x,rc=%d,errrep=%x\n",UTT,Popt,rc,repch);    //~v660R~//~vbB9R~
    return rc;
}//ucvebc3_m2b1
//****************************************************************
...
//***************************
UTRACEP("%s:opt=0x%04x,handle=%d,len=%d\n",UTT,Popt,Phandle,Plen); //~v77BR~
...
{                                                                  //~v687I~
    ...
    UCHAR wkebc[4];                                                //~v687I~
//***************************                                      //~v687I~
UTRACEP("%s:Popt=%08x,handle=%d\n",UTT,Popt,Phandle);              //~v7dyR~
UTRACED("ucvebc3 f2b inp data",Pdata,Plen);                        //~v7dyI~
//  swdbcsenv=UDBCSCHK_ISDBCSCV();                                 //~v687I~//~v69cR~
    swdbcsenv=UCVEBCH_HANDLE_ISDBCSCV(Phandle);                    //~v69cI~
    int crlfsw=Popt & UCVEBC3O_CVCRLF;                             //~v7dyI~
    swdbcs=0;                                                      //~v687I~
...
//  opt=0;	//repch                                                //~v687I~//~v6b9R~
    ...
    {                                                              //~v687I~
        ...
        UTRACEP("%s:called utfgetwc,ucs=%04x,chklen=%d,rc2=%d\n",UTT,wucs,chklen,rc2);//~v7dNR~
        if (!rc2)                                                  //~v687I~
        {                                                          //~v687I~
            opt2=Popt;                                             //~v687I~
...
//  		rc2=ucvebc3_u2b1(opt2,Phandle,(WUCS)wucs,wkebc,&outlen);//~v69cI~//~v6BjR~
        ...
        if (rc2)                                                   //~v687I~
			outlen=1;                                              //~v687I~
      if (!rc2 && crlfsw  && wucs==0x0a)                           //~v7dyR~
      {                                                            //~v7dyI~
        *pcdo++=0;                                                 //~v7dyI~
        *pco=EBC_NL;	//0x15                                     //~v7dyI~
        outlen=1;                                                  //~v7dyI~
        pco+=outlen;                                               //~v7dyI~
        resleno-=outlen;                                           //~v7dyI~
      }                                                            //~v7dyI~
      else                                                         //~v7dyI~
	  if (swdbcsenv)                                               //~v687I~
      {                                                            //~v687I~
...
UTRACEP("%s:rc=%d,opt=%x\n",UTT,rc,Popt);                     //~v687I~//~vbB9R~
    return rc;                                                     //~v687I~
}//ucvebc3_f2b                                                     //~v687I~
//**************************************************************** //~0423I~
...
//  repchdbcs=UTF_GETUCSBESTR(Spsubcharebcdbcs);                   //~v66CI~//~v69cR~
    repchdbcs=UTF_GETUCSBESTR(UCVEBCH_subcharebcdbcs(Phandle));    //~v69cI~
	UTRACEP("%s:opt=%08x,repch=%02x,repchdbcs=%04x,swsetf2lerr=%d,swdbcsenv=%d\n",UTT,Popt,repch,repchdbcs,swsetf2lerr,swdbcsenv);//~v7dGR~
    opt=0;	//repch                                                //~0423R~
    setsoopt0=Sebcopta2e;                                          //~v689I~
    if (Popt & (UCVEBC3O_SOSIINS|UCVEBC3O_SOSIREP))	//cmd option specified//~v689I~
    {                                                              //~v689I~
...
//  			setsoopt=Sebcopta2e;                               //~v688I~//~v689R~
            ...
            pcdoData=pcdo;	//reset when err                       //~v7dNI~
...
//      	if (chsz>1)                                            //~0423I~//~v66sR~
            ...
            {                                                      //~v69nI~
            	...
            	if (swsetf2lerr)                                   //~v69nI~
                {                                                  //~v7dNI~
//        			memset(pcdo,UDBCSCHK_F2LERR,(UINT)outlen);     //~v69nR~//~v7dNR~
					if (pcdoData)                                  //~v7dNI~
                    {                                              //~v7dNI~
//            			memset(pcdoData,DEFAULT_UNPATTR,(UINT)outlen);// UDBCSCHK_F2LERR is not rep to DISPLINEATTR on tabdisplay for ebc file//~v7dNR~//~vbBqR~
              			memset(pcdoData,UDBCSCHK_F2LERR,(UINT)outlen);// UDBCSCHK_F2LERR is not rep to DISPLINEATTR on tabdisplay for ebc file//~vbBqI~
                        UTRACED("pcdoData",pcdoData,outlen);       //~v7dNI~
                    }                                              //~v7dNI~
                }                                                  //~v7dNI~
            }                                                      //~v69nI~
            else                                                   //~0423I~
...
//      		*pco=repch;                                        //~0423I~//~v6BiR~
        		*pco=(UCHAR)repch;                                 //~v6BiI~
//        		*pcdo=UDBCSCHK_F2LERR;    //set outlen=1 later     //~v69nI~//~v7dNR~
              if (swsetf2lerr)                                     //~v7dNI~
                if (pcdoData)                                      //~v7dNI~
                {                                                  //~v7dNI~
//          		*pcdoData=DEFAULT_UNPATTR;    //UDBCSCHK_F2LERR is not rep to DISPLINEATTR on tabdisplay for ebc file//~v7dNR~//~vbBqR~
            		*pcdoData=UDBCSCHK_F2LERR;    //UDBCSCHK_F2LERR is not rep to DISPLINEATTR on tabdisplay for ebc file//~vbBqI~
                    UTRACED("pcdoData",pcdoData,outlen);           //~v7dNI~
                }                                                  //~v7dNI~
            }                                                      //~v69nI~
...
//      	*pco=repch;                                            //~v66mI~//~v6BiR~
        	*pco=(UCHAR)repch;                                     //~v6BiI~
            ...
            if (swsetf2lerr)                                       //~v69nI~
//        		*pcdo++=UDBCSCHK_F2LERR;                           //~v69nR~//~v7dNI~
//      		*pcdo++=DEFAULT_UNPATTR;    //UDBCSCHK_F2LERR is not rep to DISPLINEATTR on tabdisplay for ebc file//~v7dNR~//~vbBqR~
          		*pcdo++=UDBCSCHK_F2LERR;                           //~vbBqI~
            else                                                   //~v69nI~
	        	*pcdo++=0;                                         //~v69nI~
...
int ucvebc3_b2mSbcs(int Popt,int Phandle,UCHAR *Pdata,UCHAR *Pdbcs,int Pinplen,UCHAR *Poutbuff,UCHAR *Poutdbcs,int Pbuffsz,int *Ppmblen)//~v69cR~
{
    ...
    PUCVEBCH pucveh;                                               //~v69cI~
//***************************
    UTRACEP("%s:opt=%x,handle=%d\n",UTT,Popt,Phandle);                              //~0423I~//~v69cR~//~vbB9R~
...
//  	mb2=UCVEBCH_B2A(Phandle,ch);                               //~v69cR~//~v6BkR~
        ...
        UTRACEP("%s:from B2AMAP ebc=%x,mb=%x\n",UTT,ch,mb2);      //~v6hwI~//~vbB9R~
       if (mb2>>8)                                                 //~v66UI~
       {                                                           //~v66UI~
...
//      rc2=ucvebc2asc(opt,pci,0/*pdbcs*/,pco,1,repch,&mblen);     //~v69cR~
        ...
        UTRACEP("%s:b2msbcs ebc=%02x,mb=%02x%02x,rc=%d,mblen=%d\n",UTT,ch,*pco,mblen>1 ? *(pco+1):0,rc2,mblen);//~v66XR~//~v6m3R~//~vbB9R~
        if (!rc2)
        {
...
//*************                                                    //~v66cI~
    ...
    {                                                              //~v66cI~
        ...
        UTRACEP("%s:repch dbcs=%x,ucs=%x,ebc=%x\n",UTT,Srepchdbcs[0],Srepchdbcs[1],Srepchdbcs[2]);//~v66cI~//~vbB9R~
    }                                                              //~v66cI~
...
//*****************                                                //~v6bjR~
    ...
    if (Pebcopt & EBC2ASC_CRLF) //default ctl char conv            //~v7dyI~
        opt|=UCVEBC3O_CVCRLF;                                      //~v7dyI~
    ...
    UTRACEP("%s:Pebcopt=%08x,return opt=%08x\n",UTT,Pebcopt,opt);  //~v7dyI~
    return opt;                                                    //~v6bjR~
}//getebc3opt                                                      //~v6bjR~
//**************************************************************** //~v6bjR~
//ucvebc3_b2f                                                      //~v6bjR~
