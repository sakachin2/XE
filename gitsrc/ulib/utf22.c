...
//vbzV:240422 SCM(Mc) by green attr                                //~v7zVI~
//v7bB:240420 special for zwj(u-200d/zwnj(u-200c) of width=0       //~v7bBI~
//v7bq:240414 : expand range of vbzu:240302 add test option to notrace while mapinit for performance. (del prev option /Yn/Nn to winNT option)//~v7bqI~
//vbzM:240408 (WXE) support hardcopy function                      //~vbzMI~
//v7bf:240328 (LNX)wcwidth() depends locale, like v7bb it should be checked font glyph width. But wcwidth() is required becaulse pango issue warning for wcwidth()<0.//~v7bfI~
//v7bb:240326 It should not use mblen it depends locale, for unicode file it have to depend font.//~v7bbI~
//v7b3:240321 obsoluted function to adjust mapinit by ini file. update warning msg//~v7b3I~
//v7ay:240319 (LNX) it will be better to fololow wcwidth() api under LNXCON for uviom//~v7ayI~
//v7aw:240318 (LNX)chk glyph width for #Lo(Other Letter):wcwidth=0 //~v7awI~
//v7au:240317 apply wcwidth() to ambiguous but format ignoring 2 of ambiguous.//~v7auI~
//v7ap:240314 unprintable option of /G  have to apply to ambiguous by lang.//~vbzzI~
//vbzz:240311 arrange vbz3,vbzp:/Yv and vbzy:YL to /Gv[L]          //~vbzzI~
//v7ak:240309 use native wctomb for performance(less unprintable than windows api)//~v7akI~
//v7ah:240306 (LNXCON)061c is defined as ambiguous but category Cf(Format) and use 2 cell, change width 2(by ambiguous) to 1//~v7ahI~
...
//v7ac:240229 (WINCON)on Windows Console,Font Mincho, confused ambiguous char display when /Yv; try write ambiguous char 1 by 1//~v7a7I~//~v7bqR~
...
//v7a3:240223 WinCons:ambiguous char of wcwidth=1 by /Yv  misses override next char on CPU8 file(e.g See v7a0).//~v7a3I~
//            Use F1C1(NarrowDBCS) after 2byte mbcs by wctomb(this is same as WideCharacterToMultiByte).//~v7a3I~
...
//*************************                                        //~vbz5I~
    ...
    if (IS_UTF16_HIGH(Pucs)||IS_UTF16_LOW(Pucs))  //UTF16 pair     //~vbz5R~
    	return width;                                              //~vbz5I~
    UTRACEP("%s:entry ucs=0x%04x,utf8=(  %s  ),wcwidth=%d,Pmkwidth=%d,flag=0x%04x\n",UTT,Pucs,UCV_U2F1(Pucs),wcwidth((wchar_t)Pucs),width,flag);//~vbz5R~//~v7a7R~
    ...
    {                                                              //~vbz5I~
        ...
        UTRACEP("%s:callxxe ucs=x%04x,utf8=(  %s  ),width=%d->%d,wcwidth()=%d,flag=0x%04x,updated=%s\n",UTT,Pucs,UCV_U2F1(Pucs),Pmkwidth,width,wcwidth((wchar_t)Pucs),flag,Pmkwidth==width?"N":"Y");//~vbz5R~//~v7a7R~
    }                                                              //~vbz5I~
    return width;                                                  //~vbz5I~
...
{                                                                  //~v7awM~
    ...
    {                                                              //~v7awI~
    	...
    	UTRACEP("%s:set wwAmbig(>0) ucs=0x%04x\n",UTT,Pucs);       //~v7awI~
    }                                                              //~v7awI~
    else                                                           //~v7awI~
    {                                                              //~v7awI~
#ifdef XXE                                                         //~v7awM~
...
//*******************************************************          //~v640M~
//#if defined(LNXCON) && ! defined(ARM)                            //~v7ayR~
#ifndef TTT                                                        //~v7ayR~
//*******************************************************************//~v7ayI~
int utfwwapichk(int Popt,ULONG Pucs,int Pmkwidth,int *Ppflag/*masked*/,int *Ppwcwidth)//~v640M~//~vbz3R~//~v7ayI~
{                                                                  //~v640M~//~v7ayI~
	int wcw,flag,width;                                            //~v640M~//~v7ayI~
#ifndef XXX                                                        //~v6WvI~//~v7ayI~
    int rc2;                                                       //~v6WvI~//~v7ayI~
#endif                                                             //~v6WvI~//~v7ayI~
//    int swAmbigChange;                                             //~v79EI~//~v7ayR~
    int wwAmbig;                                                   //~vbzzR~//~v7ayI~
//*************************                                        //~v640M~//~v7ayI~
    if (Pucs==(ULONG)GutfTestUcs)                                  //~v79FI~//~v7ayI~
    {                                                              //~v79FI~//~v7ayI~
        UTRACEP("%s:Debug trap by GutfTestUcs ucs=%04x\n",UTT,Pucs);	//@@@@test//~v79FI~//~v7ayI~
    }                                                              //~v79FI~//~v7ayI~
	flag=*Ppflag;                                                  //~v640M~//~v7ayI~
    width=Pmkwidth;                                                //~v640M~//~v7ayI~
...
//    )                                                              //~v7ayI~//~v7bfR~
    ...
    {                                                              //~v7bfI~
        ...
        if ((Popt & UTFWWO_MK_WCWIDTH)                             //~v7bfR~
		&& (width==2)	//dbcs by mk_wcwidth                       //~v7bfR~
        )                                                          //~v7bfI~
        	;      //dbcs                                          //~v7bfI~
        else                                                       //~v7bfI~
        {                                                          //~v7bfI~
...
//  if (wcw>0 && wwAmbig>=0)    //and ambig length by -G option not to adjust//~vbzzR~//~v7ayR~
    ...
    {                                                              //~v640M~//~v7ayI~
#ifdef ARM	//adjsbcs set Pmkwcwidth to reduce sbcs                //~v70iM~//~v7ayI~
		if (wcw==2 || Pmkwidth==2)	//by Ucs.java or adjsbcs       //~v70iI~//~v7ayI~
        	width=2;                                               //~v70iR~//~v7ayI~
        else                                                       //~v70iI~//~v7ayI~
        	width=1;                                               //~v70iI~//~v7ayI~
#else  //!ARM                                                      //~v70iI~//~v7ayI~
        if (flag & UTFWWF_F1C1)      //Narrow DBCS                 //~v640M~//~v7ayI~
        {                                                          //~v640M~//~v7ayI~
            if (wcw==1)  //wcwidth=1                               //~v640M~//~v7ayI~
            {                                                      //~v640I~//~v7ayI~
            	width=1;			//same as sbcs when wcwidth=1  //~v640M~//~v7ayI~
        		flag &= ~UTFWWF_F1C1;      //usual SBCS            //~v640I~//~v7ayI~
            }                                                      //~v640I~//~v7ayI~
        }                                                          //~v640M~//~v7ayI~
        else                                                       //~v640M~//~v7ayI~
        if (!(flag & UTFWWF_F2C1))   //no tbl defintion            //~v640M~//~v7ayI~
        {                                                          //~v640M~//~v7ayI~
//        	swAmbigChange=0;                                       //~v79EI~//~v7ayR~
//  	    if ((flag & UTFWWO_MK_AMBIGUOUS)                       //~v79FR~//~v7ayR~
//          &&  wcw==1                                             //~v79EI~//~v7ayR~
//          &&  Pmkwidth==2)                                       //~v79EI~//~v7ayR~
//          {                                                      //~v79EI~//~v7ayR~
//#ifdef XXE                                                         //~v79EI~//~v7ayR~
//#else                                                              //~v79EI~//~v7ayR~
//                if (UTF_AMBIG2CELLMODE())   //1 cell mode for ambiguous//~v79EI~//~vbzzR~//~v7ayI~
//                    width=2;                                       //~v79EI~//~vbzzR~//~v7ayI~
//                else                                               //~v79EI~//~vbzzR~//~v7ayI~
//                {                                                  //~v7agI~//~vbzzR~//~v7ayI~
//                  if (wwAmbig>0)                                   //~vbzzR~//~v7awR~//~v7ayI~
//                    width=wwAmbig; //width by -G option            //~vbzzR~//~v7awR~//~v7ayI~
//                  else                                             //~vbzzR~//~v7awR~//~v7ayI~
//                  {                                                //~vbzzR~//~v7awR~//~v7ayI~
//#ifdef XXE                                                         //~vbzzR~//~v7awR~//~v7ayI~
//                    UTRACEP("%s:wcw=1 xxe call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~//~v7ayI~
//                    width=xxe_GetCursorWidth(0,Pucs);              //~vbzzR~//~v7awR~//~v7ayI~
//#else                                                              //~vbzzR~//~v7awR~//~v7ayI~
//                    UTRACEP("%s:wcw=1 lnxcons call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~//~v7ayI~
//                    width=uvioGetCursorWidth(0,Pucs);              //~v7agI~//~v7awR~//~v7ayI~
//#endif                                                             //~vbzzR~//~v7awR~//~v7ayI~
//                 }                                                 //~vbzzR~//~v7awR~//~v7ayI~
//  			 	width=getGlyphWidth(Pucs,wwAmbig);             //~v7awI~//~v7ayR~
//                if (width<0)	//failed                           //~v7agI~//~v7ayR~
//              	width=1;                                       //~v79EI~//~v7ayR~
//                    if (width==2)                                  //~v7agI~//~v7ayR~
//                    {                                            //~v7ayR~
//                        flag|=UTFWWF_F2C1;      //    0x80   //WideSBCS; font width=2,wcwidth==1//~v7agI~//~v7ayR~
//                        UTRACEP("%s:set F2C1 ucs=%04x,Pmkwidth=%d,wcwidth=%d,ambigWidth=%d,width=%d\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,width);//~v7ayR~
//                    }                                            //~v7ayR~
//                }                                                  //~v7agI~//~vbzzR~//~v7ayI~
//#endif                                                             //~v79EI~//~v7ayR~
//	        	swAmbigChange=1;                                   //~v79EI~//~v7ayR~
//            }                                                      //~v79EI~//~v7ayR~
//            else                                                   //~v7agI~//~v7ayR~
//  	    if ((flag & UTFWWO_MK_AMBIGUOUS)                       //~v7agI~//~v7ayR~
//          &&  wcw==2                                             //~v7agI~//~v7ayR~
//          &&  Pmkwidth==2)                                       //~v7agI~//~v7ayR~
//            {                                                      //~v7agI~//~v7ayR~
//            //* wcw=2                                            //~v7ayR~
//                if (UTF_AMBIG2CELLMODE())   //1 cell mode for ambiguous//~v7agI~//~vbzzR~//~v7ayI~
//                    width=2;                                       //~v7agI~//~vbzzR~//~v7ayI~
//                else                                               //~v7agI~//~vbzzR~//~v7ayI~
//                {                                                  //~v7agI~//~vbzzR~//~v7ayI~
//		            int gw=uvioGetCursorWidth(0,Pucs);             //~v7agI~//~vbzzR~//~v7ayI~
//		            int gw;                                        //~vbzzR~//~v7ayR~
//                    if (wwAmbig>0)                                 //~vbzzR~//~v7awR~//~v7ayI~
//                        gw=wwAmbig; //width by -G option           //~vbzzR~//~v7awR~//~v7ayI~
//                    else                                           //~vbzzR~//~v7awR~//~v7ayI~
//                    {                                              //~vbzzR~//~v7awR~//~v7ayI~
//#ifdef XXE                                                         //~vbzzR~//~v7awR~//~v7ayI~
//                        UTRACEP("%s:wcw=2 xxe call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~//~v7ayI~
//                        gw=xxe_GetCursorWidth(0,Pucs);             //~vbzzR~//~v7awR~//~v7ayI~
//#else                                                              //~vbzzR~//~v7awR~//~v7ayI~
//                        UTRACEP("%s:wcw=2 lnxcons call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~//~v7ayI~
//                        gw=uvioGetCursorWidth(0,Pucs);             //~vbzzR~//~v7awR~//~v7ayI~
//#endif                                                             //~vbzzR~//~v7awR~//~v7ayI~
//                    }                                              //~vbzzR~//~v7awR~//~v7ayI~
//                  gw=getGlyphWidth(Pucs,wwAmbig);                //~v7awI~//~v7ayR~
//                	if (gw==1)	//wcwidth==2 & glyphwidth=1        //~v7agI~//~v7ayR~
//                  {                                              //~v7agI~//~v7ayR~
//  	            	flag|=UTFWWF_F1C1;      //0x40 narrow dbcs //~v7agI~//~v7ayR~
//      				UTRACEP("%s:set F1C1 ucs=%04x,Pmkwidth=%d,wcwidth=%d,ambigWidth=%d,width=%d\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,width);//~v7ayR~
//                  }                                              //~v7agI~//~v7ayR~
//                }                                                  //~v7agI~//~vbzzR~//~v7ayI~
//          	swAmbigChange=1;                                   //~v7agI~//~v7ayR~
//          }                                                      //~v7agI~//~v7ayR~
//          if (swAmbigChange)                                     //~v79EI~//~v7ayR~
//          {                                                      //~v79EI~//~v7ayR~
//              UTRACEP("%s:ambiguous ucs=0x%04x(%s),wcwidth()=%d,Pmkwidth=%d,set width=%d\n",//~v79ER~//~v7ahR~//~v7ayR~
//  					UTT,Pucs,UCV_U2F1(Pucs),wcw,Pmkwidth,width);//~v79ER~//~v7a7R~//~v7ayR~
//          }                                                      //~v79EI~//~v7ayR~
//          else                                                   //~v79EI~//~v79FR~//~v7ayI~
//  	    if ((Popt & UTFWWO_MK_AMBIGUOUS) 	//ARM4             //~v6V7I~//~v79FR~//~v7ayI~
//    	    if ((flag & UTFWWO_MK_AMBIGUOUS)                       //~v79FR~//~v7ayI~
//          &&  wcw==0                      !! NO path by wcw=0    //~v79FR~//~v7ayI~
//          &&  Pmkwidth!=wcw)                                     //~v6V7I~//~v79FR~//~v7ayI~
//          {                                                      //~v6V7I~//~v79FR~//~v7ayI~
//          	width=wcw;                                         //~v6V7I~//~v79FR~//~v7ayI~
//              UTRACEP("%s:ambiguous wcw=0 wcwidth()=%d,utfwcw=%d,return width=%d\n",UTT,wcw,Pmkwidth,width);//~v6V7I~//~v79FR~//~v7ayI~
//          }                                                      //~v6V7I~//~v79FR~//~v7ayI~
//          else                                                   //~v6V7I~//~v7ayR~
//        {//swAmbigChange=0 , wcw=0 & wwAmbig>=0                  //~v7awR~//~v7ayR~
//            if (Popt & UTFWWO_FORMAT)                              //~v7awI~//~v7ayR~
//            {                                                      //~v7awI~//~v7ayR~
//                width=wcw;                                         //~v7awI~//~v7ayR~
//                int gw=getGlyphWidth(Pucs,wwAmbig);                //~v7awI~//~v7ayR~
//                if (gw>=0) //format may width0                     //~v7awR~//~v7ayR~
//                    width=gw;                                      //~v7awI~//~v7ayR~
//                UTRACEP("%s:swAmbigChage=0(wcw>0 & wwAmbig=%d) FORMAT ucs=0x%04x wcwidth()=%d ,mk_wcwidth()=%d,width=%d\n",UTT,wwAmbig,wcw,Pucs,wcw,Pmkwidth,width);//~v7awI~//~v7ayR~
//            }                                                      //~v7awI~//~v7ayR~
//            else                                                   //~v7awI~//~v7ayR~
//            if ((flag & UTFWWF_COMB))   //combining definition mkWidth=0//~v7awI~//~v7ayR~
//            {                                                      //~v7awI~//~v7ayR~
//                UTRACEP("%s:unmatch wcwidth()=%d,PmkWidth=%d,return width=%d,ucs=0x%04x\n",UTT,wcw,Pmkwidth,width,Pucs);//~v7awI~//~v7ayR~
//            }                                                      //~v7awI~//~v7ayR~
//            else                                                   //~v7awI~//~v7ayR~
//            {                                                      //~v7awI~//~v7ayR~
//                width=wcw;                                         //~v7awI~//~v7ayR~
...
//                if (gw>0)                                          //~v7awI~//~v7ayR~
//                    width=gw;                                      //~v7awI~//~v7ayR~
//                UTRACEP("%s:wcw>0 & wwAmbig>=0 but  Not Ambig change, Not Format,COMB ucs=0x%04x Pmkwidth=%d,wcwidth=%d,wwAmbig=%d,width=%d\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,width);//~v7awI~//~v7ayR~
//            }                                                      //~v7awI~//~v7ayR~
//            if ((Popt & UTFWWO_MK_WCWIDTH)  //ARM4                 //~v6V7I~//~v7awR~//~v7ayI~
//            &&  Pmkwidth!=wcw)                                     //~v6V7I~//~v7awR~//~v7ayI~
//            {                                                      //~v6V7I~//~v7awR~//~v7ayI~
//////            if (!(Popt & UTFWWO_MK_COMB))   //combining definition//~v6W9R~//~v6WmR~//~v7awR~//~v7ayI~
////              if (!(flag & UTFWWF_COMB))   //combining definition  //~v6WmR~//~v7awR~//~v7ayI~
////              {                                                    //~v6WiI~//~v7awR~//~v7ayI~
////                width=wcw;                                         //~v6V7I~//~v7awR~//~v7ayI~
////                UTRACEP("%s:unmatch wcwidth()=%d,mk_wcwidth()=%d,return width=%d,ucs=0x%04x\n",UTT,wcw,Pmkwidth,width,Pucs);//~v6V7I~//~v7auR~//~v7awR~//~v7ayI~
////              }                                                    //~v6WiI~//~v7awR~//~v7ayI~
//            }                                                      //~v6V7I~//~v7awR~//~v7ayI~
//            else                                                   //~v6VaI~//~v7awR~//~v7ayI~
//            if (Pmkwidth==1 && wcw==2)  //sbcs but wcwidth=2       //~v640M~//~v7awR~//~v7ayI~
//            {                                                      //~v640M~//~v7awR~//~v7ayI~
////              if (!(flag & UTFWWF_SBCSF))   //no tbl defintion     //~v6WiI~//~v7awR~//~v7ayI~
////              {                                                    //~v6WiI~//~v7awR~//~v7ayI~
//                flag|=UTFWWF_F1C1;      //Narrow DBCS when wcwcidth=2//~v640M~//~v7awR~//~v7ayI~
//                width=2;                //occupy 2 column          //~v640M~//~v7awR~//~v7ayI~
//                UTRACEP("%s:%04x change width 1 to 2 by wcwidth()\n",UTT,Pucs);//~v6WiI~//~v70hR~//~v7auR~//~v7awR~//~v7ayI~
////              }                                                    //~v6WiI~//~v7awR~//~v7ayI~
//            }                                                      //~v640M~//~v7awR~//~v7ayI~
////            else                                                 //~v640R~//~v7awR~//~v7ayI~
////            if (Pmkwidth==2 && wcw==1)  //dbcd but wcwidth=1     //~v640R~//~v7awR~//~v7ayI~
////            {                                                    //~v640R~//~v7awR~//~v7ayI~
////                width=1;                //occupy 1 column        //~v640R~//~v7awR~//~v7ayI~
////            }                                                    //~v640R~//~v7awR~//~v7ayI~
//            else                                                   //~v7auI~//~v7awR~//~v7ayI~
//            if (Pmkwidth==2 && wcw==1)  //under confition Pmkwidth!=wcw//~v7auI~//~v7awR~//~v7ayI~
//            {                                                      //~v7auI~//~v7awR~//~v7ayI~
//                width=wcw;                                         //~v7auI~//~v7awR~//~v7ayI~
...
//#ifdef SSS                                                       //~v7bVR~
#ifndef SSS                                                        //~v7bVI~
                width=wcw;                                         //~v7ayR~
                UTRACEP("%s:wcw>0 and FORMAT set wcw ucs=0x%04x wcwidth()=%d ,mk_wcwidth()=%d,width=%d\n",UTT,Pucs,wcw,Pmkwidth,width);//~v7bVI~
#else                                                              //~v7bVI~
                width=max(wcw,gw);	//apply glyph width            //~v7bVI~
            ...
            {                                                      //~v7ayI~
                if (width==2)                                      //~v7ayI~
                {                                                  //~v7ayI~
                    flag|=UTFWWF_F2C1;      //    0x80   //WideSBCS; font width=2,wcwidth==1//~v7ayI~
                    UTRACEP("%s:set F2C1 ucs=%04x,Pmkwidth=%d,wcwidth=%d,ambigWidth=%d,width=%d\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,width);//~v7ayI~
                }                                                  //~v7ayI~
            }                                                      //~v7ayI~
            else //wcw==2	                                       //~v7ayI~
            {                                                      //~v7ayI~
                if (width==1)                                      //~v7ayI~
                {                                                  //~v7ayI~
		           	flag|=UTFWWF_F1C1;      //0x40 narrow dbcs     //~v7ayI~
        			UTRACEP("%s:set F1C1 ucs=%04x,Pmkwidth=%d,wcwidth=%d,ambigWidth=%d,width=%d\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,width);//~v7ayI~
                }                                                  //~v7ayI~
            }                                                      //~v7ayI~
        }                                                          //~v640M~//~v7ayI~
        else //F2C1 Wide sbcs                                      //~v6VaI~//~v7ayI~
        {                                                          //~v6VaI~//~v7ayI~
            if (wcw==2 && Pmkwidth==2)	//dbcs                     //~v6VaI~//~v7ayI~
                flag&=~UTFWWF_F2C1;      //reset WIDE sbcs         //~v6VaR~//~v7ayI~
        }                                                          //~v6VaI~//~v7ayI~
#endif //!ARM                                                      //~v70iI~//~v7ayI~
    }                                                              //~v640M~//~v7ayI~
    else                                                           //~v640I~//~v7ayI~
//  if (wcw)                                                       //~v650I~//~vbzzR~//~v7ayI~
//* wcw<=0 or wwAmbig<0                                            //~vbzzR~//~v7ayI~
//  if (wcw||wwAmbig)  //wcw<0 or wwAmbig<0                        //~vbzzR~//~v7auR~//~v7ayI~
//  if (wcw||wwAmbig<0)  //wcw<0 or wwAmbig<0                      //~v7auI~//~v7ayR~
    if (wcw||width<0) //wcwidth()==-1 or wwAmbig<0                 //~v7ayI~
    {                                                              //~v6BZI~//~v7ayI~
    	width=-1;                                                  //~v640I~//~v7ayI~
		UTRACEP("%s:wcwidth err set width=%d,wcw=%d wwAmbig=%d for ucs=%04x\n",UTT,width,wcw,wwAmbig,Pucs);//~v6BZI~//~vbzzR~//~v7ayI~
    }                                                              //~v6BZI~//~v7ayI~
    else                                                           //~v650I~//~v7ayI~
//* wcw==0 && wwambig>=0                                                        //~vbzzR~//~v7auI~//~v7ayR~
    {                                                              //~v6c5I~//~v7ayI~
//      flag|=UTFWWF_WIDTH0;    //combining char                   //~v650I~//~v6V7R~//~v7ayI~
//  	if (Popt & UTFWWO_APIW0)	//ARM4                         //~v6c5R~//~v6V7R~//~v7ayI~
//  	if (Popt & (UTFWWO_APIW0|UTFWWO_MK_WCWIDTH))	//ARM4     //~v6V7I~//~v6VpR~//~v7ayI~
//    	if (Pmkwidth && (Popt & (UTFWWO_APIW0|UTFWWO_MK_WCWIDTH)))	//ARM4//~v6VpI~//~v6WiR~//~v7ayI~
#ifdef ARM                                                         //~v70iI~//~v7ayI~
        width=0;                //accept result of wcwidth(),get sbcsid for width0//~v70iI~//~v7ayI~
        flag|=UTFWWF_WIDTH0;    //combining char                   //~v70iI~//~v7ayI~
        UTRACEP("%s:wcwidth()=%d,mk_wcwidth()=%d,return width=%d,flag=%x\n",UTT,wcw,Pmkwidth,width,flag);//~v70iI~//~v7ayI~
#else                                                              //~v70iI~//~v7ayI~
        flag|=UTFWWF_WIDTH0;    //combining char                   //~v7ayI~
      if (!(Popt & UTFWWO_FORMAT))                                 //~v6WiR~//~v7ayI~
      {                                                            //~v6WiI~//~v7ayI~
//     	if (Pmkwidth && (Popt & (UTFWWO_APIW0|UTFWWO_MK_WCWIDTH))==(UTFWWO_APIW0|UTFWWO_MK_WCWIDTH))//ARM4//~v6WiI~//~v79dR~//~v7ayI~
//        if (Pmkwidth && (Popt & UTFWWO_APIW0))                     //~v79dR~//~v7awR~//~v7ayI~
//        {                                                          //~v6V7I~//~v7awR~//~v7ayI~
//            width=0;                //accept result of wcwidth(),get sbcsid for width0//~v6c5I~//~v7awR~//~v7ayI~
//            flag|=UTFWWF_WIDTH0;    //combining char               //~v6V7I~//~v7awR~//~v7ayI~
//            UTRACEP("%s:@@@@set ucs=0x%04x,width=0 by unmatch wcwidth()=%d,mk_wcwidth()=%d,return width=%d,flag=%x\n",UTT,Pucs,wcw,Pmkwidth,width,flag);//~v6V7I~//~v6VpM~//~v79dI~//~vbz5R~//~v7awR~//~v7ayI~
//        }                                                          //~v6V7I~//~v6VpM~//~v7awR~//~v7ayI~
//      width=0;    //COMB is width 0                              //~v7awR~//~v7ayR~
//      if (!(flag & UTFWWF_COMB))                                 //~v7awR~//~v7ayR~
        if ((flag & UTFWWF_COMB))                                  //~v7ayI~
        {                                                          //~v7ayI~
		    width=0;    //COMB is width 0                          //~v7ayI~
	        UTRACEP("%s:set width=0 by COMB ucs=0x%04x,wcw=%d,flag=0x%04x\n",UTT,Pucs,wcw,flag);//~v7ayI~
        }                                                          //~v7ayI~
//        {                                                          //~v7awI~//~v7ayR~
//            int gw=getGlyphWidth(Pucs,wwAmbig);                    //~v7awR~//~v7ayR~
//            if (gw>0)                                              //~v7awI~//~v7ayR~
//                width=gw;                                          //~v7awI~//~v7ayR~
//        }                                                          //~v7awI~//~v7ayR~
      ...
      {                                                            //~v7ahI~//~v7ayI~
      //*FORMAT                                                    //~v7ayI~
//      width=1;                                                   //~v7ahI~//~v7auR~//~v7ayI~
        width=wcw;                                                 //~v7auI~//~v7ayI~
//        int gw=getGlyphWidth(Pucs,wwAmbig);                        //~v7awI~//~v7ayR~
//        if (gw>=0)  //format may width0                            //~v7awR~//~v7ayR~
//            width=gw;                                              //~v7awI~//~v7ayR~
...
#ifdef XXX                                                         //~v6WvI~//~v7ayI~
	if (!flag && (width==1||width==2))                             //~v6VbR~//~v7ayI~
#else                                                              //~v6WvI~//~v7ayI~
	if (width>=0 && width<=2)                                      //~v6WvR~//~v7ayI~
#endif                                                             //~v6WvI~//~v7ayI~
    {                                                              //~v6VbR~//~v7ayI~
#ifdef XXX                                                         //~v6WvI~//~v7ayI~
		if (utf4_isSpacingCombiningMark(0,(UWUCS)Pucs))            //~v6VbR~//~v7ayI~
#else                                                              //~v6WvI~//~v7ayI~
		if (rc2=utf4_isSpacingCombiningMark(UTF4ISCMO_WIDTHPARM|width,(UWUCS)Pucs),rc2)//~v6WvI~//~v7ayI~
#endif                                                             //~v6WvI~//~v7ayI~
        {                                                          //~v6VbR~//~v7ayI~
			UTRACEP("%s:SCM ucs=%04x,wcw=%d,Pmkwidth=%d,flag=%x\n",UTT,Pucs,wcw,Pmkwidth,flag);//~v6VbR~//~v7ayI~
#ifndef XXX                                                        //~v6WvI~//~v7ayI~
          	if (rc2==2)	//combining2scm                            //~v6WvI~//~v7ayI~
            {                                                      //~v6WvI~//~v7ayI~
				flag|=UTFWWF_COMB2SCM;    //assume combining as spacing combining char//~v6WvR~//~v7ayI~
				flag&=~UTFWWF_COMB;                                //~v6WvI~//~v7ayI~
            }                                                      //~v6WvI~//~v7ayI~
#endif                                                             //~v6WvI~//~v7ayI~
			flag|=UTFWWF_SCM;    //spacing combining char          //~v6VbR~//~v7ayI~
            width=2;                                               //~v6VbR~//~v7ayI~
			UTRACEP("%s:SCM set DBCS ucs=%04x,wcw=%d,Pmkwidth=%d,out width=%d,flag=%x\n",UTT,Pucs,wcw,Pmkwidth,width,flag);//~v6VbR~//~v7ayI~
        }                                                          //~v6VbR~//~v7ayI~
    }                                                              //~v6VbR~//~v7ayI~
#ifdef XXE                                                         //~vbz5R~//~v7ayI~
#ifdef AAA                                                         //~vbzzR~//~v7ayI~
    width=utfwwapichkCursorXXE(Popt,Pucs,width,&flag);             //~vbz5R~//~v7ayI~
#endif                                                             //~vbzzR~//~v7ayI~
#endif                                                             //~vbz5R~//~v7ayI~
	UTRACEP("%s:return ucs=0x%04x,(  %s  ),newwidth=%d,mkwidth=%d,wcwidth()=%d,flag=0x%04x-->0x%04x,%s\n",UTT,Pucs,//~vbz5R~//~v79CI~//~v7ayI~
			    UCV_U2F1(Pucs),//~vbz5I~//~v79CM~                  //~v7a7R~//~v7ayI~
				width,Pmkwidth,wcw,*Ppflag,flag,(width!=Pmkwidth?"updated":"same"));//~vbz5R~//~v79CI~//~v7ayI~
    *Ppflag=flag;                                                  //~v640M~//~v7ayI~
    return width;                                                  //~v640M~//~v7ayI~
}//utfwwapichk                                                     //~v640M~//~v7ayI~
#else//else TTT                                                    //~v7ayR~
//*******************************************************************//~v7ayI~
...
//  wcw=wcwidth((wchar_t)Pucs);   //better to return large va      //~v7agR~
    wcw=wcwidth((UWCHART)Pucs);   //better to return large value even displayed 1 cell on english terminal//~v6a0I~
    wwAmbig=GambigWidth;    //-1,1,2                               //~vbzzR~
    if (Ppwcwidth)                                                 //~v640M~
        *Ppwcwidth=wcw;                                            //~v640M~
    UTRACEP("%s:entry ucs=0x%04x,Pmkwidth=%d,wcwidth=%d,GambigWidth=%d,flag=0x%04x\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,flag);	//@@@@test//~vbzzR~//~v7awR~
//  if (wcw>0)                                                     //~v640M~//~vbzzR~
    if (wcw>0 && wwAmbig>=0)    //and ambig length by -G option not to adjust//~vbzzR~
    {                                                              //~v640M~
...
#ifdef XXE            	                                           //~v79EI~
#else                                                              //~v79EI~
//                if (UTF_AMBIG2CELLMODE())   //1 cell mode for ambiguous//~v79EI~//~vbzzR~
//                    width=2;                                       //~v79EI~//~vbzzR~
//                else                                               //~v79EI~//~vbzzR~
//                {                                                  //~v7agI~//~vbzzR~
//                  if (wwAmbig>0)                                   //~vbzzR~//~v7awR~
//                    width=wwAmbig; //width by -G option            //~vbzzR~//~v7awR~
//                  else                                             //~vbzzR~//~v7awR~
//                  {                                                //~vbzzR~//~v7awR~
//#ifdef XXE                                                         //~vbzzR~//~v7awR~
//                    UTRACEP("%s:wcw=1 xxe call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~
//                    width=xxe_GetCursorWidth(0,Pucs);              //~vbzzR~//~v7awR~
//#else                                                              //~vbzzR~//~v7awR~
//                    UTRACEP("%s:wcw=1 lnxcons call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~
//                    width=uvioGetCursorWidth(0,Pucs);              //~v7agI~//~v7awR~
//#endif                                                             //~vbzzR~//~v7awR~
//                 }                                                 //~vbzzR~//~v7awR~
				 	width=getGlyphWidth(Pucs,wwAmbig);             //~v7awI~
                  if (width<0)	//failed                           //~v7agI~
                	width=1;                                       //~v79EI~
                    if (width==2)                                  //~v7agI~
		            	flag|=UTFWWF_F2C1;      //    0x80   //WideSBCS; font width=2,wcwidth==1//~v7agI~
//                }                                                  //~v7agI~//~vbzzR~
#endif                                                             //~v79EI~
	        	swAmbigChange=1;                                   //~v79EI~
            }                                                      //~v79EI~
            else                                                   //~v7agI~
    	    if ((flag & UTFWWO_MK_AMBIGUOUS)                       //~v7agI~
            &&  wcw==2                                             //~v7agI~
            &&  Pmkwidth==2)                                       //~v7agI~
            {                                                      //~v7agI~
//                if (UTF_AMBIG2CELLMODE())   //1 cell mode for ambiguous//~v7agI~//~vbzzR~
//                    width=2;                                       //~v7agI~//~vbzzR~
//                else                                               //~v7agI~//~vbzzR~
//                {                                                  //~v7agI~//~vbzzR~
//		            int gw=uvioGetCursorWidth(0,Pucs);             //~v7agI~//~vbzzR~
  		            int gw;                                        //~vbzzR~
//                    if (wwAmbig>0)                                 //~vbzzR~//~v7awR~
//                        gw=wwAmbig; //width by -G option           //~vbzzR~//~v7awR~
//                    else                                           //~vbzzR~//~v7awR~
//                    {                                              //~vbzzR~//~v7awR~
//#ifdef XXE                                                         //~vbzzR~//~v7awR~
//                        UTRACEP("%s:wcw=2 xxe call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~
//                        gw=xxe_GetCursorWidth(0,Pucs);             //~vbzzR~//~v7awR~
//#else                                                              //~vbzzR~//~v7awR~
//                        UTRACEP("%s:wcw=2 lnxcons call get ambig width ucs=0x%04x\n",UTT,Pucs);//~vbzzR~//~v7awR~
//                        gw=uvioGetCursorWidth(0,Pucs);             //~vbzzR~//~v7awR~
//#endif                                                             //~vbzzR~//~v7awR~
//                    }                                              //~vbzzR~//~v7awR~
                    gw=getGlyphWidth(Pucs,wwAmbig);                //~v7awI~
                  	if (gw==1)	//wcwidth==2 & glyphwidth=1        //~v7agI~
                    {                                              //~v7agI~
		            	flag|=UTFWWF_F1C1;      //0x40 narrow dbcs //~v7agI~
                    }                                              //~v7agI~
//                }                                                  //~v7agI~//~vbzzR~
	        	swAmbigChange=1;                                   //~v7agI~
            }                                                      //~v7agI~
            if (swAmbigChange)                                     //~v79EI~
            {                                                      //~v79EI~
                UTRACEP("%s:ambiguous ucs=0x%04x(%s),wcwidth()=%d,Pmkwidth=%d,set width=%d\n",//~v79ER~//~v7ahR~
						UTT,Pucs,UCV_U2F1(Pucs),wcw,Pmkwidth,width);//~v79ER~//~v7a7R~
            }                                                      //~v79EI~
//          else                                                   //~v79EI~//~v79FR~
...
//          }                                                      //~v6V7I~//~v79FR~
          ...
          {//swAmbigChange=0 , wcw=0 & wwAmbig>=0                  //~v7awR~
      		if (Popt & UTFWWO_FORMAT)                              //~v7awI~
            {                                                      //~v7awI~
        		width=wcw;                                         //~v7awI~
				int	gw=getGlyphWidth(Pucs,wwAmbig);                //~v7awI~
        		if (gw>=0) //format may width0                     //~v7awR~
            		width=gw;                                      //~v7awI~
        		UTRACEP("%s:swAmbigChage=0(wcw>0 & wwAmbig=%d) FORMAT ucs=0x%04x wcwidth()=%d ,mk_wcwidth()=%d,width=%d\n",UTT,wwAmbig,wcw,Pucs,wcw,Pmkwidth,width);//~v7awI~
            }                                                      //~v7awI~
            else                                                   //~v7awI~
    		if ((flag & UTFWWF_COMB))   //combining definition mkWidth=0//~v7awI~
            {                                                      //~v7awI~
                UTRACEP("%s:unmatch wcwidth()=%d,PmkWidth=%d,return width=%d,ucs=0x%04x\n",UTT,wcw,Pmkwidth,width,Pucs);//~v7awI~
            }                                                      //~v7awI~
            else                                                   //~v7awI~
            {                                                      //~v7awI~
        		width=wcw;                                         //~v7awI~
        		...
        		if (gw>0)                                          //~v7awI~
            		width=gw;                                      //~v7awI~
        		UTRACEP("%s:wcw>0 & wwAmbig>=0 but  Not Ambig change, Not Format,COMB ucs=0x%04x Pmkwidth=%d,wcwidth=%d,wwAmbig=%d,width=%d\n",UTT,Pucs,Pmkwidth,wcw,wwAmbig,width);//~v7awI~
            }                                                      //~v7awI~
//            if ((Popt & UTFWWO_MK_WCWIDTH)  //ARM4                 //~v6V7I~//~v7awR~
//            &&  Pmkwidth!=wcw)                                     //~v6V7I~//~v7awR~
//            {                                                      //~v6V7I~//~v7awR~
//////            if (!(Popt & UTFWWO_MK_COMB))   //combining definition//~v6W9R~//~v6WmR~//~v7awR~
////              if (!(flag & UTFWWF_COMB))   //combining definition  //~v6WmR~//~v7awR~
////              {                                                    //~v6WiI~//~v7awR~
////                width=wcw;                                         //~v6V7I~//~v7awR~
////                UTRACEP("%s:unmatch wcwidth()=%d,mk_wcwidth()=%d,return width=%d,ucs=0x%04x\n",UTT,wcw,Pmkwidth,width,Pucs);//~v6V7I~//~v7auR~//~v7awR~
////              }                                                    //~v6WiI~//~v7awR~
//            }                                                      //~v6V7I~//~v7awR~
//            else                                                   //~v6VaI~//~v7awR~
//            if (Pmkwidth==1 && wcw==2)  //sbcs but wcwidth=2       //~v640M~//~v7awR~
//            {                                                      //~v640M~//~v7awR~
////              if (!(flag & UTFWWF_SBCSF))   //no tbl defintion     //~v6WiI~//~v7awR~
////              {                                                    //~v6WiI~//~v7awR~
//                flag|=UTFWWF_F1C1;      //Narrow DBCS when wcwcidth=2//~v640M~//~v7awR~
//                width=2;                //occupy 2 column          //~v640M~//~v7awR~
//                UTRACEP("%s:%04x change width 1 to 2 by wcwidth()\n",UTT,Pucs);//~v6WiI~//~v70hR~//~v7auR~//~v7awR~
////              }                                                    //~v6WiI~//~v7awR~
//            }                                                      //~v640M~//~v7awR~
////            else                                                 //~v640R~//~v7awR~
////            if (Pmkwidth==2 && wcw==1)  //dbcd but wcwidth=1     //~v640R~//~v7awR~
////            {                                                    //~v640R~//~v7awR~
////                width=1;                //occupy 1 column        //~v640R~//~v7awR~
////            }                                                    //~v640R~//~v7awR~
//            else                                                   //~v7auI~//~v7awR~
//            if (Pmkwidth==2 && wcw==1)  //under confition Pmkwidth!=wcw//~v7auI~//~v7awR~
//            {                                                      //~v7auI~//~v7awR~
//                width=wcw;                                         //~v7auI~//~v7awR~
//                UTRACEP("%s:%04x change width 2 to 1 by wcwidth()\n",UTT,Pucs);//~v7auI~//~v7awR~
//            }                                                      //~v7auI~//~v7awR~
          }//swAmbigChange=0                                       //~v7awI~
        }                                                          //~v640M~
...
//  if (wcw)                                                       //~v650I~//~vbzzR~
//* wcw<=0 or wwAmbig<0                                            //~vbzzR~
//  if (wcw||wwAmbig)  //wcw<0 or wwAmbig<0                        //~vbzzR~//~v7auR~
    if (wcw||wwAmbig<0)  //wcw<0 or wwAmbig<0                      //~v7auI~
    {                                                              //~v6BZI~
    	width=-1;                                                  //~v640I~
		UTRACEP("%s:wcwidth err set width=%d,wcw=%d wwAmbig=%d for ucs=%04x\n",UTT,width,wcw,wwAmbig,Pucs);//~v6BZI~//~vbzzR~
    }                                                              //~v6BZI~
    else                                                           //~v650I~
//* wcw==0 && ambig>=0                                                        //~vbzzR~//~v7auI~
    {                                                              //~v6c5I~
...
//        if (Pmkwidth && (Popt & UTFWWO_APIW0))                     //~v79dR~//~v7awR~
//        {                                                          //~v6V7I~//~v7awR~
//            width=0;                //accept result of wcwidth(),get sbcsid for width0//~v6c5I~//~v7awR~
//            flag|=UTFWWF_WIDTH0;    //combining char               //~v6V7I~//~v7awR~
//            UTRACEP("%s:@@@@set ucs=0x%04x,width=0 by unmatch wcwidth()=%d,mk_wcwidth()=%d,return width=%d,flag=%x\n",UTT,Pucs,wcw,Pmkwidth,width,flag);//~v6V7I~//~v6VpM~//~v79dI~//~vbz5R~//~v7awR~
//        }                                                          //~v6V7I~//~v6VpM~//~v7awR~
        width=0;    //COMB is width 0                              //~v7awR~
        if (!(flag & UTFWWF_COMB))                                 //~v7awR~
        {                                                          //~v7awI~
			int	gw=getGlyphWidth(Pucs,wwAmbig);                    //~v7awR~
            if (gw>0)                                              //~v7awI~
            	width=gw;                                          //~v7awI~
        }                                                          //~v7awI~
        UTRACEP("%s:%06x unmatch wcwidth()=%d Not Format, mk_wcwidth()=%d,wwAmbig=%d,return width=%d,flag=0x%04x\n",UTT,Pucs,wcw,Pmkwidth,wwAmbig,width,flag);//~v7awR~
      }                                                            //~v6WiI~
      else                                                         //~v79dI~
      {                                                            //~v7ahI~
//      width=1;                                                   //~v7ahI~//~v7auR~
        ...
        if (gw>=0)  //format may width0                            //~v7awR~
            width=gw;                                              //~v7awI~
        UTRACEP("%s:@@@@FORMAT ucs=0x%04x wcwidth()=%d ,mk_wcwidth()=%d,return width=%d,flag=0x%04x,wwAmbig=%d\n",UTT,Pucs,wcw,Pmkwidth,width,flag,wwAmbig);//~v79dR~//~vbz5R~//~v7auM~//~v7awR~
      }                                                            //~v7ahI~
#endif                                                             //~v70iI~
    }                                                              //~v6c5I~
//UTRACEP("apichk sizeof uwchart=%d,ucs=%4x,wcwidth=%d,mkw=%d,ret-width=%d,flag=0x%04x->0x%04x\n",sizeof(UWCHART),Pucs,wcw,Pmkwidth,width,*Ppflag,flag);//~v6a0R~//~v6b9R~//~v6c5R~//~v6p2R~//~vbz5R~
	...
	UTRACEP("%s:return ucs=0x%04x,(  %s  ),newwidth=%d,mkwidth=%d,wcwidth()=%d,flag=0x%04x-->0x%04x,%s\n",UTT,Pucs,//~vbz5R~//~v79CI~
			    UCV_U2F1(Pucs),//~vbz5I~//~v79CM~                  //~v7a7R~
				width,Pmkwidth,wcw,*Ppflag,flag,(width!=Pmkwidth?"updated":"same"));//~vbz5R~//~v79CI~
    *Ppflag=flag;                                                  //~v640M~
    return width;                                                  //~v640M~
}//utfwwapichk                                                     //~v640M~
#endif //else TTT                                                  //~v7ayI~
#else	//WIN                                                      //~v697I~
//*******************************************************          //~v796I~
int chkMBlen(ULONG Pucs)                                           //~v7akI~
{                                                                  //~v7akI~
	wchar_t wkucs[4];                                              //~v7akI~
	char wkmbs[8];                                                 //~vbzzR~
    int utf16ctr;                                                  //~v7akI~
//**********                                                       //~v7akI~
	if (Pucs>0xffff)                                               //~v7akI~
		UCS4_TO_UTF16(Pucs,wkucs,&utf16ctr);                       //~v7akR~
    else                                                           //~v7akI~
    {                                                              //~v7akI~
	    wkucs[0]=(wchar_t)Pucs;                                    //~v7akI~
    	utf16ctr=1;                                                //~v7akI~
    }                                                              //~v7akI~
    wkucs[utf16ctr]=0;                                             //~v7akI~
    int len=(int)wcstombs(wkmbs,wkucs,sizeof(wkmbs));   //                      //~v7akI~//~vbzzR~
    wkmbs[len>=0?len:0]=0;                                         //~vbzzR~
    UTRACEP("%s:len=%d,ucs=0x%04x,mbs=%s\n",UTT,len,Pucs,wkmbs);                //~v7akR~//~vbzzR~
    return len;                                                    //~v7akI~
}                                                                  //~v7akI~
//*******************************************************          //~vbz4R~
//W32 console                                                      //~v7a2R~
//bypass for w32 console                                           //~v7a2I~
//*******************************************************          //~v79mR~
int utfwwapichkCursorW32(int Popt,ULONG Pucs,int Pmkwidth,int *Ppflag)//~v796I~//~v79mR~
{                                                                  //~v796I~//~v79mR~
    int flag,width;                                                //~v79mR~
//  int cursorstep;                                                //~v79mI~
//  char wkmbs[8];                                                 //~v7a3I~//~v7akR~
//*************************                                        //~v796I~//~v79mR~
    flag=*Ppflag;                                                  //~v796I~//~v79mR~
    width=Pmkwidth;                                                //~v796I~//~v79mR~
    if (Pucs==(ULONG)GutfTestUcs)                                  //~v7akI~
    {                                                              //~v7akI~
        UTRACEP("%s:Debug trap by GutfTestUcs ucs=%04x\n",UTT,Pucs);	//@@@@test//~v7akI~
    }                                                              //~v7akI~
...
//    {                                                              //~v796I~//~v79mR~
    if (IS_UTF16_HIGH(Pucs)||IS_UTF16_LOW(Pucs))  //UTF16 pair     //~v79mI~
    	return width;                                              //~v79mI~
    UTRACEP("%s:entry opt=0x%04x,ucs=x%04x( %s ),width=%d,flag=0x%04x,GambigWidth=%d\n",UTT,Popt,Pucs,UCV_U2F1(Pucs),width,*Ppflag,GambigWidth);//~v798I~//~v79mR~//~v79MR~//~v7a7R~//~v7bbR~
    if (width==0 || width==-1)                                     //~v79mI~
        return width;                                              //~v79mI~
    if (width==2                                                   //~v79CI~//~vbzzR~
//  &&  flag==UTFWWO_MK_AMBIGUOUS  //x2000   //ambiguous only(exclude space combining)//~v79CI~//~v7a7R~//~vbzzR~
    &&  (flag & UTFWWO_MK_AMBIGUOUS)  //x2000   //ambiguous only(exclude space combining)//~v7a7I~//~vbzzR~
    &&  !(flag & UTFWWF_SCM)                                       //~v7a7I~//~vbzzR~
    )                                                              //~v79CI~//~vbzzR~
    {                                                              //~v79CI~//~vbzzR~
//        if (!UTF_AMBIG2CELLMODE())  //1 cell mode for ambiguous    //~v79CI~//~vbzzR~//~vbzzR~
//        {                                                          //~v79CI~//~vbzzR~//~vbzzR~
//            UTRACEP("%s:1cellmode\n",UTT);                       //~v7akR~
//            int opt3=UW2MWO_NOBESTFIT|UW2MWO_NOREVERSECHK|UW2MWO_SUBCHRC;//~v7a3R~//~v7a7R~
//            int len2=uwctombW(opt3,Pucs,wkmbs);                    //~v7a3R~//~v7a7R~
//            int wwDraw=uvioGetCursorWidth(0,Pucs);               //~v7a7R~
//          if (len2>1)                                              //~v7a3I~//~v7a7R~
//          {                                                        //~v7a3I~//~v7a7R~
//           if (wwDraw==1) //1 cell glyph                         //~v7a7R~
//           {                                                     //~v7a7R~
//            flag|=UTFWWF_F1C1;      //     0x40   //NarrowDBCS; font width=2,distance to next column=1 at uviom//~v7a3I~//~v7a7R~
////          *Ppflag=flag;                                          //~v7a3I~//~v7a7R~
//           }                                                     //~v7a7R~
//          }                                                        //~v7a3I~//~v7a7R~
//          else                                                     //~v7a3I~//~v7a7R~
//          {                                                        //~v7a3I~//~v7a7R~
//           if (wwDraw==1)    //1 cell glyph,fix to not ambiguous and sbcs//~v7a7R~
//           {                                                     //~v7a7R~
////          flag &=~UTFWWO_MK_AMBIGUOUS; //x2000                   //~v79CI~//~v7a7R~
////          *Ppflag=flag;                                          //~v79CI~//~v7a7R~
////          return 1;                  //use 1 cell if 2cellmode off//~v79CI~//~v7a7R~
//            width=1;                   //use 1 cell if 2cellmode off//~v7a7R~
//           }                                                     //~v7a7R~
//           else //sbcs is 2 cell glyph                           //~v7a7R~
//           {                                                     //~v7a7R~
//            flag|=UTFWWF_F2C1;      //     0x80   //WideSBCS; font width=2,distance to next column=1 at uviom//~v7a7R~
////          *Ppflag=flag;                                        //~v7a7R~
//           }                                                     //~v7a7R~
//          }                                                        //~v7a3I~//~v7a7R~
                                                                   //~v7a7I~
                                                                   //~v7a7I~
//      	int opt3=UW2MWO_NATIVE; //native no support NoBestFit  //~v7a7R~
//      	int opt3=UW2MWO_NOBESTFIT|UW2MWO_NOREVERSECHK|UW2MWO_SUBCHRC;//~v7a7M~//~v7akR~
//          int len2=uwctombW(opt3,Pucs,wkmbs);                    //~v7a7I~//~v7akR~
#ifdef DDD                                                         //~v7bbI~
            int len2=chkMBlen(Pucs);                               //~v7akI~
#else                                                              //~v7bbI~
            int len2=width;	//*Pmkwidth, 0 form COMBINING,2 for ambiguous, 0/1/2 for format//~v7bbR~
#endif                                                             //~v7bbI~
//          int wwDraw=uvioGetCursorWidth(0,Pucs);                 //~v7a7I~//~vbzzR~
            int wwDraw;                                            //~vbzzR~
            if (GambigWidth)  //No adjust required, 1 or 2 by /G option(-1 is processed already)//~vbzzR~
                wwDraw=GambigWidth;                                //~vbzzR~
            else                                                   //~vbzzR~
            {                                                      //~v7bbI~
		        UTRACEP("%s:@@@@set 0x%04x(%s),call GetGlyphWidth for Ambig ucs width=%d\n",UTT,Pucs,UCV_U2F1(Pucs),width);//~v7bbI~
#ifdef WXE                                                         //~vbzzI~
#ifndef SSS                                                        //~vbzzI~
//  	        wwDraw=wxe_GetCursorWidth(0,Pucs);                 //~vbzzR~//~v7c7R~
				int optgcw=0;                                      //~v7c7I~
                if (Popt & UTFWWO_FORMAT) //       0x8000   //defined as Format    char at utf4.c  ZWJ etc//~v7c7I~
					optgcw|=UVGCWO_RETW0;	//    0x20        //return width=0//~v7c7I~
    	        wwDraw=wxe_GetCursorWidth(optgcw,Pucs);            //~v7c7I~
#endif                                                             //~vbzzI~
#else                                                              //~vbzzI~
//              wwDraw=uvioGetCursorWidth(0,Pucs);                 //~vbzzR~//~v7c7R~
				int optgcw=0;                                      //~v7c7I~
                if (Popt & UTFWWO_FORMAT) //       0x8000   //defined as Format    char at utf4.c  ZWJ etc//~v7c7I~
					optgcw|=UVGCWO_RETW0;	//    0x20        //return width=0//~v7c7I~
                wwDraw=uvioGetCursorWidth(optgcw,Pucs);            //~v7c7I~
#endif                                                             //~vbzzI~
            }                                                      //~v7bbI~
        //*if returned -1(err) keep width=2                        //~vbzzI~
           	if (wwDraw>1) //2 cell glyph                           //~v7a7I~
           	{                                                      //~v7a7I~
//        		if (len2==1)                                       //~v7a7I~//~v7bbR~
          		if (!(Popt & UTFWWO_MK_WCWIDTH))	//width=2 is not by mk_width but ambiguous//~v7bbI~
                {                                                  //~v7bbI~
		            flag|=UTFWWF_F2C1;      //     0x80   //WideSBCS; font width=2,distance to next column=1 at uviom//~v7a7I~
		            UTRACEP("%s:@@@@set 0x%04x(%s),F2C1 ambiguous and wwDraw>1,GambigWidth=%d\n",UTT,Pucs,UCV_U2F1(Pucs),GambigWidth);//~v7bbR~
                }                                                  //~v7bbI~
                else                                               //~v7bbI~
                {                                                  //~v7bbI~
		            UTRACEP("%s:@@@@set 0x%04x(%s),through because MK_WIDTH=2>1\n",UTT,Pucs,UCV_U2F1(Pucs));//~v7bbI~
                }                                                  //~v7bbI~
//              else    //mblen>1                                  //~v7acR~//~v7bbR~
//  	            flag|=UTFWWF_AMBDBCS;   //     0x20   //Ambiguous dbcs//~v7acR~//~v7bbR~
           	}                                                      //~v7a7I~
            else                                                   //~v7a7R~
           	if (wwDraw==1) //1 cell glyph                          //~v7a7I~
            {                                                      //~v7a7I~
//        		if (len2>1)   //mb dbcs                            //~v7a7R~//~v7bbR~
          		if (Popt & UTFWWO_MK_WCWIDTH)	//width=2 is also by mk_width//~v7bbI~
                {                                                  //~v7bbI~
		            flag|=UTFWWF_F1C1;      //     0x40   //NarrowDBCS; font width=2,distance to next column=1 at uviom//~v7a7I~
		            UTRACEP("%s:@@@@set 0x%04x(%s),F1C1 ambiguous and wwDraw=1 by MK_WCWIDTH on,GambigWidth=%d\n",UTT,Pucs,UCV_U2F1(Pucs),GambigWidth);//~v7bbR~
                }                                                  //~v7bbI~
                else                                               //~v7a7I~
                {                                                  //~v7bbI~
//        		if (len2==1)     //not -1:mb err                   //~v7a7R~
                	width=1;	//assume as sbcs                   //~v7a7I~
		            UTRACEP("%s:@@@@set 0x%04x(%s),width=1 ambiguous and wwDraw=1 by MK_WCWIDTH=off,GambigWidth=%d\n",UTT,Pucs,UCV_U2F1(Pucs),GambigWidth);//~v7bbR~
				}                                                  //~v7bbI~
            }                                                      //~v7a7I~
            else                                                   //~v7c7I~
            if (wwDraw==0)                                         //~v7c7I~
            {                                                      //~v7c7I~
	    	    UTRACEP("%s:set width=0 by Cf glyph width=%d,ucs=0x%04x(%s)\n",UTT,Pucs,wwDraw,UCV_U2F1(Pucs));//~v7c7I~
            	width=0;                                           //~v7c7I~
            }                                                      //~v7c7I~
    	    UTRACEP("%s:/Gy ambiguous glyph chk opt=0x%04x,width=%d,flag=0x%04x,ucs=0x%04x(%s),mblen=%d,glyphlen=%d,Gambigwidth=%d,old flag=0x%04x\n",UTT,Popt,width,flag,Pucs,UCV_U2F1(Pucs),len2,wwDraw,GambigWidth,*Ppflag);//~v7a7R~//~v7akR~//~vbzzR~//~vbzzR~//~v7bbR~
            *Ppflag=flag;                                          //~v7a7I~
//        }                                                          //~v79CI~//~vbzzR~//~vbzzR~
//        else            // /Nv option, abiguous's wcw=2, add padding at uviom for locale file//~v7a2R~//~vbzzR~
//        {                                                          //~v7a2I~//~vbzzR~
////            UTRACEP("%s:2cellmode\n",UTT);                       //~v7akR~//~vbzzR~
////          flag &=~UTFWWO_MK_AMBIGUOUS; //x2000                   //~v7a2R~//~vbzzR~
//          if (Guvio2Stat & UVIO2S_AMBIG_SBCS)   //for all ambiguous, default off(Wide)//~vbzzR~
//          {                                                      //~vbzzR~
//            width=1;                                             //~vbzzR~
//            UTRACEP("%s:by /GN1 change to sbcs width=%d,flag=0x%04x,ucs=0x%04x(%s),old flag=0x%04x\n",UTT,width,flag,Pucs,UCV_U2F1(Pucs),*Ppflag);//~vbzzR~
//          }                                                      //~vbzzR~
//          else                                                   //~vbzzR~
//          if (Guvio2Stat & UVIO2S_AMBIG_UNP)   //for all ambiguous, default off(Wide)//~vbzzR~
//          {                                                      //~vbzzR~
//            width=-1;                                            //~vbzzR~
//            UTRACEP("%s:by /GN0 change to unprintable width=%d,flag=0x%04x,ucs=0x%04x(%s),old flag=0x%04x\n",UTT,width,flag,Pucs,UCV_U2F1(Pucs),*Ppflag);//~vbzzR~
//          }                                                      //~vbzzR~
//          else                                                   //~vbzzR~
//          {                                                      //~vbzzR~
//            flag|=UTFWWF_F2C1;      //     0x80   //WideSBCS; font width=2,distance to next column=1 at uviom//~v7a2I~//~vbzzR~
//            UTRACEP("%s:by /GN2 keep F2C1 width=%d,flag=0x%04x,ucs=0x%04x(%s),old flag=0x%04x\n",UTT,width,flag,Pucs,UCV_U2F1(Pucs),*Ppflag);//~vbzzR~
//          }                                                      //~vbzzR~
//            *Ppflag=flag;                                          //~v7a2I~//~vbzzR~
//        }                                                          //~v7a2I~//~vbzzR~
//        UTRACEP("%s:ambiguous return width=%d,flag=0x%04x,ucs=0x%04x(%s)\n",UTT,width,*Ppflag,Pucs,UCV_U2F1(Pucs));//~v7a7R~//~v7akR~//~vbzzR~
    }                                                              //~v79CI~//~vbzzR~
    else                                                           //~v7b3I~
//*    (width!=2 || (flag & UTFWWO_MK_AMBIGUOUS)==0 ||(flag & UTFWWF_SCM)//~v7b3I~
    if (width==1 && (flag & UTFWWO_MK_AMBIGUOUS)==0 && !(flag & UTFWWF_SCM))//~v7b3I~
    {                                                              //~v7b3I~
//  if (0)                                                         //~v7b3R~
//  {                                                              //~v7b3R~
//  	int len2=chkMBlen(Pucs);                                   //~v7b3R~
        int wwExt;                                                 //~v7b3I~
		UTRACEP("%s:@@@@set 0x%04x(%s),call GetGlyphWidth for NOT ambig ucs width=%d,GambigWidth=%d\n",UTT,Pucs,UCV_U2F1(Pucs),width,GambigWidth);//~v7bbR~
#ifdef WXE                                                         //~v7b3I~
//		wwExt=wxe_GetCursorWidth(UVGCWO_NOAMB,Pucs);               //~v7c7I~
		int optgcw2=UVGCWO_NOAMB;                                  //~v7c7I~
        if (Popt & UTFWWO_FORMAT) //       0x8000   //defined as Format    char at utf4.c  ZWJ etc//~v7c7I~
			optgcw2|=UVGCWO_RETW0;	//    0x20        //return width=0//~v7c7R~
		wwExt=wxe_GetCursorWidth(optgcw2,Pucs);               //~v7b3R~//~v7c7R~
#else                                                              //~v7b3I~
//      wwExt=uvioGetCursorWidth(UVGCWO_NOAMB,Pucs);               //~v7b3R~//~v7c7R~
		int optgcw2=UVGCWO_NOAMB;                                  //~v7c7R~
        if (Popt & UTFWWO_FORMAT) //       0x8000   //defined as Format    char at utf4.c  ZWJ etc//~v7c7I~
			optgcw2|=UVGCWO_RETW0;	//    0x20        //return width=0//~v7c7I~
        wwExt=uvioGetCursorWidth(optgcw2,Pucs);                    //~v7c7R~
#endif                                                             //~v7b3I~
        if (wwExt>1) //2 cell glyph                                //~v7b3R~
        {                                                          //~v7b3I~
//      	if (len2!=2)                                           //~v7b3R~
//          {                                                      //~v7b3R~
			    flag|=UTFWWF_F2C1;      //     0x80   //WideSBCS; font width=2,distance to next column=1 at uviom//~v7b3I~
	            *Ppflag=flag;                                      //~v7b3I~
				UTRACEP("%s:NON ambiguous wwDraw=2 set F2C1 ucs=0x%04x(%s),width=%d,mblen=%d,flag=0x%04x,wwExt=%d\n",UTT,Pucs,UCV_U2F1(Pucs),flag,width,mblen,wwExt);//~v7b3R~
		        UTRACEP("%s:@@@@set F2C1 NOT ambiguous and width=1 and wwExt>1\n",UTT);//~v7bbR~
//          }                                                      //~v7b3R~
        }                                                          //~v7b3I~
        else                                                       //~v7c7I~
        if (wwExt==0)                                              //~v7c7I~
        {                                                          //~v7c7I~
            UTRACEP("%s:set width=0 by Cf glyph width=%d,ucs=0x%04x(%s)\n",UTT,Pucs,wwExt,UCV_U2F1(Pucs));//~v7c7I~
            width=0;                                               //~v7c7I~
        }                                                          //~v7c7I~
//  }//TODO test                                                   //~v7b3R~
    }                                                              //~v7b3I~
...
//    if (width==2                                                   //~v79CI~//~vbzzR~
//    &&  flag==UTFWWO_MK_AMBIGUOUS  //x2000   //ambiguous only(exclude space combining)//~v79CI~//~vbzzR~
//    )                                                              //~v79CI~//~vbzzR~
//    {                                                              //~v79CI~//~vbzzR~
//        if (UTF_AMBIG2CELLMODE())   //2 cell mode for ambiguous    //~v79CI~//~vbzzR~
//        {                                                          //~v79CI~//~vbzzR~
//            return width;                  //else(1 cellmode) chkcursor//~v79CI~//~vbzzR~
//        }                                                          //~v79CI~//~vbzzR~
//    }                                                              //~v79CI~//~vbzzR~
    UTRACEP("%s:entry ucs=x%04x ( %s ),width=%d,flag=0x%04x,Gulibutfmode=0x%08x,GambigWidth=%d\n",UTT,Pucs,UCV_U2F1(Pucs),width,*Ppflag,Gulibutfmode,GambigWidth);//~vbzzR~
//    if (!GambigWidth) //no adjust by /G option                     //~vbzzR~//~vbzpR~
//    {                                                              //~vbzzR~//~vbzpR~
//    if (!(Gulibutfmode & GULIBUTF_APICHK_CSR)) //0x08000000  //utfwcwidth chr cursor step//~vbz4I~//~vbzpR~
//        return width;                                              //~vbz4R~//~vbzpR~
...
//    }                                                              //~vbzzR~//~vbzpR~
    ...
    {                                                              //~vbz4I~
      if (GambigWidth)  //No adjust required, 1 or 2 by /G option(-1 is processed already)//~vbzzR~
      	cursorstep=GambigWidth;                                    //~vbzzR~
      else                                                         //~vbzzR~
      {                                                            //~vbzzR~
    	UTRACEP("%s:callwxe getcursorwidth ucs=x%04x ( %s ),width=%d,flag=0x%04x,GambigWidth=%d\n",UTT,Pucs,UCV_U2F1(Pucs),width,*Ppflag,GambigWidth);//~vbzzR~
        ...
        {                                                          //~vbz4R~
            *Ppflag=0;                                             //~vbz4R~
            ...
            UTRACEP("%s:updated ucs=0x%04x (%s ),width=%d->%d,flag=0x%04x->0x%04x\n",//~vbz4R~
					UTT,Pucs,                                      //~vbz4I~
    			    UCV_U2F1(Pucs),         //~v79MR~              //~v7a7R~
					Pmkwidth,width,flag,*Ppflag);                  //~vbz4I~
        }                                                          //~vbz4R~
    }                                                              //~vbz4I~
#else                                                              //~vbzzI~
    width=utfwwapichkCursorW32(Popt,Pucs,width,Ppflag);            //~vbzzI~
#endif                                                             //~vbzzI~
    return width;                                                  //~vbz4I~
...
#endif                                                             //~v6WvI~
//    if (width!=-1 && !swUpdated)                                   //~vbzzI~//~vbzzR~
//    {                                                              //~vbzzI~//~vbzzR~
//        if (chkAmbiguous(GparmApichk,Pucs,&flag,&width))  //width was fixed by /G option//~vbzzR~//~vbzzR~
//            swUpdated=2;                                           //~vbzzI~//~vbzzR~
//    }//!updated by SCM                                             //~vbzzI~//~vbzzR~
//  if (swUpdated!=2)     //Not ambiguous is processed by /GN        //~vbzzI~//~vbzzR~
  if (GambigWidth==-1)                                             //~vbzzR~
    width=-1;                                                      //~vbzzR~
  else                                                             //~vbzzR~
  {                                                                //~vbzzI~//~vbzzR~
...
{                                                                  //~v6V4I~
	int width;                                                     //~vbzzI~
//    width=Pmkwidth;                                                //~vbzzI~//~vbzzR~
//    int flag=*Ppflag;                                              //~vbzzI~//~vbzzR~
//    if (chkAmbiguous(GparmApichk,Pucs,&flag,&width))               //~vbzzR~//~vbzzR~
//    {                                                              //~vbzzI~//~vbzzR~
//        UTRACEP("%s:ambiguous return width=%d\n",UTT,width);       //~vbzzR~//~vbzzR~
//        return width;                                              //~vbzzI~//~vbzzR~
//    }                                                              //~vbzzI~//~vbzzR~
//  return Pmkwidth;                                               //~v6V4I~//~vbzzR~
	width=utfwwapichk(Popt,Pucs,Pmkwidth,Ppflag,Ppwcwidth);        //~vbzzR~
    UTRACEP("%s:rc=width=%d\n",UTT,width);    //~vbzzR~            //~vbzzR~
    return width;                                                  //~vbzzR~
...
//*************************                                        //~v640I~
	...
	if (Pflag & UTFWWF_AMBDBCS)      //ambiguous mblen=2           //~v7a7I~
    	sbcsid=UCSDDID_AMBDBCS;                                    //~v7a7I~
    else                                                           //~v7a7I~
...
//*************************                                        //~v640I~
    ...
    if (Pucs==(ULONG)GutfTestUcs)                                  //~vbzzR~
    {                                                              //~vbzzR~
        UTRACEP("%s:Debug trap by GutfTestUcs ucs=%04x\n",UTT,Pucs);	//@@@@test//~vbzzR~
    }                                                              //~vbzzR~
  if (Pucs<0x80)                                                   //~v650I~
...
//  len=2;    //set UTFWWF_SBCSOVF                                 //~v6uBI~//~v6V4R~
	len=utfwcwidthNoMap(Popt,Pucs,&flag);                          //~v6V4I~
  	...
  	{                                                              //~v6V4I~
		UTRACEP("%s:W32,treate as SBCS overflow for len=1(not 0/2) ucs=%04x\n",UTT,Pucs);//~vbzzI~
...
//      UTRACEP("%s:@@@@F2C1 ucs=0x%04x\n",UTT,Pucs);              //~vbz5R~
    ...
    case UCSDDID_NARROWDBCS:         //wcwidth=2,narrow dbcs       //~v640R~
        ...
        len=2;                                                     //~v640I~
        break;                                                     //~v640I~
    case UCSDDID_AMBDBCS:         //mblen=2 ambigous               //~v7a7I~
		flag|=UTFWWF_AMBDBCS;                                      //~v7a7I~
        len=2;                                                     //~v7a7I~
        break;                                                     //~v7a7I~
    default:                                                       //~v640I~
#ifdef WWW    	                                                   //~v6BYI~
...
//  if (Pucs>=UCS4_START	//ucs4; treate as dbcs except ucs sbcsid was set//~v65cI~//~v6VyR~
    ...
    if (Ppflag)                                                    //~v640I~
	    *Ppflag=flag;                                              //~v640I~
    *Pplen=len;                                                    //~v640I~
    UTRACEP("%s:ucs=0x%04x,flag=0x%04x,len=%d\n",UTT,Pucs,flag,len);          //~v740R~//~vbz5R~//~vbzzR~
    return 0;                                                      //~v640I~
...
//  if (!(Gulibutfmode & GULIBUTF_APICHK_CSR))                     //~vbz4I~//~vbzpR~
//  	return;                                                    //~vbz4I~//~vbzpR~
...
//    if (!(Gulibutfmode & GULIBUTF_APICHK_CSR))  //chk fixed width by /G//~vbzpR~
//        return;                                                    //~vbz5I~//~vbzpR~
...
//    	Gulibutfmode|=GULIBUTF_APICHK_CSR; //0x08000000  //utfwcwidth chr cursor step;xe.c set this optionally //~v796I~//~vbz3R~
        UTRACEP("%s:Gulibutfmode0x%08x,Guvio2Stat=0x%04x\n",UTT,Gulibutfmode,Guvio2Stat);//~vbz3I~//~v7akR~
  		uvioGetCursorWidth(UVGCWO_OPEN,0);                         //~v796I~//~v798R~
    #endif                                                         //~v796I~
#endif                                                             //~v796I~
#ifdef LNXCON                                                      //~v7agI~
  	uvioGetCursorWidth(UVGCWO_OPEN,0);                             //~v7agI~
#endif                                                             //~v7agI~
    opt=UTFWWO_UTF8UCS2;                                           //~v640R~
//#ifdef LNX                                                       //~v640R~
//    opt|=UTFWWO_APIONLY;    //anyway                             //~v640R~
//#endif                                                           //~v640R~
    if (Gulibutfmode & GULIBUTFMODE_NOTRACEMAPINIT)                //~vbzuR~
    {                                                              //~vbzuI~
        UTRACEP_FLUSH("%s:Stop utrace by cmdline option\n",UTT);   //~vbzuR~
    	UTRACE_INIT(0/*fnm*/,UTRACEO_TEMPCLOSE);                   //~vbzuR~
    }                                                              //~vbzuI~
...
UTRACEP("%s:sbcsid ucs=0x%04x,sbcsid=0x%04x,width=%d,flag=0x%04x\n",UTT,ii,sbcsid,width,flag);//~v@@@R~//~v6Y0R~//~v79MR~//~v7a2R~//~v7a3R~
//      pud->UCS2DDsbcsid=(WUCS)sbcsid;                            //~v640I~//~v6B4R~
        pud->UCS2DDsbcsid=(USHORT)sbcsid;                          //~v6B4I~
    }                                                              //~v640I~
//    if (Gulibutfmode & GULIBUTFMODE_NOTRACEMAPINIT)              //~v7bqR~
//    {                                                            //~v7bqR~
//        UTRACE_INIT(0/*fnm*/,UTRACEO_REOPEN);                    //~v7bqR~
//        UTRACEP_FLUSH("%s:Reopen utrace stopped by cmdline option\n",UTT);//~v7bqR~
//    }                                                            //~v7bqR~
    if (Gulibutfmode & GULIBUTFMODE_NOTRACEMAPINIT)                //~v7bqI~//~v7zVM~
    {                                                              //~v7bqI~//~v7zVM~
    	UTRACE_INIT(0/*fnm*/,UTRACEO_REOPEN);                      //~v7bqI~//~v7zVM~
        UTRACEP_FLUSH("%s:Reopen utrace stopped by cmdline option\n",UTT);//~v7bqI~//~v7zVM~
    }                                                              //~v7bqI~//~v7zVM~
#ifdef WWW                                                         //~v6BYM~
...
#endif                                                             //~v796I~
#ifdef LNXCON                                                      //~v7agI~
	uvioGetCursorWidth(UVGCWO_CLOSE,0);                            //~v7agI~
#endif                                                             //~v7agI~
...
#endif                                                             //~v65cI~
//****************************                                     //~v640I~
    UTRACEP("%s:ucs=0x%04x,dbcsid=%d\n",UTT,Pucs,Pdbcsid);         //~v7acI~
...
//          *pcdo=UDBCSCHK_UCS42|ucs12;                            //~v6x8R~
        ...
        default:                                                   //~v640I~
            ...
            *pco++=(UCHAR)(ucs&0xff);   //big endian ucs           //~v640I~
	        rc=UTFCVFDRC_DBCS|UTFCVFDRC_UTF8;   //valid utf8 char  //~v640I~
//#ifdef WINCON                                                      //~v7acM~//~vbzpR~
            if (Pdbcsid==-UCSDDID_AMBDBCS) //ambiguous mblen=2     //~v7acI~
            {                                                      //~v7acI~
//                *pcdo++=UDBCSCHK_NDBCSUCS1;  //borrow for uviowrite 1 by 1//~v7acR~
//                *pcdo=UDBCSCHK_NDBCSUCS2;                        //~v7acR~
                *pcdo++=UDBCSCHK_WSBCSUCS1;    //narrow dbcs upper //~v7acI~
                *pcdo=UDBCSCHK_WSBCSUCS2;    //narrow dbcs lower   //~v7acI~
            }                                                      //~v7acI~
            else                                                   //~v7acI~
//#endif                                                             //~v7acM~//~vbzpR~
    ...
    UTRACEP("%s:outdata=x%02x,outdbcs=0x%02x\n",UTT,*Ppoutdata,*Ppoutdbcs);//~v7acI~
	return rc;                                                     //~v640I~
}//utfcvu2dd1w                                                     //~v640R~
//*******************************************************          //~v640I~
...
//      ucs=*Ppdata;                                               //~v660R~//~v66mR~
    if (Ppchsz)                                                    //~v65pI~
    	*Ppchsz=chsz;                                              //~v65pI~
UTRACED("utfdd2u1chsz inp data",Ppdata,chklen);                      //~v660I~//~v7bfR~//~vbArR~
UTRACED("utfdd2u1chsz inp dbcs",Ppdbcs,chklen);                      //~v660I~//~v7bfR~//~vbArR~
...
//**************************************************               //~v640M~//~v6BMI~
//*get ucs4(UWUCS) string from data/dbcs DDucs format                     //~v640M~//~v6BMI~//~v7bfR~
...
//  for (reslen=Plen/2,outreslen=Poutbuffsz;reslen>0 && outreslen>0;pucs++,reslen--,outreslen--)//~vbzMR~
    for (reslen=Plen/(int)sizeof(WUCS),outreslen=Poutbuffsz;reslen>0 && outreslen>0;pucs++,reslen--,outreslen--)//~vbzMR~
    {                                                              //~v640I~
        ucs=(ULONG)*pucs;                                          //~v640I~
...
//                  	ucs=altch;                                 //~v65iI~//~v6B4R~
                    	ucs=(ULONG)altch;                          //~v6B4I~
  	        			if (altch>=0x80 && (Popt & UTFDD2FO_SETDDDBCSALT))//     0x4000 //update dddbcs by altch//~vbArR~
                        {                                          //~vbArI~
                        	char wkaltdddata[4],wkaltdddbcs[4];    //~vbArI~
                            int altddlen;                          //~vbArI~
							UTF_U2DD1((wchar_t)ucs,wkaltdddata,wkaltdddbcs,&altddlen);//~vbArR~
                            UTRACEP("%s:src dd data=0x%02x,dbcs=0x%02x,altch=0x%04x,out dd data=0x%02x,dbcs=0x%02x\n",UTT,*pdata,*pdbcs,altch,wkaltdddata[0],wkaltdddbcs[0]);//~vbArR~
                        	*pdata=wkaltdddata[0];                 //~vbArI~
                        	*pdbcs=wkaltdddbcs[0];                 //~vbArI~
                        }                                          //~vbArI~
...
//*rc1:ucs data,0:no ucs(may contain locale)                       //~vbzMR~
//**************************************************               //~v640M~
...
//**************************************************               //~vbzMI~
//*chk dd/dbcs length                                              //~vbzMR~
//*rc1:ucs data and ascii,0:locale data and ascii, -1:all ascii    //~vbzMR~
//**************************************************               //~vbzMI~
int utfchkddlen(int Popt,UCHAR *Ppdbcs,int Plen,int *Pplen)        //~vbzMI~
{                                                                  //~vbzMI~
	UCHAR *pdbcs,dbcsid;                                           //~vbzMI~
    int reslen;                                                    //~vbzMI~
    int len=0,found=-1;                                            //~vbzMR~
//******************                                               //~vbzMI~
UTRACED("dbcs",Ppdbcs,Plen);                                       //~vbzMR~
	for (pdbcs=Ppdbcs,reslen=Plen;reslen>0;pdbcs++,reslen--)       //~vbzMI~
    {                                                              //~vbzMI~
    	dbcsid=*pdbcs;                                             //~vbzMI~
//UTRACEP("%s:len=%d,reslen=%d,dbcsid=0x%02x\n",UTT,len,reslen,dbcsid);//~vbzMR~
    	if (UDBCSCHK_ISUCSDBCST(dbcsid))                           //~vbzMI~
        {                                                          //~vbzMI~
//UTRACEP("%s:ddfmt found=%d\n",UTT,found);                        //~vbzMR~
            if (found==0)  //dbcs detected already                 //~vbzMI~
            {                                                      //~vbzMM~
        		*Pplen=len;                                        //~vbzMM~
//UTRACEP("%s:rc=1 outlen=%d,reslen=%d\n",UTT,len,reslen);         //~vbzMR~
        		return 0;                                          //~vbzMI~
            }                                                      //~vbzMM~
        	if (found<0) //1st dd fmt                              //~vbzMR~
            {                                                      //~vbzMI~
            	found=1;                                           //~vbzMI~
            }                                                      //~vbzMI~
        }                                                          //~vbzMI~
        else                                                       //~vbzMI~
    	if (UDBCSCHK_DBCSCHARS(dbcsid))//not dd fmt                //~vbzMI~
        {                                                          //~vbzMI~
//UTRACEP("%s:dbcs found=%d\n",UTT,found);                         //~vbzMR~
            if (found==1)   //ddfmt detected already               //~vbzMI~
            {                                                      //~vbzMM~
        		*Pplen=len;                                        //~vbzMM~
//UTRACEP("%s:rc=0 outlen=%d,reslen=%d\n",UTT,len,reslen);         //~vbzMR~
        		return 0;                                          //~vbzMM~
            }                                                      //~vbzMM~
        	if (found<0)                                           //~vbzMI~
            {                                                      //~vbzMI~
    ...
    *Pplen=len;                                                    //~vbzMI~
UTRACEP("%s:rc=%d no dbcs/ucs outlen=%d,reslen=%d\n",UTT,found,len,reslen);//~vbzMR~
    return found;	//no ucs2,all sbcs local string                //~vbzMR~
}//utfchkddlen                                                     //~vbzMR~
//**************************************************               //~vbzMI~
//*chk dd/dbcs length                                              //~vbzMI~
//*rc1:ucs data and ascii,0:locale data and ascii, -1:all ascii    //~vbzMI~
//**************************************************               //~vbzMI~
int utfchkddpos(int Popt,UCHAR *Ppdbcs,int Plen,int *Pplen)        //~vbzMI~
{                                                                  //~vbzMI~
	UCHAR *pdbcs,dbcsid;                                           //~vbzMI~
    int len=0,found=0,pos=-1,rc,ii;                                //~vbzMR~
//******************                                               //~vbzMI~
UTRACED("dbcs",Ppdbcs,Plen);                                       //~vbzMI~
	for (pdbcs=Ppdbcs,ii=0;ii<Plen;pdbcs++,ii++)                   //~vbzMI~
    {                                                              //~vbzMI~
    	dbcsid=*pdbcs;                                             //~vbzMI~
    	if (UDBCSCHK_ISUCSDBCST(dbcsid))                           //~vbzMI~
        {                                                          //~vbzMI~
        	if (!found) //1st dd fmt                               //~vbzMI~
            {                                                      //~vbzMI~
            	pos=ii;                                            //~vbzMI~
            	found=1;                                           //~vbzMI~
            }                                                      //~vbzMI~
...
//200c/200d is not combinable by category:Format                   //~v7bBI~
...
//UICRC_COMB2SCM:08     //3099(handakuten)/309a(dakuten)           //~v7zVI~
      	}                                                          //~v6X5I~
    }                                                              //~v6WrI~
    else                                                           //~v6WrI~
    {                                                              //~v6WrI~
        ...
        {                                                          //~v7bVI~
			if (Popt & UICO_COMBONLY)    //confirm defined combining//~v7bVR~
                ...
                {                                                  //~v7bVI~
                    ...
                    {                                              //~v7bVI~
				    	UTRACEP("%s:rc=0 by utf_iswidth0=True but mk_wcwidth_combining=False,ucs=%06x,opt=0x%02x\n",UTT,Pucs,Popt);//~v7bVI~
					    rc|=UICRC_NOCOMB;// 0x08    //wcwidth=0 but not combining//~v7bVI~
                    }                                              //~v7bVI~
                    else                                           //~v7bVI~
                    {                                              //~v7bVI~
				    	UTRACEP("%s:rc=0 by utf_iswidth0=True but mk_wcwidth_combining=False,ucs=%06x,opt=0x%02x\n",UTT,Pucs,Popt);//~v7bVR~
...
//***************************************************************************//~v7zVI~
int utf_iscombiningDD(int Popt,char *Pdddata,char *Pdddbcs,int Plen)//~v7zVI~
{                                                                  //~v7zVI~
    ...
    {                                                              //~v7zVI~
        if (UDBCSCHK_ISUCSSBCS(*Pdddbcs)) //                       //~v7zVR~
    ...
    UTRACEP("%s:opt=%x,rc=%d,ucs=%06x,dddata=0x%02x,dddbcs=0x%02x\n",UTT,Popt,rc,ucs4,*Pdddata,*Pdddbcs);//~v7zVI~
    return rc;                                                     //~v7zVI~
}//utf_iscombiningDD                                               //~v7zVI~
