...
//                  );                                             //~va3kR~
    ...
    {                                                              //~va30I~
      ...
      {                                                            //~vbDpI~
        UTRACEP("%s:WXEIcsrposx=%d\n",UTT,Mwxeintf.WXEIcsrposx);   //~vbDpI~
...
//  for (col=0;col<totlen;col++,pattr++,pdbcs++)                   //~v780R~
    ...
    {                                                              //~v003I~
        if (inboxsw>=0)	//row is in the cap range                  //~2A05R~
            ...
            inboxswn=-1;                                           //~2A05R~
#ifdef TEST                                                        //~vbDmI~
		if (col==csrposnext)	//get char width to write as single//~vbDmI~
...
#ifdef TEST                                                        //~vbDmI~
		||  (col>csrposnext && chszNext>0)                         //~vbDmR~
#endif                                                             //~vbDmI~
...
#ifdef TEST                                                        //~vbDmI~
            if (col>csrposnext)                                    //~vbDmM~
            {                                                      //~vbDmI~
                nextcolstart=colstart+chszNext;                    //~vbDmI~
                UTRACEP("%s:col=%d,nextstart=%d,chszNext=%d,colstart=%d\n",UTT,col,nextcolstart,chszNext,colstart);//~vbDmR~
            	chszNext=0;                                        //~vbDmM~
            }                                                      //~vbDmI~
#endif                                                             //~vbDmI~
...
//          strput(Ppmemdc,Pprintsw,Prow,colstart,len,attr,style,inboxsw);//~vbDpR~
            strput(optStrput,Ppmemdc,Pprintsw,Prow,colstart,len,attr,style,inboxsw);//~vbDpI~
        	attr=*pattr;                                           //~v003I~
        	style=*pstyle;                                         //~v780I~
//          colstart=col;                                          //~v69SR~
...
//int  CWxemain::strput(CDC *Ppmemdc,int Pprintsw,int Prow,int Pcol,int Plen,USHORT Pattr,int Pstyle,int Pcapsw)//~vbDpR~
int  CWxemain::strput(int PoptStrput,CDC *Ppmemdc,int Pprintsw,int Prow,int Pcol,int Plen,USHORT Pattr,int Pstyle,int Pcapsw)//~vbDpI~
{                                                                  //~2901I~
...
//	rc_ligature=wxe_chkligaturerow(0,NULL,Prow,Pcol,&rrow);        //~vb4iR~
    ...
    {                                                              //~2A27M~
UTRACEP("%s:strput optStrput=%02x,row=%d,col=%d,style=%d,attr=%04x,len=%d\n",UTT,PoptStrput,Prow,Pcol,Pstyle,Pattr,Plen);//~v780I~//~vbDpR~
...
UTRACEP("%s:selectfont nosyntax normal style=%d,font=%p\n",UTT,Pstyle,Mpfont);//~va74R~//~vbzMR~
		}                                                          //~va74I~
        else                                                       //~v780I~
        {                                                          //~va74I~
...
//	csubtextoutw(opt,lineopt,Ppmemdc->m_hDC,xx,yy,pdata,pdbcs,Plen,cellw);//~vbDpR~
  	csubtextoutw(opt,lineopt,Ppmemdc->m_hDC,xx,yy,pdata,pdbcs,Plen,cellw,opt2);//~vbDpR~
  }                                                                //~va20M~
  else                                                             //~va20M~
#endif                                                             //~va20M~
...
{                                                                  //~vbzMI~
    ...
    int lineopt=wxe_getlineopt(0,Prow,Pcol);                       //~vbzMI~
    if (WXE_LIGATUREMODE(lineopt))                                 //~vbzMI~
    {                                                              //~vbzMI~
//  	if (lineopt & UVIOO_CSRPOSCHK)                             //~vbzMR~
//			if (!xxe_chkcsrpos(XXECCPO_STRCHK,Prow,Pcol,0/*fileinfo*/))//chk disable ligature for each str//~vbzMR~
        if (!(lineopt & UVIOO_LIGATURE2))  //errmsg/cmdline        //~vbzMI~
        	if (lineopt & UVIOO_LIGATURE1)                         //~vbzMI~
				swlocaleligature=1;                                //~vbzMI~
    }                                                              //~vbzMI~
	UTRACEP("%s:row=%d,col=%d,lineopt=0x%02x,swlocalligature=%d\n",UTT,Prow,Pcol,lineopt,swlocaleligature);//~vbzMR~
    int opt=Popt;                                                  //~vbzMR~
    if (swlocaleligature)                                          //~vbzMI~
    	opt|=WXEHCO_LIGATURE;                                      //~vbzMI~
	csubtextout_locale_ligatureHC(opt,Ppwxei,Ppdata,Ppdbcs,Plen,Ppucs,Pbuffsz,&ucsctr);//~vbzMR~
    rc=0;                                                          //~vbzMI~
    if (Ppucsctr)                                                  //~vbzMI~
    	*Ppucsctr=ucsctr;                                          //~vbzMI~
	UTRACEP("%s:rc=%d,ucsctr=%d\n",UTT,rc,*Ppucsctr);              //~vbzMM~
    UTRACED("out ucs",Ppucs,ucsctr*sizeof(WUCS));                  //~vbzMI~
    return rc;                                                     //~vbzMI~
}                                                                  //~vbzMI~
//===============================================================================//~vbzMM~
//=for nondbcs ucs of ddfmt                                        //~vbzMI~
//===============================================================================//~vbzMI~
int  strputHC(int Popt,UCHAR *Ppdata,UCHAR *Ppdbcs,int Plen,WUCS *Ppucs,int *Ppucsctr)//~vbzMR~
{                                                                  //~vbzMM~
    char   *pdata,*pdbcs;                                          //~vbzMM~
    int  ii,ucsctr=0;                                              //~vbzMI~
    WUCS ucs,*pucs;                                                //~vbzMR~
    WUCS ucsalt;                                                   //~vbzMM~
    int optaltch,swaltch;                                          //~vbzMM~
//************************************                             //~vbzMM~
    UTRACEP("%s:len=%d\n",UTT,Plen);                               //~vbzMR~
    pdata=(char*)Ppdata;                                           //~vbzMI~
    pdbcs=(char*)Ppdbcs;                                           //~vbzMM~
    UTRACED("data",pdata,Plen);                                    //~vbzMM~
    UTRACED("dbcs",pdbcs,Plen);                                    //~vbzMM~
    pucs=Ppucs;                                                    //~vbzMI~
    ucsalt=0;                                                      //~vbzMI~
    for (ii=0;ii<Plen;ii++,pdata++,pdbcs++)                        //~vbzMR~
    {                                                              //~vbzMI~
        if (!*pdata)                                               //~vbzMI~
            *pdata=' ';                                            //~vbzMI~
        ucs=(WUCS)*pdata;                                          //~vbzMI~
        optaltch=UTF_GETALTCH_SBCSID(*pdbcs);                      //~vbzMI~
        if (optaltch>=0)    //not tab/unp                          //~vbzMI~
            swaltch=csubtextoutw1alt(optaltch,0/*hDC*/,0/*xx*/,0/*yy*/,ucs);    //alt ch not written//~vbzMR~
        else                                                       //~vbzMI~
        if (ucs<0x20)                                              //~vbzMI~
            swaltch=csubtextoutw1alt(0,0/*Ppmemdc->m_hDC**/,0/*xx*/,0/*yy*/,ucs);//~vbzMI~
        else                                                       //~vbzMI~
            swaltch=0;                                             //~vbzMI~
        if (swaltch)                                               //~vbzMR~
            *pucs=swaltch;                                         //~vbzMI~
    ...
    ucsctr=PTRDIFF(pucs,Ppucs)/sizeof(WUCS);                       //~vbzMR~
    *Ppucsctr=ucsctr;                                              //~vbzMI~
    UTRACEP("%s:ucsctr=%d\n",UTT,ucsctr);                          //~vbzMI~
    UTRACED("out ucs",Ppucs,ucsctr*sizeof(WUCS));                  //~vbzMI~
    return 0;                                                      //~vbzMM~
...
//===============================================================================//~vbzMI~
//int csubtextoutw(int Popt,int Plineopt,HDC Phdc,int Px,int Py,char *Pdata,char *Pdbcs,int Plen,int Pcellw)//~vbzMM~
//===============================================================================//~vbzMI~
int wxescr_u2altch(int Popt,WXEINTF *Ppwxei,int Prow,int Pcol,UCHAR *Ppdata,UCHAR *Ppdbcs,int Plen,WUCS *Ppucs,int Pbuffsz,int *Ppucsctr)//~vbzMR~
{                                                                  //~vbzMI~
	int rc,ucsctr,ucsctrT=0,ddlen;                                 //~vbzMR~
    UCHAR *pc,*pcd,*pcde;                                          //~vbzMR~
    WUCS *pucs;                                                    //~vbzMI~
    //*****************                                            //~vbzMI~
    ...
    for (;pcd<pcde;)                                               //~vbzMI~
    {                                                              //~vbzMI~
    	int reslen=PTRDIFF(pcde,pcd);                              //~vbzMR~
    	int pos=csubchkddpos(0,pcd,reslen,&ddlen);	               //~vbzMR~
        if (pos<0)                                                 //~vbzMI~
        	pos=reslen;                                            //~vbzMI~
        if (pos>0)	//locale str before ucs dd                     //~vbzMI~
        {                                                          //~vbzMI~
        	strputHC(Popt,pc,pcd,pos,pucs,&ucsctr);                //~vbzMR~
            pucs+=ucsctr;                                          //~vbzMI~
	        pc+=pos;                                               //~vbzMR~
    	    pcd+=pos;                                              //~vbzMR~
        	ucsctrT+=ucsctr;                                       //~vbzMI~
        }                                                          //~vbzMI~
        if (ddlen>0)                                               //~vbzMR~
        {                                                          //~vbzMI~
            ucsctr=csubtextoutwHC(Popt,pc,pcd,ddlen,pucs);         //~vbzMR~
            pucs+=ucsctr;                                          //~vbzMR~
            pc+=ddlen;                                             //~vbzMR~
            pcd+=ddlen;                                            //~vbzMR~
            ucsctrT+=ucsctr;                                       //~vbzMR~
        }                                                          //~vbzMI~
    }                                                              //~vbzMI~
    if (Ppucsctr)                                                  //~vbzMI~
    	*Ppucsctr=ucsctrT;                                         //~vbzMI~
    rc=0;                                                          //~vbzMI~
	UTRACEP("%s:rc=%d,ucsctr=%d\n",UTT,rc,ucsctrT);                //~vbzMR~
...
{                                                                  //~vbDmI~
    int len;                                                       //~vbDmI~
    if (*Ppcd==UDBCSCHK_DBCS1ST)	//cplc                         //~vbDmI~
    	len=UDBCSCHK_PCD_MBCHSZ(Ppcd,Plen);    //1 if err          //~vbDmR~
    else					                                       //~vbDmI~
	if (UDBCSCHK_DBCS1STUCS2NW(*Ppcd) && Plen>1)                   //~vbDmI~
    	len=2;                                                     //~vbDmI~
    else                                                           //~vbDmI~
    	len=1;                                                     //~vbDmI~
	UTRACEP("%s:rc=%d,Plen=%d,*Ppcd=%02x\n",UTT,len,Plen,*Ppcd);   //~vbDmR~
    return len;                                                    //~vbDmI~
}                                                                  //~vbDmI~
