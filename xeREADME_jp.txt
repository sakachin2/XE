  ...
  .v129T<--129S:(2022/12/04)                                           ||~v29TI~||~v29VR~
     ...
     1.BUGs                                                           ||~v29TI~
       -=0.1 画面で sc 行の変更ができない                             ||~v29TI~
        また S+C/S+A を設定してもできない S+ なしと同一視される       ||~v29TI~
        そのことで wxe でマウスによるカット&ペーストが効かなくなる    ||~v29TI~
       -=0.2 画面で 行を削除するとキー割り当てができなくなる          ||~v29TI~
                                                                      ||~v29TI~
     2.機能追加                                                       ||~v29TI~
       -シンタックスハイライト                                        ||~v29TI~
		Andre Simon の Highlight、GNU の source-highlightのANSI:反転属性をサポート。||~v29TI~
		また xesynl.cfg/::xesynl.cfg の C_UNDERLINE=R/G_UNDERLINE=R 指定で||~v29TI~
		アンダーライン属性を反転とすることも可能                      ||~v29TI~
       -CIDプレフィックスでユニコードをサポート                       ||~v29TI~
		APL 言語では u-235d(⍝) がコメントプレフィックスなのでこれに対応。||~v29TI~
     ...
     3.その他                                                         ||~v29TI~
       -Windows Terminal が Windows11 で 省略値のコマンドプロンプト PGM となった様ですが||~v29TI~
        行コマンドで不具合が生じることがあります。                    ||~v29TI~
		旧コマンドプロンプトの c:\Windows\System32\conhost.exe を使用してください||~v29TI~
       -CID コマンドで APPEND のリセットを簡単にするため NOAPPEND オプションを追加||~v29TI~
       -=0.1 画面で "#" で始まるコメント記述をサポート。              ||~v29TI~
       -=0.2、=0.3 画面の見直し。                                     ||~v29TI~
        罫線文字も見直し、ユニコードの省略値を追加                    ||~v29TI~
